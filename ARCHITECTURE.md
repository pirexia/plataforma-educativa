...
- **Backoffice de plataforma** (`REQ-BO-007`, `ADR-046`, paso 1.6): mismo monolito y mismo despliegue de API, con *guard* de sesión (`platform`), grupo de rutas (`/api/platform/v1`) y —en el paso de interfaz posterior a 1.7/1.9— SPA propios; **no** un servicio desplegable aparte (`ADR-002` no se toca — la Opción B se descartó por duplicar la resolución de dependencias de módulos entre dos despliegues). Separación de dominio real por `Host()` en Traefik, no solo por prefijo de ruta: sin esa regla, el *host* de un centro alcanzaría el mismo grupo de rutas. Cuatro barreras independientes: `Host()` en Traefik, lista blanca de IP en el *ingress* (`ipallowlist`), `RequirePlatformHost` en la aplicación (`404`, antes de sesión y credenciales) y *guard*/cookie/tabla de sesión distintos (`platform_sessions`, no `sessions`). MFA obligatorio para los cuatro roles internos, sin excepción. `App\Support\Tenancy::runAsPlatform()` exige propósito declarado (`PlatformAccessPurpose`) y ausencia de tenant activo — es infraestructura compartida por todo el producto, no código del módulo.
