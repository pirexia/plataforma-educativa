...
| Auditoría | Tabla `audit_logs` polimórfica, append-only, con `REVOKE UPDATE, DELETE` a nivel de motor para todos los roles de aplicación — inmutabilidad forzada, no por convención | `ADR-034` §3 |
| Datos de categoría especial (salud, NEAE, convivencia) | Se modelan en tablas separadas, cifradas, con permisos propios y auditoría de lectura. La política de redacción de `audit_logs` nunca registra el valor de un campo de categoría especial, solo qué atributo cambió | `CLAUDE.md` §8, `ADR-034` §3 |
