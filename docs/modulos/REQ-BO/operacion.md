...
| `BO_HEALTH_FAILED_JOBS_WINDOW` | Ventana, en horas, del recuento «fallos recientes» de la ficha (`funcional.md §5.9.2`) | Sub-paso `1.6d`. **Acotada de hecho por la retención de `failed_jobs`**: pedir 72 horas cuando `queue:prune-failed` borra a las 24 devuelve siempre lo mismo que pedir 24. Por eso el valor viaja en la respuesta (`api.md §2.10.1`) en vez de darse por sabido |
...
| `queue.failed.database` | `env('DB_CONNECTION')`, es decir **`pgsql` = `plataforma_app`** | **El proveedor del framework apunta al rol que perdió los privilegios en `0.7`**: `queue:retry`, `queue:failed` y `queue:prune-failed` no pueden funcionar (`funcional.md §5.9.6`) |
...
**`1.6d` no añade ni un trabajo en cola ni una tarea programada, y es el único de los cinco del que eso es cierto.** Es de lectura salvo por el reintento, y ese reintento **se ejecuta dentro de la petición** (`RN-BO-89`): es un `INSERT` en `jobs` y un `DELETE` en `failed_jobs`, y `INV-012` habla de trabajo pesado — encolar un reintento sería encolar el encolado. **Si en la implementación aparece un *job* de «consolidar métricas» o de «recalcular salud», el diseño se ha desviado de `funcional.md §5.10.3`** y hay que volver a él, no añadir el *job*: `RN-BO-95` decide explícitamente que ninguna métrica se materializa.

> **Y una tarea que sí existe, que no es de este módulo y que le afecta de lleno**: `queue:prune-failed --hours=24`, programada a diario en `routes/console.php` por el issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73). Fija la retención de `failed_jobs` en 24 horas —y con ella, cuánto ve la ficha de salud hacia atrás— y **hoy no puede borrar nada**, por el mismo defecto de conexión de `funcional.md §5.9.6`. Su arreglo es una decisión de alcance (`funcional.md §15.4`), y hasta que ocurra, `failed_jobs` crece sin límite y la retención documentada no es la real.
...
| **`1.6d`** · «`queue:prune-failed` corre a diario y `failed_jobs` no para de crecer» | Si la purga da error de privilegios en el registro del planificador | `funcional.md §5.9.6`, hallazgo **Alta**: esa purga usa el mismo proveedor sobre `plataforma_app`, que tiene `REVOKE DELETE`. **Lleva sin borrar nada desde `0.7`**, y es la segunda capa del issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73) — tokens de un solo uso conservados más allá de las 24 horas que se decidió conservarlos |
...
| `SYSADMIN.md` (`1.6d`) | **`APP_VERSION` tiene que fijarse en el despliegue** desde la etiqueta de imagen de `ADR-037`, o la ficha de salud devuelve `0.1.0` para siempre (§2). Y la pila de colas real de §3.1: qué tablas hay, por qué conexión se leen y **por qué `queue:retry`/`queue:failed`/`queue:prune-failed` no sirven** en este proyecto |
...
| `PRIVACY.md` (`1.6d`) | **La retención de 24 horas de `failed_jobs` está documentada y no aplicada** (`funcional.md §5.9.6`, hallazgo Alta). Mientras ese defecto viva, los *payloads* cifrados de los correos de `REQ-AUTH` —con su token— se conservan **indefinidamente**. Tiene que constar como riesgo conocido con su fecha, no aparecer en una auditoría |
...
**Sobre la reversión de `1.6d` en concreto: no hay nada que revertir en base de datos.** Es el único sub-paso sin una sola migración (`datos.md §14`), así que revertir su código retira cinco rutas de lectura y una de escritura, y nada más. **Dos consecuencias que sí conviene tener presentes**: las entradas `job.reintentado` que ya se hayan escrito **se quedan** en `admin_action_logs`, que es *append-only* y donde son prueba de qué se reejecutó y por orden de quién; y `bo:retry-provisioning` **vuelve a estar roto**, porque su arreglo va con este código — así que, si se revierte con un centro atascado en `en_alta`, el único camino que queda es volver a desplegar.
