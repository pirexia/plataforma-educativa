...
| **`APP_VERSION`** | Versión desplegada, que la ficha de salud devuelve en su bloque de plataforma (`funcional.md §5.9.2`) | **Ya existe** (`config/app.php`: `env('APP_VERSION', '0.1.0')`) y ya la sirve `GET /api/health`. Lo que `1.6d` añade no es la variable: es **la obligación de que el despliegue la fije** desde la etiqueta de imagen de `ADR-037`. Sin fijarla, la ficha devuelve `0.1.0` en todos los entornos para siempre — un dato que **parece** medido y no lo es, que es justo lo que `RN-BO-83` existe para impedir. Va en el `EnvironmentFile` de la unidad, junto al resto |
| `BO_HEALTH_FAILED_JOBS_WINDOW` | Ventana, en horas, del recuento «fallos recientes» de la ficha (`funcional.md §5.9.2`) | Sub-paso `1.6d`. **Acotada de hecho por la retención de `failed_jobs`**: pedir 72 horas cuando `bo:purge-failed-jobs` borra a las 24 devuelve siempre lo mismo que pedir 24. Por eso el valor viaja en la respuesta (`api.md §2.10.1`) en vez de darse por sabido |
| **La retención de `failed_jobs`** | 24 horas (issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73)) | **No es una variable y no debe serlo** (`RN-BO-98`): vive como constante literal en `config/backoffice.php`, sin `env()`. Sería la primera clave de ese fichero que no lee del entorno, y ése es el punto — ver la tabla de abajo |
| `BO_FLAG_CACHE_TTL` | Vida de una entrada de la caché de evaluación de *flags*, en segundos (§4.3) | Sub-paso `1.6e`. **Es una red de seguridad, no el mecanismo de invalidación**: éste es `rules_version`, y funciona en el acto. El TTL sólo limita cuánto sobreviven las entradas huérfanas de versiones anteriores. Del orden de minutos. **Sujeta a `OPEN-BO-24`**: si se decide no cachear, esta variable no llega a existir; si se decide cachear el catálogo en **una** entrada, pasa a ser la vida de esa entrada y no la de N |
| **`CACHE_STORE`** | Almacén de caché de toda la aplicación. **Valor decidido: `redis`** | **No es una variable de este módulo y aun así `1.6e` tuvo que nombrarla** (§4.3.1): es el primer sub-paso que pone una caché en el camino de petición de un centro, y **ningún fichero del repositorio la fijaba** — `config/cache.php` cae a `database`, mientras `phpunit.xml` fuerza `redis`. **`OPEN-BO-26`, resuelta el 2026-09-21: se fija `CACHE_STORE=redis` para el *runtime*, no sólo para los tests.** Redis ya estaba desplegado para el prefijo por tenant (`ADR-033 §9`) y el límite de tasa; lo que faltaba era declararlo. La corrección de `compose.yaml` y de las unidades Quadlet va por el issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236), **fuera del ámbito de esta especificación**. **Deuda residual anotada en ese issue**: el cliente (`predis/predis` o `ext-redis`) sigue sin declararse en `composer.json` |
...
| Conmutador para **reintentar automáticamente** los trabajos fallidos de un centro (`1.6d`) | Un reintento reejecuta efectos secundarios —correos a familias incluidos— y **no se deshace** (`OPEN-BO-21`). Automatizarlo significa reenviarlos sin que nadie haya mirado por qué fallaron, y a la tercera vez el fallo ya no es un incidente sino un ruido de fondo. El reintento es una decisión de una persona, con su motivo y su entrada en `admin_action_logs` (`RN-BO-88`) |
| Variable para **alargar la retención de `failed_jobs`** más allá de 24 horas (`1.6d`) | Los *payloads* de `SendPasswordResetEmail` y `SendAccountLockedEmail` llevan un token de un solo uso; están cifrados (`ShouldBeEncrypted`), y el issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73) decidió además **no conservarlos más de 24 horas**. Una variable que lo alargue anula esa segunda capa **sin que nadie lo decida explícitamente**, y se pondría a un valor cómodo «para depurar» en el mismo entorno que un día es producción. Mismo argumento exacto que con los 90 días del período de gracia (`RN-BO-61`), y por eso el plazo es constante y no configuración (`RN-BO-98`) |
| Variable para apuntar `queue.failed.database` a **`pgsql_platform`** (`1.6d`) | Parece el arreglo obvio de los tres hallazgos de `funcional.md §5.9.6` y es el equivocado: le daría al *worker* —que sólo necesita `INSERT` para registrar un fallo— una conexión con **`BYPASSRLS`**, deshaciendo la mitad del endurecimiento de `0.7`. El camino correcto es propio del módulo y corre por `pgsql_platform` **sólo donde hace falta** (`datos.md §14.3`) |
...
| **Redis** | Límite de tasa y las **cachés que hay que invalidar** (§4), incluida la de evaluación de *flags*. **No la sesión de plataforma** y **tampoco la cola**: `QUEUE_CONNECTION` es `database` (§3.1) | Ver §4: el modo de fallo importa |
| ***Worker* de colas** | Procesa lo que la ficha de salud observa. **No hay ninguno desplegado todavía** (issue [#128](https://github.com/pirexia/plataforma-educativa/issues/128)) | **`1.6d` no depende de que lo haya**: lee dos tablas que se llenan solas con el *driver* `database`. Lo que un *worker* ausente produce —`jobs` creciendo y `failed_jobs` vacío— **es exactamente lo que la ficha enseña**, y verlo es el motivo de que exista |
...
### 3.1 La pila de colas, tal como está de verdad (`1.6d`)

**Va aquí porque `funcional.md §5.9` decía que los trabajos en cola salen «de Horizon», y es falso.** Verificado el 2026-09-16 sobre `composer.json`: **no hay `laravel/horizon`**, no hay `config/horizon.php` y no hay ninguna unidad suya en `infra/`. `CLAUDE.md §1` lo nombra en la tabla de *stack* como **intención**, y una intención no es una dependencia instalada (`OPEN-BO-23`).

| Pieza | Valor real | Qué implica para `1.6d` |
|---|---|---|
| `QUEUE_CONNECTION` | `database` por defecto (`config/queue.php`) | Los trabajos pendientes viven en la tabla **`jobs`**, no en Redis |
| `queue.failed.driver` | `database-uuids` sobre **`failed_jobs`** | Los fallidos viven en una tabla de plataforma ya endurecida (`datos.md §14.3`) |
| `queue.failed.database` | `env('DB_CONNECTION')`, es decir **`pgsql` = `plataforma_app`** | **El proveedor del framework apunta al rol que perdió los privilegios en `0.7`**: `queue:retry`, `queue:failed` y `queue:prune-failed` no pueden funcionar (`funcional.md §5.9.6`). **`1.6d` no cambia esta clave** —hacerlo daría `BYPASSRLS` al *worker*, ver §2— sino que construye el camino propio del módulo por `pgsql_platform` |
| Panel de observabilidad | **Ninguno** | El observatorio de colas del producto **es** la ficha de salud de `1.6d`, y es todo el que hay |

> **Adoptar Horizon no es gratis y no es de este sub-paso** (`OPEN-BO-23`): es una dependencia nueva que `CLAUDE.md §1` obliga a justificar y a envolver tras interfaz propia, exige mover la cola a Redis, y trae **un panel web con su propia autenticación** — una tercera superficie HTTP, después de la del producto y la de plataforma, que habría que dejar fuera del *host* del backoffice o dentro de él con su propia autorización. Con cero centros reales y ni un *worker* desplegado, es construir el termómetro antes que el paciente.
...
**Tres detalles que `1.6c` fija y que deciden si esto funciona o sólo lo parece** (`funcional.md §5.8.6`):

| # | Detalle | Qué pasa si se ignora |
|---|---|---|
| 1 | **La invalidación va en una fase aparte, fuera del bloque de plataforma y después del `COMMIT`** (`RN-BO-75`) | No es una preferencia: `runAsPlatform()` **lanza** si hay tenant activo (`ADR-046 §6.4`), y `runFor()` es justamente entrar en uno. Un diseño de una sola fase acaba componiendo el prefijo `t{id}:` a mano, que es lo que §4.2 prohíbe |
| 2 | **Si la invalidación falla, la petición no falla** (`RN-BO-76`) | Devolver `5xx` por algo ya escrito y ya auditado invita a reintentarlo. Se registra, se emite la señal de §7 y el **TTL de 300 s es el peor caso acotado** — que es por lo que ese TTL no debe subirse |
| 3 | **La carrera residual está declarada, no cerrada** | Un lector que leyó antes del `COMMIT` puede cachear el valor anterior después del `forget`. Acotada por el mismo TTL, y es la misma que §4.1 acepta con 60 s. **Si algún día importa, la solución es la clave versionada de §4.3, no un TTL más corto** |
...
#### 4.3.1 Dos cosas que hay que comprobar **antes** de construir esta caché (`1.6e`)

> **Lo añade la pasada de `1.6e`, y la primera es el hallazgo del sub-paso.** Lo que sigue no reabre el mecanismo de `rules_version` —que es correcto— sino los dos supuestos sobre los que descansa y que nadie había comprobado. Detalle completo en `funcional.md §5.11.9`.

**1 · El almacén de caché configurado no era Redis · `OPEN-BO-26`, RESUELTA 2026-09-21.** Verificado el 2026-09-21: `config/cache.php` tiene `'default' => env('CACHE_STORE', 'database')`, **ningún fichero del repositorio fijaba `CACHE_STORE`** —`infra/quadlet/plataforma.env.example` y `infra/compose/compose.prodlike.yaml` fijan `REDIS_HOST` y nada más—, `composer.json` **no declara `predis/predis` ni `ext-redis`**, y `phpunit.xml` **sí** fuerza `CACHE_STORE=redis`.

> **Decisión del usuario del 2026-09-21: se fija `CACHE_STORE=redis` para el *runtime* real.** Redis ya estaba desplegado y en uso —prefijo por tenant de `ADR-033 §9`, límite de tasa—, así que lo que faltaba no era la pieza sino declararla. Corrección de `compose.yaml` y de las unidades Quadlet por el issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236), fuera del ámbito de esta especificación.
>
> **Consecuencia: la caché de evaluación de *flags* se construye sobre Redis, y §4.3 queda vigente sin una sola corrección.** Es la diferencia con `OPEN-BO-23`, donde la salida fue corregir la documentación porque Horizon no estaba: aquí la pieza sí estaba, y lo que faltaba era usarla. **`CLAUDE.md §1` no necesita corrección** — con la variable fijada, «Caché: Redis, instalado y en uso» dice la verdad.
>
> **Deuda residual, anotada en el issue #236 y no convertida en pregunta nueva**: el cliente de Redis sigue **sin declararse** en `composer.json`. Existe en la imagen —la suite corre sobre Redis y pasa—, pero nada impide que un cambio de la imagen base lo retire y el fallo aparezca en ejecución en vez de en el despliegue (`CLAUDE.md §1`, `RNF-MANT-007`). **La comprobación operativa que sustituye a la declaración, mientras no la haya**: tras cualquier cambio de imagen, verificar que la aplicación arranca con `CACHE_STORE=redis` y no cae en silencio a otro almacén.

**Lo que el hallazgo decía, y que se conserva porque explica por qué había que preguntarlo:**

| | Consecuencia |
|---|---|
| **Sobre el almacén `database`** | Una lectura de caché es **una consulta SQL a la tabla `cache`**. Cachear la evaluación de un *flag* dejaría de ser un ahorro: sustituiría una consulta a `feature_flag_rules` —decenas de filas, con índice— por una consulta a `cache` más una escritura en cada fallo |
| **Sobre los tests** | Corren con Redis forzado, así que **una caché medida en verde lo estaría sobre un almacén que el producto no usa por defecto**. Es la forma exacta de la incidencia de `1.6d`, donde la purga estaba «decidida, documentada y no aplicada» (§6.2) |
| **Sobre `§4.1`, detalle 3** | Aquel aviso decía que la caché tiene que ser **compartida**. `database` lo es, así que ese aviso se cumple. Éste es el escalón siguiente: además de compartida, tiene que ser **barata**, porque `1.6e` es el primer sub-paso que la pone en el camino caliente |

**No se afirmó que el despliegue corriera sobre `database`** —`CACHE_STORE` podía estar puesta en el `EnvironmentFile` real, que no está en el repositorio— sino lo comprobable, que es lo que `CLAUDE.md §1` obliga a distinguir desde la v2.5.2: **el valor de respaldo era `database`, nada en el repositorio lo cambiaba, y `CLAUDE.md §1` describe Redis como «instalado y en uso»**. Era la misma clase de discrepancia que `OPEN-BO-23` encontró con Horizon, y se resolvió por el otro lado: **fijándola**, porque aquí la pieza sí estaba desplegada.

**2 · Componer la clave exige leer `rules_version`, y leerla ya es una consulta** (`OPEN-BO-24`). `rules_version` vive en `feature_flags`; la caché de evaluación no evita la consulta, evita **la segunda** y el cálculo. Las tres salidas —dejarlo así, cachear el catálogo entero en una entrada y evaluar en memoria, o no cachear— están en `funcional.md §5.11.9.2` con su coste. **Lo que no cambia en ninguna de las tres**, y es lo que hace segura a esta caché y no sólo rápida: **una entrada ausente nunca significa «expuesto»** (§3, `RN-BO-35`).

> **Y un punto donde las salidas de `OPEN-BO-24` no son intercambiables, que es el que conviene no perder de vista al decidir**: la designación de *early adopter* **no incrementa ningún `rules_version`** (`RN-BO-109`), porque no cambia ninguna regla — cambia el sujeto. Con caché por sujeto hay que invalidar la del centro afectado, que es un prefijo y cabe en la petición (el mecanismo de §4.2); con el catálogo en una sola entrada no hay nada que invalidar. **Es la única escritura de `1.6e` cuya visibilidad depende de qué se decida** (`api.md §2.11.1`).
...
| 2 | `php artisan platform:sync-registry` | Gana **tres** validaciones de `depends_on` que **abortan el despliegue** —código inexistente, ciclo, y un módulo esencial que dependa de uno no esencial (`CA-BO-034`, `CA-BO-035`, `CA-BO-129`, `RN-BO-64`)— y, desde `1.6e`, la del catálogo de *feature flags*: clave duplicada entre dos módulos, clave con formato inválido o `module_code` inexistente **también abortan** (`CA-BO-089`). Si aborta, **no se sigue**: se arregla el descriptor. **Es un solo comando y un solo punto de aborto**, a propósito: un segundo comando de sincronización sería un segundo sitio del que olvidarse |
...
| **`1.6b`** · Un tenant se queda atascado en `en_alta` porque su aprovisionamiento falló | `php artisan bo:retry-provisioning <slug>`, que reencola un trabajo **idempotente** (§6.1). Auditado como `console`. **No es un *endpoint***: no hace falta capacidad nueva ni pantalla, y el camino de recuperación de este módulo ya es la consola. **Y nunca se arregla escribiendo `status = 'activo'` a mano**: dejaría un centro marcado como listo sin roles, sin configuración y sin administrador (`RN-BO-52`, `funcional.md §5.3.5`). **`1.6d` lo arregla**: hoy ese comando consulta `failed_jobs` por la conexión equivocada y **falla por privilegios** en cualquier entorno con los tres roles de `ADR-033 §5` aprovisionados (`funcional.md §5.9.1`); pasa a usar el servicio de reintento del módulo, por `pgsql_platform`, y gana su primer test (`CA-BO-164`) |
| **`1.6d`** · Hay que reintentar un trabajo fallido y no hay pantalla ni sesión de plataforma a mano | `POST /api/platform/v1/tenants/{public_id}/failed-jobs/{uuid}/retry` con `curl`, desde una IP de la lista blanca, con motivo. **No hay comando de consola equivalente y no se crea uno**: a diferencia de `bo:retry-provisioning`, que repara un estado del que no se sale de otra forma, aquí sí existe el camino por API, y un segundo camino de escritura sería un segundo sitio donde olvidarse de la auditoría |
...
| `RunModuleRollout` | Activación masiva | Idempotente por `Idempotency-Key`. Emite **un evento por centro** (`RN-BO-27`) e invalida **el prefijo de cada centro** (§4.2). **Una transacción por centro, no una sobre N**, en orden ascendente de `tenants.id` para que dos lotes concurrentes no se interbloqueen; **el fallo de un centro no aborta el lote** (`RN-BO-78`). No implementa la contratación: llama al contrato público `ModuleContracting` de `REQ-CORE` (`funcional.md §5.8.2`), en sus dos fases. Al terminar escribe **una** entrada `modulo.masivo_ejecutado` con `affected_tenant_id` nulo y los recuentos en `context`. **Con `enabled: false` no se encola hasta que la doble autorización se aprueba** (`RN-BO-79`, `RN-BO-80`) |
...
| **`bo:purge-failed-jobs`** (`1.6d`) | Diaria | Borra de `failed_jobs` lo que pase de **24 horas**, por `pgsql_platform` (`RN-BO-98`, `funcional.md §5.9.7`). **Sustituye a `Schedule::command('queue:prune-failed', ['--hours' => 24])`**, que apunta al rol sin `DELETE` y lleva desde `0.7` sin borrar nada. **Es la única tarea del módulo que ejecuta su trabajo ella misma en vez de encolarlo**, y el motivo está justo debajo |

**`1.6d` no añade ni un trabajo en cola, y añade una sola tarea programada —`bo:purge-failed-jobs`— que no es suya de origen**: es el arreglo del hallazgo Alta de `funcional.md §5.9.6`, que entra en su alcance por decisión del usuario del 2026-09-16 y que **sustituye** a una tarea que ya estaba programada y no funcionaba. Todo lo demás del sub-paso es lectura salvo el reintento, y ese reintento **se ejecuta dentro de la petición** (`RN-BO-89`): es un `INSERT` en `jobs` y un `DELETE` en `failed_jobs`, y `INV-012` habla de trabajo pesado — encolar un reintento sería encolar el encolado. **Si en la implementación aparece un *job* de «consolidar métricas» o de «recalcular salud», el diseño se ha desviado de `funcional.md §5.10.3`** y hay que volver a él, no añadir el *job*: `RN-BO-95` decide explícitamente que ninguna métrica se materializa.

> **La excepción de `bo:purge-failed-jobs`, dicha aquí para que nadie la «arregle» por simetría** (`funcional.md §5.9.7`): las otras dos purgas del módulo despachan un trabajo a `backoffice-maintenance`; ésta **ejecuta el `DELETE` en el propio comando**, por dos motivos. Uno, **una purga encolada no purga si no hay quien la procese**, y hoy no hay ningún *worker* desplegado (issue [#128](https://github.com/pirexia/plataforma-educativa/issues/128)) — poner la mitigación del issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73) detrás de una cola que nadie vacía es repetir el mismo fallo con otra forma. Dos, **un trabajo que limpia `failed_jobs` puede acabar dentro de `failed_jobs`**. Y un `DELETE` acotado por fecha no es trabajo pesado, así que `INV-012` no obliga a encolarlo. El planificador **sí** corre en su propio contenedor (`ADR-037`).
>
> **Y el cambio en `routes/console.php` no es aditivo: la línea de `queue:prune-failed` se retira.** Dejar las dos programadas significaría que una de ellas falla a diario en silencio, que es justamente el estado del que se sale. `CA-BO-166` comprueba las dos mitades.
...
| Aristas de dependencia incoherentes reportadas por `platform:sync-registry` | Centros con `M` contratado y `N` no, tras una versión que añadió la arista (`ADR-045 §4.5`) | Se muestran en la ficha de salud del centro (`funcional.md §5.9`) y en `GET /tenants/{id}/modules`, **y no se corrigen solas** |
| **Fase 2 de una escritura de módulos fallida** (`1.6c`) | La escritura quedó hecha y auditada, y el centro no ve el cambio hasta que expire el TTL de 300 s (`RN-BO-76`). **Es el único camino de este módulo en el que un fallo no llega al cliente**, y por eso tiene que llegar aquí | **Alarma ante cualquier aparición**: si se repite, lo que está caído es Redis y el síntoma visible será otro |
| **Centros omitidos o fallidos en un lote masivo** (`1.6c`) | Del `context` de `modulo.masivo_ejecutado`. Un lote que omite muchos centros suele significar que el operador compuso la lista de `GET /tenants` sin filtrar por estado (`RN-BO-71`) | Informativo por lote; **revisado** si la proporción de omitidos supera lo trivial |
| Crecimiento de `admin_action_logs` | Alimenta el disparador de revisión de particionado de `datos.md §4.4` | 10 M de filas |
| **Crecimiento de `failed_jobs`** (`1.6d`) | Con la purga de 24 horas funcionando, esta tabla **no debería crecer de forma sostenida**. Que lo haga significa una de dos cosas, y las dos importan: algo falla en masa, o la purga no corre (`funcional.md §5.9.6`) | **Alarma** por encima de unos cientos de filas sostenidas. Es además el disparador del índice de expresión de `datos.md §14.2` |
| **`jobs` creciendo con `failed_jobs` quieto** (`1.6d`) | Nadie está procesando la cola: o no hay *worker* (issue [#128](https://github.com/pirexia/plataforma-educativa/issues/128)) o el que hay está parado. **Es el síntoma que la ficha de salud existe para hacer visible**, y es indistinguible de «todo va bien» si sólo se mira `failed_jobs` | **Alarma** sobre la antigüedad del trabajo pendiente más viejo, no sobre el recuento |
| **Uso de `job.reintentar`** (`1.6d`) | Cada reintento es una reejecución con efectos secundarios reales, decidida por una persona. **Reintentar tres veces el mismo trabajo significa que nadie ha leído por qué falla** | Informativo por evento; **revisado** si se repite sobre trabajos de la misma clase. Los recuentos salen de `admin_action_logs`, que es donde vive el actor y el motivo |
| **Latencia de `GET /metrics/*`** (`1.6d`) | `RN-BO-95` decide **no cachear**, y esa decisión tiene un disparador de revisión escrito: si estas dos consultas empiezan a notarse, el orden correcto es primero medir, después indexar, y sólo al final cachear | Cualquier aparición sostenida en el percentil 95 abre la revisión de `RN-BO-95`, **no** la introduce una caché directamente |
...
| «Un tenant lleva horas en `en_alta`» | **La última entrada `tenant.aprovisionamiento_fallido` de la ficha de salud**, no la lista de trabajos del centro | `funcional.md §5.3.5`. Se repara con `bo:retry-provisioning`, **nunca** escribiendo `status` a mano (§5.1). **Corregido por `1.6d`**: este renglón decía «`failed_jobs` **del tenant**» y era inexacto — ese trabajo lo despacha el backoffice sin tenant activo, así que su `payload.tenant_id` es nulo y no sale bajo el filtro del centro (`RN-BO-90`, `OPEN-BO-20`). La entrada de auditoría sí lleva `affected_tenant_id`, y es la que hay que mirar |
...
| «Un centro tiene `M` sin `N` y el sistema no lo arregla» | Ficha de salud del centro, y `missing_dependencies` de `GET /tenants/{id}/modules` | `ADR-045 §4.5`: el comando **informa y no corrige**. Contratar automáticamente sería tomar una decisión comercial facturable sobre 200 centros |
| **`1.6c`** · «`platform:sync-registry` aborta y el mensaje habla de un módulo esencial» | El `depends_on` del descriptor de `core` o de `auth` | `RN-BO-64`, tercera validación: **un esencial no puede depender de uno no esencial**. Es utilizable sin fila, luego ninguna escritura puede protegerlo. Se arregla en el descriptor, no relajando el comando |
| **`1.6c`** · «Contrato un módulo y la respuesta trae `cache_invalidated: false`» | Redis | La escritura **está hecha y auditada**; lo que falló es la fase 2 (§4.2, detalle 2). El centro lo verá como tarde al expirar el TTL de 300 s. **No se reintenta la contratación**: produciría una no-operación (`RN-BO-69`) y no arreglaría la caché |
| **`1.6c`** · «Un lote masivo ha terminado y faltan centros» | La entrada `modulo.masivo_ejecutado`, campo `context` | `RN-BO-78`: el fallo de un centro **no aborta el lote**. Ahí están los recuentos y el motivo de cada omisión. Los centros omitidos o fallidos **no tienen entrada propia**, y es correcto: no les pasó nada |
| **`1.6c`** · «No puedo descontratar un módulo en masa» | Cuántos administradores con `modulo.contratar_masivo` hay | `RN-BO-79`: la descontratación masiva es de las tres operaciones que exigen doble autorización (`REQ-BO-007`), y la aprueba **otro administrador con la misma capacidad** (`permisos.md §5.2`), no necesariamente un `superadministrador` |
| **`1.6c`** · «La aprobación de una descontratación masiva dice `ejecutada` y los centros siguen igual» | Que el *worker* esté corriendo | `RN-BO-80`: aprobar **encola**; `executed_at` marca ese instante y no el final del lote. Es la única de las tres acciones del vocabulario cuya ejecución es asíncrona |
| **`1.6c`** · «Al contratar desde el backoffice falla una clave foránea sobre `users`» | Si el código escribe `created_by`/`updated_by` | `RN-BO-73`: **no se escriben**. La clave foránea compuesta a `users` está haciendo su trabajo — un `platform_admins.id` no es un usuario de ese centro. El actor va en `admin_action_logs` |
| **`1.6c`** · «Al contratar desde el backoffice falla un `NOT NULL` en `tenant_id`» | Si el código confía en el `DEFAULT` de la columna | `datos.md §7.6`: `app.current_tenant_id()` es nulo sin tenant activo, y el backoffice escribe sin él (`ADR-046 §6.4`). Se fija a mano |
| **`1.6d`** · «La ficha de salud falla con error de privilegios al leer `failed_jobs`» | Por qué conexión corre la consulta | `RN-BO-86`: `plataforma_app` tiene `REVOKE SELECT, UPDATE, DELETE` sobre esa tabla desde `0.7` y conserva sólo `INSERT`. **Es el comportamiento correcto y es ruidoso a propósito**; se arregla en la consulta, por `pgsql_platform`, **nunca ampliando el `GRANT`** ni moviendo `queue.failed.database` (§2, `datos.md §14.3`) |
| **`1.6d`** · «`bo:retry-provisioning` falla por privilegios» | Ídem | Mismo defecto y misma causa: el comando usaba la conexión por defecto y `queue:retry`. Lo arregla `1.6d` (§5.1). **Si vuelve a aparecer tras el arreglo, alguien ha reintroducido el proveedor del framework** |
| **`1.6d`** · «La purga corre a diario y `failed_jobs` no para de crecer» | **Qué comando está programado**: si sigue siendo `queue:prune-failed`, el arreglo de `1.6d` no está desplegado | `funcional.md §5.9.6`, hallazgo **Alta**: ese comando usa el proveedor del framework sobre `plataforma_app`, que tiene `REVOKE DELETE`, y **llevaba sin borrar nada desde `0.7`** — con él, la segunda capa del issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73) estaba documentada y no aplicada. Lo sustituye `bo:purge-failed-jobs` (§6.2, `RN-BO-98`). **Si las dos están programadas, una falla a diario en silencio**: se retira la del framework |
| **`1.6d`** · «Un tenant lleva más de un día en `en_alta` y `bo:retry-provisioning` dice que no hay nada que reintentar» | La antigüedad del fallo frente a las 24 horas de retención | **Es la retención de #73 funcionando**, no un defecto (`RN-BO-98`). Pasado ese plazo, el camino es dar de alta el centro otra vez o completar su aprovisionamiento a mano; **nunca escribir `status`** (`RN-BO-52`). **Antes de `1.6d` esto no pasaba porque la purga no borraba** — es la única regresión visible que introduce el arreglo, y es deliberada |
| **`1.6d`** · «Un aprovisionamiento ha fallado y no lo veo en los trabajos del centro» | El bloque `platform_incident` de la ficha, no el bloque `jobs` | `RN-BO-90`. **No es un defecto de la ficha**: el trabajo se despachó sin tenant y su `payload.tenant_id` es nulo. La salida de fondo está en `OPEN-BO-20` |
| **`1.6d`** · «Un lote masivo falló sobre un centro y no aparece en su ficha» | La entrada `modulo.masivo_ejecutado`, campo `context` | `RN-BO-78`: los centros fallidos de un lote **no tienen entrada propia** porque «no les pasó nada». Es una decisión de `1.6c` y `1.6d` **no la cambia** para taparlo |
| **`1.6d`** · «La ficha dice `0` trabajos fallidos y sé que hubo uno anteayer» | La retención de `failed_jobs` | 24 horas, fijadas por el issue #73 (§6.2). **No es de este módulo cambiarlas**, y `failed_recent_hours` viaja en la respuesta para que el número sea interpretable (`api.md §2.10.1`) |
| **`1.6d`** · «La adopción por módulo dice que `core` lo tienen 0 centros» | **Es un defecto** | `RN-BO-93`: un `essential` se devuelve **marcado y sin recuento**, nunca con `0`. Si aparece un cero, el agregado está contando filas de `module_subscriptions` en vez de partir del catálogo declarado |
| **`1.6d`** · «Las métricas devuelven números pequeños y crecientes que no cuadran con el inventario» | Si la consulta corre dentro de `runAsPlatform(BackofficeLectura, …)` | `RN-BO-94`. Fuera de ese bloque `TenantScope` filtra por el tenant activo y **el agregado sale reducido sin dar ningún error**. Es el modo de fallo silencioso de este sub-paso y el que `CA-BO-162` existe para atrapar |
| **`1.6d`** · «`tenants_by_status` dice 0 eliminados y sé que hay varios» | Si la consulta excluye los borrados lógicos | `RN-BO-91`: la eliminación escribe `status` y `deleted_at` a la vez, así que un `SoftDeletes` activo deja esa fila **siempre a cero**. Mismo defecto por la vía equivocada que `RN-BO-50` corrigió en `ResolveTenant` |
| **`1.6d`** · «La ficha devuelve versión `0.1.0` en producción» | `APP_VERSION` en el `EnvironmentFile` de la unidad | §2. No es un fallo de la ficha: es el valor por defecto de `config/app.php` porque el despliegue no fija la variable desde la etiqueta de imagen (`ADR-037`) |
...
| **`1.6e`** · «Al apagar un *flag* la petición falla con un error de restricción de la base de datos» | El `CHECK` de `admin_action_logs.action` | `datos.md §4.2.2`: **falta la migración 6b**, la que añade `flag.estado_cambiado` y los otros tres valores. La auditoría va en la misma transacción que la escritura, así que el `CHECK` hace fallar **la operación entera** — y el síntoma es el peor posible: no se puede apagar el *flag* que está rompiendo colegios (`RN-BO-105`, `CA-BO-174`) |
| **`1.6e`** · «He apagado un *flag* y en algunos procesos sigue encendido» | Si `rules_version` sube **de una en una** con cada escritura | `datos.md §9.7`: el incremento tiene que ser `SET rules_version = rules_version + 1` **en SQL**, no leer y escribir desde PHP. Con lectura-y-escritura, dos escrituras concurrentes producen **un solo incremento** y las entradas de la versión anterior **siguen siendo alcanzables** hasta que caduque el TTL |
| **`1.6e`** · «El *flag* está expuesto y este usuario concreto no lo ve» | El campo **`matched_by`**: si dice `role_filtered`, es una regla de rol | `RN-BO-40`. El centro **sí** está expuesto; el filtro por rol es el paso 4 de `funcional.md §5.11.5` y no amplía, sólo restringe. Si dice `module_disabled`, es otra cosa: el centro no tiene contratado el módulo dueño (`RN-BO-102`) |
| **`1.6e`** · «Un *flag* que ya no existe en el código sigue evaluando verdadero» | Si el despliegue ejecutó `platform:sync-registry` y si el retiro incrementó `rules_version` | `RN-BO-103`. `retired_at` sin incremento de versión deja las entradas anteriores alcanzables: el *flag* retirado sobrevive hasta el TTL. **No se arregla borrando caché a mano**, se arregla en el comando (`CA-BO-172`) |
| **`1.6e`** · «He designado un centro como *early adopter* y tarda en verlo» | **No es un defecto, es `RN-BO-109`** | Esa escritura **no incrementa ningún `rules_version`** porque no cambia ninguna regla. Lo que hace falta es invalidar la caché **de ese centro** (§4.2) o no tener caché por sujeto en absoluto; cuál de las dos, lo decide `OPEN-BO-24` (§4.3.1) |
| **`1.6e`** · «La caché de *flags* no acelera nada, o va más lenta» | **`CACHE_STORE`**, y que valga `redis` en el entorno de ejecución | §4.3.1: **la decisión es `CACHE_STORE=redis`** (`OPEN-BO-26`, issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236)). Si la variable no llega al contenedor, `config/cache.php` **cae en silencio a `database`** y una lectura de caché pasa a ser una consulta SQL a la tabla `cache` — sustituir por ella una consulta a una tabla de decenas de filas con índice no ahorra nada. **Y los tests no lo detectan**, porque `phpunit.xml` fuerza `redis`. **Síntoma característico**: la caché «funciona» y el percentil 95 no mejora |
| **`1.6e`** · «La aplicación arranca pero ninguna caché usa Redis, y nadie ha tocado nada» | Si el cliente de Redis sigue en la imagen | §4.3.1, deuda residual: `composer.json` **no declara** `predis/predis` ni `ext-redis`, así que un cambio de imagen base puede retirarlo **sin que falle el despliegue**. Se comprueba tras cualquier cambio de imagen, no cuando aparece el síntoma |
| **`1.6e`** · «La ruta de un *flag* devuelve `404` y la clave existe» | Si la clave lleva un punto y la ruta declara la restricción de parámetro | `api.md §2.11`: las rutas van por `key` (`OPEN-BO-11`, resuelta), y el formato admite puntos —`comedor.reserva_v2`—. **Sin la restricción correspondiente, Laravel no encaja la clave como un solo segmento.** Es el único detalle de implementación que arrastra esa decisión |
...
| **`CLAUDE.md §1`** (`1.6d`) | **La tabla de *stack* nombra «Redis + Laravel Horizon» y Horizon no está instalado** (§3.1). Ya indujo a error a esta misma especificación, que afirmaba que los trabajos en cola salen de Horizon. O se instala (`OPEN-BO-23`) o la tabla dice que es intención y no estado. **Es una edición de `CLAUDE.md`, que no corresponde a esta especificación**: la decide el usuario |
| `SYSADMIN.md` (`1.6d`) | **`APP_VERSION` tiene que fijarse en el despliegue** desde la etiqueta de imagen de `ADR-037`, o la ficha de salud devuelve `0.1.0` para siempre (§2). Y la pila de colas real de §3.1: qué tablas hay, por qué conexión se leen, **por qué `queue:retry`/`queue:failed`/`queue:prune-failed` no sirven** en este proyecto, y **qué comando los sustituye** (`bo:purge-failed-jobs`, §6.2) |
| `SECURITY.md` (`1.6d`) | **El backoffice lee trabajos fallidos de todos los centros y no devuelve su `payload`** (`RN-BO-84`): un *payload* serializado contiene datos personales del centro y, en los correos de `REQ-AUTH`, un token de un solo uso. La barrera es la proyección del *resource* —aquí **no** hay `GRANT` de columna que la respalde, a diferencia de `admin_action_logs`—, y eso hay que decirlo como lo que es: **una garantía más débil que la de `ADR-047 §4.4`**, sostenida por `CA-BO-150` y no por el motor. Y la excepción consciente de `OPEN-BO-22`, si se aprueba devolver el mensaje de la excepción |
| `RUNBOOK.md` (`1.6d`) | Las once filas nuevas de §8 y la de §5.1: cómo se reintenta un trabajo con `curl`, qué significa cada error de privilegios, y por qué `jobs` creciendo con `failed_jobs` quieto es el síntoma de que nadie procesa la cola |
| `PRIVACY.md` (`1.6d`) | **La retención de 24 horas de `failed_jobs` estuvo documentada y sin aplicar desde `0.7` hasta `1.6d`** (`funcional.md §5.9.6`, hallazgo Alta): durante ese periodo, los *payloads* cifrados de los correos de `REQ-AUTH` —con su token de un solo uso— se conservaron **indefinidamente**. `bo:purge-failed-jobs` lo cierra (§6.2). **Tiene que constar como incidencia con sus fechas**, no desaparecer porque se haya arreglado: lo que se arregla hacia delante no borra lo que se conservó de más |
| **`docs/modulos/REQ-AUTH/operacion.md`** (`1.6d`) | **Obligatorio al cerrar, no opcional.** Su tabla de trabajos programados lista `queue:prune-failed --hours=24` como «comando directo» y **otros tres párrafos** lo citan como *«la segunda capa»* del issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73). Los cuatro pasan a nombrar **`bo:purge-failed-jobs`** y a decir que la purga **es de `REQ-BO`** — coherente con `INV-007` y con el *docblock* de la migración de `0.7`: *«gestionar la cola de fallos es un asunto de plataforma (`REQ-BO-004`)»*. Una tarea retirada que sigue documentada en el módulo que la pidió es exactamente la desincronización de la regla 7 de `CLAUDE.md §6` |
| **`docs/modulos/REQ-CORE/operacion.md`** (`1.6d`) | Menciona la misma tarea programada. Misma corrección |
| `RUNBOOK.md` (`1.6d`) | Ídem, y además el diagnóstico nuevo de §8: qué mirar cuando `failed_jobs` crece pese a la purga, y por qué un fallo de hace más de un día ya no es reintentable |
| `CHANGELOG.md`, `docs/historial/*` | **No se tocan.** Nombran `queue:prune-failed` al relatar `1.2` y el cierre de #73, y decían la verdad cuando se escribieron |
...
| `docs/modulos/REQ-CORE/funcional.md` | **Resuelto al cerrar `1.6b`** (`ADR-048 §7`, `§11` punto 3): `§7` ya enumera la sexta interfaz pública, **`TenantProvisioner`**, y `§4.7` ya documenta las cinco opciones nuevas de `tenant:provision-defaults` |
...
| `CONTRIBUTING.md` (`1.6c`) | **Cómo se declara un módulo nuevo**: `depends_on` y `essential` en `moduleDescriptor()`, y que `platform:sync-registry` **aborta el despliegue** ante un código inexistente, un ciclo, o un esencial que dependa de uno no esencial. Es lo que un desarrollador necesita antes de escribir su primer `ServiceProvider` de módulo, y hoy no está en ningún sitio |
| `SECURITY.md` (`1.6c`) | **Que un centro no pueda contratarse un módulo a sí mismo deja de ser un `if` y pasa a ser un `REVOKE`** (`ADR-045 §4.4`, `INV-001`). Y, si `OPEN-BO-19` se aprueba, que **el motivo comercial escrito por el proveedor no es legible desde el *runtime* de un centro**, garantizado por privilegio de columna y no por la proyección de un *resource* — mismo criterio y misma anotación de riesgo residual que `ADR-047 §8` hizo para las otras dos tablas |
| `docs/modulos/REQ-CORE/funcional.md` (`1.6c`) | Pendiente al cerrar: su `§7` enumera las interfaces públicas de `REQ-CORE` y tiene que ganar **`ModuleCatalog` y `ModuleContracting`** (`funcional.md §5.8.2`), igual que ganó `TenantProvisioner` al cerrar `1.6b`. Sin esa línea, `REQ-CORE` acaba siendo dueño de un contrato que su propia especificación no documenta — que es exactamente lo que ya se anotó para el evaluador de *flags* |
...
| **`docs/modulos/REQ-CORE/funcional.md §7`** (`1.6e`) | **Obligatorio al cerrar, y es más que una línea.** Ese apartado enumera las interfaces públicas de `REQ-CORE` y ganó `TenantProvisioner` al cerrar `1.6b` y `ModuleCatalog`/`ModuleContracting` al cerrar `1.6c`. Tiene que ganar **`FeatureFlagEvaluator` y `FeatureFlagExplainer`**, con la precisión de que **no viven en `Core\Domain` sino en `App\Support\FeatureFlags`**, igual que `ModuleAvailability`, y con el criterio general que `funcional.md §5.11.8` deduce: un contrato que consume **un** módulo va en el `Domain` de su dueño; uno que consume **todo el producto** va en `App\Support`. Sin esa línea, `REQ-CORE` vuelve a ser dueño de un componente que su propia especificación no documenta — que es exactamente lo que ya se anotó dos veces |
| **`docs/modulos/REQ-CORE/datos.md`** (`1.6e`) | **Nada.** Las dos tablas de *flags* son de plataforma y viven en `datos.md §9` de este módulo. Se dice para que nadie las duplique allí al ver que el evaluador es de `REQ-CORE`: **el dueño del contrato no es el dueño de la tabla**, y en este módulo esa separación ya la hace `ADR-045 §4.1` con `module_subscriptions` |
| **`CLAUDE.md §1`** (`1.6e`) | **Nada, y es el resultado de haberlo comprobado.** La tabla de *stack* dice «Caché: **Redis**, instalado y en uso» y ningún fichero del repositorio fijaba `CACHE_STORE` (§4.3.1). **`OPEN-BO-26` se resolvió fijando la variable** (issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236)), de modo que **esa fila pasa a decir la verdad y no hay que corregirla**. Es lo contrario de lo que ocurrió con `OPEN-BO-23`, donde la fila de colas sí hubo que corregir porque Horizon no estaba. Se deja escrito **para que conste que se verificó**: una fila que resulta ser cierta y una fila que nadie ha mirado se leen igual |
| **`SYSADMIN.md`** (`1.6e`) | **`CACHE_STORE=redis` tiene que constar entre las variables que el despliegue fija**, junto a `APP_VERSION` (§2), con dos comprobaciones operativas y no una: que el contenedor de Redis está arriba antes de dar el servicio por desplegado, y —mientras el cliente siga sin declararse en `composer.json`— **que la aplicación arranca de verdad sobre Redis y no ha caído en silencio a `database`**, que es el modo de fallo que `config/cache.php` permite por diseño (§4.3.1) |
| **`RUNBOOK.md`** (`1.6e`) | Las **nueve** filas nuevas de §8: el error de restricción al apagar un *flag*, el incremento de `rules_version` que se pierde por concurrencia, cómo se lee `matched_by` al depurar, el *flag* retirado que sobrevive al despliegue, la designación de *early adopter* que tarda en verse, la caché que no acelera, el cliente de Redis que desaparece con un cambio de imagen, y el `404` de una ruta de *flag* cuya clave lleva un punto |
| **`PRIVACY.md`** (`1.6e`) | **Nada nuevo, y conviene dejarlo dicho en vez de omitirlo**: el motor no trata datos personales (`funcional.md §11`). El reparto por porcentaje usa el `public_id` del centro o del usuario **como entrada de un hash**, no almacena ninguna asignación y no conserva el resultado; el filtro por rol lee el rol dentro de la propia petición. **Es la única pieza de los cinco sub-pasos que toca al usuario final de un centro y no deja rastro suyo en ninguna tabla**, y eso merece constar afirmado, no deducirse de que no haya fila |
...
**Sobre la reversión de `1.6d` en concreto: no hay nada que revertir en base de datos.** Es el único sub-paso sin una sola migración (`datos.md §14`), así que revertir su código retira cinco rutas de lectura y una de escritura, y nada más. **Tres consecuencias que sí conviene tener presentes**:

- Las entradas `job.reintentado` que ya se hayan escrito **se quedan** en `admin_action_logs`, que es *append-only* y donde son prueba de qué se reejecutó y por orden de quién.
- **`bo:retry-provisioning` vuelve a estar roto**, porque su arreglo va con este código: si se revierte con un centro atascado en `en_alta`, el único camino que queda es volver a desplegar.
- **Y la purga de `failed_jobs` vuelve a no purgar.** Revertir restituye `Schedule::command('queue:prune-failed', …)` en `routes/console.php`, que apunta al rol sin `DELETE` (§6.2): con ello **se reabre el hallazgo Alta** y los *payloads* con token de `REQ-AUTH` vuelven a conservarse indefinidamente. **Es lo único de esta reversión con consecuencia de datos personales**, y por eso una reversión de `1.6d` que vaya a durar más de unos días exige purgar `failed_jobs` a mano por `pgsql_platform` mientras dure.
