...
#### 5.11.8 El evaluador: dónde vive, qué firma tiene y por qué son dos interfaces y no una

> **Esto es lo que la pasada de `1.6e` añade y lo que antes no existía.** §9 decidía —correctamente— que el evaluador pertenece a `REQ-CORE` y no a `REQ-BO`, y lo dejaba en una frase y un nombre de clase. Un nombre de clase no es un contrato: no dice qué firma tiene, de dónde saca el sujeto, qué hace sin contexto de tenant, ni cómo lo consume el backoffice, que necesita evaluar **para otro centro** desde un sitio donde no hay tenant activo. Sin eso, `implementer` lo inventa.

**Dónde vive, con el precedente verificado el 2026-09-21 y no supuesto.** `ModuleAvailability` —la única pieza de este proyecto con exactamente el mismo perfil: un booleano que cualquier módulo consulta en el camino de petición— **no vive en `Core\Domain`**: vive en **`App\Support\Modules\ModuleAvailability`**, la implementa `App\Modules\Core\Infrastructure\EloquentModuleAvailability` y la enlaza `CoreServiceProvider`. Su propio *docblock* dice por qué: *«Esta interfaz la posee `REQ-CORE` […]; `App\Support\Authorization` la consume sin importar nada de `App\Modules\Core` (`INV-007`)»*. El evaluador de *flags* copia esa forma **exactamente**:

| Pieza | Dónde | Precedente literal |
|---|---|---|
| `FeatureFlagEvaluator` (interfaz) | `App\Support\FeatureFlags` | `App\Support\Modules\ModuleAvailability` |
| `FeatureFlagExplainer` (interfaz) | Ídem | — (no hay precedente; §5.11.8.2) |
| `EloquentFeatureFlagEvaluator` (implementación única de las dos) | `App\Modules\Core\Infrastructure` | `EloquentModuleAvailability` |
| Enlace | `CoreServiceProvider::register()` | La línea que ya enlaza `ModuleAvailability` |

> **Por qué `App\Support` y no `Core\Domain`, que es donde `1.6b` y `1.6c` pusieron sus contratos.** No es una incoherencia y conviene dejarla escrita, porque parece una. `TenantProvisioner` (`ADR-048`), `ModuleCatalog` y `ModuleContracting` (`1.6c`) viven en `Core\Domain` porque su consumidor es **un módulo con nombre**, `Backoffice`, y uno solo: que ese módulo escriba `use App\Modules\Core\Domain\TenantProvisioner` es una dependencia declarada y visible entre dos módulos. **El evaluador no tiene un consumidor: tiene cincuenta y tres.** Una interfaz en `Core\Domain` obligaría a todos los módulos del producto a importar del espacio de nombres interno de otro — que es literalmente el `use` que `INV-007` existe para impedir, y el motivo por el que `ModuleAvailability` está donde está. **La regla que se deduce, y que vale para el resto del proyecto**: un contrato que consume **un** módulo va en el `Domain` de su dueño; un contrato que consume **todo el producto** va en `App\Support`, y su dueño lo implementa.

##### 5.11.8.1 La firma del camino de petición, y lo que deliberadamente no acepta
...
**Una sola operación, un solo argumento y ningún sujeto.** El tenant y el usuario se toman del contexto —`TenantContext` y el *guard* `web`—, exactamente como `ModuleAvailability::isEnabled()` toma el tenant de `TenantContext::hasTenant()`. Es la decisión que más consecuencias tiene de toda esta sección, y las tres son de seguridad:

1. **No se puede evaluar para otro centro desde el camino de petición, porque no hay parámetro con el que pedirlo.** Un `isEnabled(string $key, ?int $tenantId = null)` sería una lectura entre centros disponible en cada módulo del producto, defendida sólo por que nadie pase el segundo argumento. `INV-001` no se sostiene sobre que nadie pase un argumento.
2. **Falla en cerrado sin contexto de tenant**: `false`, nunca el valor de otro tenant y nunca una excepción. Es la misma propiedad que el *docblock* de `ModuleAvailability` declara con esas palabras, y la que hace que `RN-BO-41` —sin sujeto usuario, un *flag* con reglas de rol es falso— sea un caso particular y no una regla aparte.
3. **Devuelve `bool` y no un objeto con el motivo.** El motivo es lo que pide el backoffice, no el producto; devolverlo aquí pondría en manos de cualquier módulo —y, por la vía de un *resource* descuidado, de cualquier centro— el mapa de a qué ritmo se despliega qué, que es justo lo que `api.md §2.14` punto 2 se niega a entregar.

##### 5.11.8.2 La segunda interfaz, que es la que el backoffice necesita y no puede ser la misma

`api.md §2.13` promete dos lecturas que **no se pueden servir con la firma de arriba**: `GET /tenants/{public_id}/feature-flags`, que evalúa para **otro** centro y devuelve **`matched_by`**, y `POST /feature-flags/{key}/rules/preview`, que evalúa un conjunto de reglas **que todavía no está escrito**, sobre todo el parque. Las dos corren desde el backoffice, donde por `ADR-046 §6.4` **no hay tenant activo**.
...
- **`FeatureFlagSubject`** es un objeto de valor con el `tenant_id` y el `public_id` del centro, y opcionalmente el `public_id` del usuario y sus códigos de rol. Es lo que hace posible evaluar **sin entrar en el tenant**, que es la condición que `ADR-046 §6.4` impone y la razón de que el sujeto sea explícito aquí y no lo sea en `isEnabled()`.
- **`FeatureFlagDecision`** lleva `enabled` y `matched_by` ∈ {`tenant`, `early_adopters`, `percentage`, `global`, `forced_off`, `retired`, `module_disabled`, `role_filtered`, `none`} — el vocabulario que `api.md §2.13` devuelve. **Es enumerado extensible** (`ADR-038 §7.3`).
- **`explainWith()` es lo que hace honesta la vista previa**: recibe el conjunto **propuesto**, no el escrito, y por eso la vista previa puede responder «a cuántos afectaría» sin tocar la base de datos ni bloquear nada (`RN-BO-68`).

> **Dos interfaces y no dos métodos de una, y el motivo es el enlace.** `FeatureFlagExplainer` acepta un sujeto arbitrario: es, por construcción, la puerta por la que se evalúa para un centro que no es el de la petición. Separarla permite que **su enlace por defecto deniegue**, con el patrón exacto que `ADR-046 §6.3` ya aplica a `PlatformAccessCheck` —*«el enlace por defecto deniega los propósitos de backoffice»*—, y que un test de arquitectura compruebe que **ningún fichero fuera de `app/Modules/Backoffice` la inyecta** (`CA-BO-169`). Con dos métodos de la misma interfaz eso no se puede comprobar: cualquier consumidor legítimo del booleano tendría también el explicador inyectado y a un `->explain()` de distancia.

**Y una sola implementación del algoritmo, no dos.** Las dos interfaces las sirve la misma clase, y las dos delegan en la **misma función de decisión pura** —conjunto de reglas más sujeto, sin acceso a base de datos dentro— que implementa el orden de §5.11.5. Es el argumento de `RN-BO-22` aplicado aquí: *«una sola implementación, en un servicio de dominio de `REQ-CORE`»*. Dos implementaciones del orden de precedencia significan que el `matched_by` que un operador lee al depurar puede no ser el motivo por el que el centro está viendo la pantalla, que es la peor forma posible de que falle una herramienta de diagnóstico (`CA-BO-170`).

##### 5.11.8.3 Los códigos de rol: §9 los atribuye a `REQ-PERM` y **es inexacto**

**Verificado el 2026-09-21 sobre `develop`, y es una corrección a una frase de esta misma especificación.** §9 punto 1 dice que los códigos de rol del usuario *«son dato de `REQ-PERM`»* y que, si no hay interfaz pública que los sirva, *«añadirla es parte de `1.6e` y debe declararse como ampliación de la superficie pública de `REQ-PERM`»* — el mismo tipo de ampliación que fue `OPEN-BO-15` en `1.6b` y `OPEN-BO-18` en `1.6c`. Al comprobarlo contra el código, la premisa no se sostiene:

| Lo que §9 suponía | Lo que hay |
|---|---|
| Los roles del usuario son dato de `REQ-PERM` | **`roles` y `role_user` son de `REQ-CORE`** (`REQ-CORE-004`: *«listado de roles predefinidos […], asignación de roles a usuarios»*; el esquema es de `0.8` por `ADR-034 §2`). Lo que `REQ-PERM` añadió en `1.5` es el **resolutor de permisos**, no la propiedad de los roles |
| Haría falta una interfaz pública de `REQ-PERM` | **No hace falta ninguna.** El evaluador vive en `REQ-CORE`, y `App\Models\User::roles()` y `Role::code` son suyos. Leerlos desde `Core\Infrastructure` no cruza ninguna frontera de módulo |
| Lo más parecido que existe | `PermissionResolver::effectivePermissionCodes(User $subject): array`, en `App\Support\Authorization`, que devuelve **códigos de permiso y no de rol**: no sirve, y forzarlo habría sido el acoplamiento que §9 temía |

**Consecuencia: `1.6e` no amplía la superficie pública de ningún módulo ajeno, y no hay `OPEN-BO` equivalente a `OPEN-BO-15`/`OPEN-BO-18` que plantear por este motivo.** Lo que sí hay es el de las **dos interfaces nuevas de `App\Support`**, que es una ampliación de la superficie pública de `REQ-CORE` y va como `OPEN-BO-25` (§14).

**Lo que sigue en pie de §9 punto 1, y no es cosmético**: el evaluador **no hace `$user->roles->pluck('code')` en línea**. Los códigos se obtienen por un método con nombre dentro de `REQ-CORE` —el mismo dato que `App\Support\Api\UserProfilePresenter` ya mapea hoy para `GET /me`—, porque `RN-BO-40` tiene semántica propia (se compara el **código**, un código inexistente no es un error, y funciona sobre roles personalizados) y esa semántica debe vivir en un sitio y tener un test, no repetirse en dos.

**Se señala en vez de corregir §9 en silencio**, y esa es la norma del módulo: `1.6d` hizo exactamente esto con las dos frases falsas sobre `failed_jobs` del tenant (§5.9.3). La frase de §9 se reescribe en esta misma pasada con una nota que dice qué decía antes.

##### 5.11.8.4 El evaluador comprueba el módulo dueño antes de mirar una sola regla

`RN-BO-45` dice que un *flag* nunca enciende una funcionalidad de un módulo no contratado, y `CA-BO-092` lo verifica **desde el *endpoint***, donde quien lo garantiza es el *middleware* `EnsureModuleEnabled`. Eso cubre el camino HTTP y **no cubre los demás**: un *flag* consultado desde un trabajo en cola, desde un comando, desde un *listener* o desde un servicio que no está detrás de ese *middleware* devolvería verdadero para un centro que no tiene el módulo.

**Decisión: el evaluador comprueba `ModuleAvailability::isEnabled($flag->module_code)` como paso 0, antes del catálogo y antes de `forced_off`, cuando `module_code` no es nulo.** No cambia `RN-BO-45` —la refuerza— y no cuesta una consulta nueva: esa llamada ya está cacheada 300 s y, para los *flags* del núcleo, `module_code` es nulo y el paso se salta entero. `matched_by` devuelve `module_disabled`, que es la respuesta que un operador necesita ver en `GET /tenants/{id}/feature-flags` cuando pregunta *«¿por qué este centro no ve lo que está al 100 %?»* — hoy ese caso se diagnostica leyendo dos pantallas (`operacion.md §8`).

#### 5.11.9 El coste en el camino de petición, y las dos cosas que hay que comprobar antes de pagarlo

§12.3 justifica que `1.6e` vaya el último con una frase fuerte: es *«el único que añade código al camino de petición de todos los centros»*. Sigue siendo cierta como propiedad del diseño, y **hay que acotarla con lo que de verdad ocurre en fase 1**, porque de lo contrario la revisión de rendimiento no sabe a qué prestar atención:

- **El evaluador es perezoso y no está en ningún *middleware*.** No se ejecuta nada hasta que un módulo llama a `isEnabled()`. No hay *middleware* de *flags*, no se precarga nada en el arranque y una petición que no consulte ningún *flag* **no hace ni una consulta ni una lectura de caché del motor** (`CA-BO-175`).
- **En fase 1 no hay ni un consumidor real.** §12.3 ya lo decía —*«en fase 1 no hay todavía ninguna funcionalidad que merezca ir detrás de un *flag*»*— y la consecuencia concreta es que, al cerrar `1.6e`, **el único llamador del evaluador en todo el producto es `GET /api/v1/feature-flags`** (`api.md §2.14`), que es una ruta que la SPA pide una vez por sesión y no en cada petición.
- **El coste real llega con el primer consumidor, no con este paso.** Por eso `operacion.md §7` vigila la latencia del evaluador como señal permanente y no como comprobación de cierre: lo que hay que medir todavía no existe.

##### 5.11.9.1 Primera comprobación: el almacén de caché configurado no era Redis · **RESUELTA 2026-09-21**

> **Decisión del usuario del 2026-09-21 (`OPEN-BO-26`): se fija `CACHE_STORE=redis` para el *runtime* real, no sólo para los tests.** Redis ya estaba desplegado y en uso para el prefijo por tenant de `ADR-033 §9` y para el límite de tasa; lo que faltaba era **declararlo en la configuración de ejecución**. La corrección de `compose.yaml` y de las unidades Quadlet va por el issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236), fuera del ámbito de esta especificación.
>
> **Consecuencia directa: la caché de evaluación de *flags* se construye sobre Redis**, que es lo que `datos.md §9.5` y `operacion.md §4.3` asumían antes de que esta pasada encontrara la contradicción. **Las dos secciones quedan vigentes sin una sola corrección**, y lo que la decisión retira del razonamiento de §5.11.9.2 es únicamente el argumento *«sobre `database` la caché no ahorra nada»* — no el hueco de la clave, que sigue siendo `OPEN-BO-24`.
>
> **Deuda residual, anotada en el issue #236 y no convertida en pregunta nueva**: `composer.json` **sigue sin declarar** `predis/predis` ni `ext-redis`. El cliente existe en la imagen —la suite corre sobre Redis y pasa— pero no está declarado, así que nada impide que un cambio de la imagen base lo retire y el fallo aparezca en ejecución en vez de en el despliegue (`CLAUDE.md §1`, `RNF-MANT-007`). No bloquea `1.6e`.

**El hallazgo, que se conserva porque explica por qué hubo que preguntar.** Verificado el 2026-09-21, y era el equivalente en este sub-paso al de Horizon en `1.6d`. Toda la estrategia de caché de `datos.md §9.5` y `operacion.md §4.3` está escrita —razonablemente— sobre el supuesto de que la caché es Redis, porque `CLAUDE.md §1` lo lista como *«instalado y en uso»* y `ADR-033 §9` describe su prefijo por tenant. Lo que dice el repositorio:

| Dónde | Qué dice |
|---|---|
| `apps/api/config/cache.php` | `'default' => env('CACHE_STORE', 'database')` — **el valor de respaldo es `database`** |
| `infra/quadlet/plataforma.env.example`, `infra/compose/compose.prodlike.yaml` | Fijan `REDIS_HOST`, y **no fijan `CACHE_STORE`** |
| `apps/api/phpunit.xml` | `<env name="CACHE_STORE" value="redis" force="true"/>` — **la suite sí corre sobre Redis** |
| `apps/api/composer.json` | **No declara `predis/predis` ni `ext-redis`** |

**Ningún fichero del repositorio fija `CACHE_STORE` para el *runtime*.** No afirmo que el despliegue corra sobre `database` —`CACHE_STORE` puede estar puesta en el `EnvironmentFile` real, que no está en el repositorio—; afirmo lo que sí es comprobable y lo que importa: **el valor de respaldo es `database`, nada en el repositorio lo cambia, y la suite prueba el otro.**

**Por qué le importa a este sub-paso más que a ningún otro, y no es una queja genérica:**

1. **Sobre el almacén `database`, una lectura de caché es una consulta SQL a la tabla `cache`.** Cachear la evaluación de un *flag* dejaría de ser un ahorro: sustituiría una consulta a `feature_flag_rules` —tabla de decenas de filas, con índice— por una consulta a `cache` más una escritura en cada fallo. **El mecanismo más caro que este paso construye se volvería más lento que no tenerlo.**
2. **Y los tests no lo detectarían**, porque `phpunit.xml` fuerza `redis`. Es exactamente la forma de la incidencia de `1.6d`: *«la retención estaba decidida, documentada y no aplicada»* — aquí sería una caché medida en verde sobre un almacén que el producto no usa.
3. **`operacion.md §4.1`, detalle 3, ya avisaba de la mitad de esto** al decir que la caché tiene que ser compartida y no de proceso. Es el mismo aviso, un escalón más abajo: no basta con que sea compartida, tiene que ser barata, porque **este paso es el primero que la pone en el camino caliente**.

**No lo resolví yo**: fijar `CACHE_STORE` y declarar el cliente de Redis es una decisión de despliegue y de dependencias (`CLAUDE.md §1`), y está fuera del ámbito de una especificación de módulo — issue [#150](https://github.com/pirexia/plataforma-educativa/issues/150). Fue **`OPEN-BO-26`** (§14), **la que había que resolver antes de implementar** porque decidía si la caché de §5.11.9.2 se construye o no. **Resuelta el 2026-09-21 por la salida (a)**, arriba.

##### 5.11.9.2 Segunda comprobación: para componer la clave de caché hay que leer `rules_version`, que ya es una consulta

`datos.md §9.5` y `operacion.md §4.3` deciden que la clave de la caché de evaluación **incluye `rules_version` del *flag***, y que eso es lo que hace innecesaria la invalidación: al incrementarse en la misma transacción que la escritura (`RN-BO-42`), todas las entradas anteriores quedan **inalcanzables de golpe, en todos los centros, sin tocar ninguna clave**. **El mecanismo es correcto y no lo reabro.** Lo que la pasada de implementación tiene que resolver, y que ninguna de las dos secciones dice, es de dónde sale `rules_version` **en el momento de componer la clave**:

> **`rules_version` vive en `feature_flags`. Leerla es una consulta. Luego la caché de evaluación no evita la consulta: evita la *segunda*** —la de `feature_flag_rules`— y el cálculo. Con un solo *flag* por petición el ahorro es real pero pequeño; con diez *flags* en una pantalla, son diez lecturas de versión más diez lecturas de caché para ahorrar diez lecturas de una tabla de decenas de filas.

Tres formas de cerrarlo, con su coste. **No elijo por mi cuenta** porque la tercera se aparta de la letra de `datos.md §9.5`, que está aprobada desde el 2026-09-08:

| Salida | Qué hace | Coste |
|---|---|---|
| **(a)** Dejarlo como está | Una lectura de `rules_version` por *flag* y por petición, más la entrada cacheada de la decisión | Es la letra de lo aprobado. El ahorro neto es pequeño y la caché tiene tantas entradas como (*flag* × sujeto) — con `rollout_unit: 'user'`, una por usuario |
| **(b)** Cachear **el catálogo entero con sus reglas** en **una** entrada, bajo el prefijo base, y evaluar en memoria | Una sola lectura por petición, resuelta una vez por proceso como el catálogo de módulos (`RN-BO-63`). La decisión pasa a ser un hash y dos búsquedas en array: **no hay caché por sujeto y no hay explosión de claves** | La clave necesita una versión **global**, no por *flag*; `rules_version` se conserva tal cual —es lo que la API devuelve y lo que `operacion.md §8` manda mirar— pero deja de estar en la clave de caché |
| **(c)** Sin caché | El evaluador lee `feature_flags` y `feature_flag_rules` una vez por petición | Dos consultas a dos tablas de decenas de filas. Era la opción honesta mientras no hubiera consumidor real que medir **y mientras el almacén fuera `database`** — con `OPEN-BO-26` resuelta por Redis, **ese segundo argumento desaparece** |

**Recomendación: (b).** Conserva íntegro `RN-BO-42` —la versión sigue haciendo inalcanzables las entradas anteriores, sólo que una en vez de N— y es la única de las tres que no crece con el número de usuarios. Va como **`OPEN-BO-24`** (§14), **abierta y sin bloquear**: se resuelve durante la implementación.

> **Lo que `OPEN-BO-26` cambió aquí, y lo que no.** Al fijarse `CACHE_STORE=redis` (§5.11.9.1), **una lectura de caché vuelve a ser barata** y con ello decae el argumento que hacía a (c) atractiva. **Lo que no cambia es el hueco**: `rules_version` sigue viviendo en `feature_flags` y componer la clave sigue exigiendo leerla. Redis abarata el segundo acceso, no elimina el primero.

**Y una propiedad que hay que conservar decida lo que se decida**, porque es la que hace que la caché sea segura y no sólo rápida: **una entrada de caché ausente nunca significa «expuesto»**. `operacion.md §3` ya lo dice —*«la ausencia de entrada en caché no es "expuesto", es "hay que calcularlo", y el cálculo sin reglas da falso»*— y con cualquiera de las tres salidas sigue siendo cierto. Un Redis vacío, un `database` recién migrado o un PostgreSQL caído producen **falso**, nunca verdadero (`RN-BO-35`).
...
### 7.7 El evaluador y las escrituras de *flag* · sub-paso `1.6e`

> Mismo criterio de numeración que §7.2.1, §7.3.1 y §7.6: **van a continuación de la última existente**, `RN-BO-98`, y no intercaladas con `RN-BO-34` a `RN-BO-47`, que son las de §7.5 y están citadas desde los otros cuatro ficheros. Su sitio temático es §7.5; su número es el que les toca. **Ninguna de las once sustituye a una anterior**: nueve la completan y dos —`RN-BO-105` y `RN-BO-107`— salen de comprobar el código y encontrar un hueco.

| ID | Regla |
|----|-------|
| `RN-BO-99` | **El evaluador se consume por interfaz y nunca por la tabla.** `FeatureFlagEvaluator` y `FeatureFlagExplainer` viven en **`App\Support\FeatureFlags`**, las implementa `REQ-CORE` en `Core\Infrastructure` y las enlaza `CoreServiceProvider`, con el precedente literal y verificado de `ModuleAvailability` (§5.11.8). **Ningún módulo del producto lee `feature_flags` ni `feature_flag_rules`**, ni por modelo ni por consulta: `INV-007` y el error característico de «consultar directamente tablas de otro módulo». El único que las escribe es `REQ-BO` (`datos.md §9.6`) |
| `RN-BO-100` | **`isEnabled(string $key): bool` no acepta tenant ni usuario**: los toma del contexto, y **sin contexto de tenant devuelve `false`**, nunca el valor de otro tenant y nunca una excepción. Es la propiedad que hace que `INV-001` no dependa de que nadie pase un segundo argumento, y la que convierte `RN-BO-41` en un caso particular de la misma regla (§5.11.8.1) |
| `RN-BO-101` | **Un solo algoritmo, en una función de decisión pura.** Las dos interfaces delegan en la misma implementación del orden de §5.11.5, sin acceso a base de datos dentro. Mismo argumento que `RN-BO-22` para el cierre de dependencias: dos implementaciones significan que el `matched_by` que se lee al depurar puede no ser el motivo real, que es la peor forma de que falle una herramienta de diagnóstico |
| `RN-BO-102` | **El módulo dueño se comprueba antes que nada.** Con `module_code` no nulo, el paso 0 del evaluador es `ModuleAvailability::isEnabled($flag->module_code)`, y si es falso el resultado es falso con `matched_by = 'module_disabled'`. **Refuerza `RN-BO-45` y no la cambia**: `CA-BO-092` la garantiza sólo en el camino HTTP, donde actúa `EnsureModuleEnabled`; esta regla la extiende a colas, comandos y *listeners*, que no pasan por él (§5.11.8.4) |
| `RN-BO-103` | **Retirar un *flag* también incrementa `rules_version`.** `RN-BO-42` nombra «toda escritura de reglas o del estado», y `retired_at` no es ninguna de las dos: lo escribe `platform:sync-registry` por `plataforma_owner`. Sin este incremento, un *flag* retirado en un despliegue **seguiría evaluando verdadero** hasta que caducara el TTL, que es exactamente lo que `RN-BO-44` dice que no puede pasar (`CA-BO-172`) |
| `RN-BO-104` | **`PUT …/rules` es un reemplazo del conjunto resuelto por diferencia, en una transacción.** Borra lógicamente las reglas que desaparecen, inserta las nuevas, actualiza las que cambian, y **incrementa `rules_version` una sola vez** por petición, no una por regla. Una regla retirada conserva su fila con `deleted_at` y **sigue siendo consultable**: es la prueba de a qué centros se expuso qué y cuándo (`datos.md §13`). Los tres índices únicos parciales de `datos.md §9.3` son `WHERE deleted_at IS NULL`, así que volver a añadir una regla idéntica crea fila nueva y no resucita la anterior |
| `RN-BO-105` | **Toda escritura de *flags* necesita un valor de `admin_action_logs.action` que hoy no existe.** Ese vocabulario es **cerrado por `CHECK`**, y `1.6e` lo amplía por migración con `flag.estado_cambiado`, `flag.reglas_cambiadas`, `tenant.early_adopter_designado` y `tenant.early_adopter_retirado` (`datos.md §4.2.2`). **Sin esa migración, `RN-BO-43` no se puede cumplir**: la auditoría va en la misma transacción que la escritura, así que el `CHECK` haría fallar la operación entera — y el síntoma sería «no puedo apagar el *flag* que está rompiendo colegios». `subject_type` gana `feature_flag` **sin** migración, porque ese vocabulario es abierto |
| `RN-BO-106` | **El evaluador es perezoso y no está en ninguna pila de *middleware*.** No se precarga nada, no se resuelve nada en el arranque, y **una petición que no consulte ningún *flag* no ejecuta ni una consulta ni una lectura de caché del motor** (`CA-BO-175`). Complementa `RN-BO-47` por el otro lado: aquella prohíbe que un control de seguridad dependa del motor; ésta prohíbe que el motor se cuele en el camino de toda petición sin que nadie lo haya pedido |
| `RN-BO-107` | **Los códigos de rol se leen dentro de `REQ-CORE`, por un método con nombre.** `roles`, `role_user` y `User::roles()` son de `REQ-CORE` (`REQ-CORE-004`), **no de `REQ-PERM`** — §5.11.8.3 corrige la atribución de §9—, así que no hay frontera que cruzar ni superficie pública ajena que ampliar. Lo que no se admite es un `pluck('code')` en línea: la semántica de `RN-BO-40` —código y no fila, código inexistente no es error, vale para roles personalizados— vive en un solo sitio y tiene su test |
| `RN-BO-108` | **Qué estados de tenant admiten la designación de *early adopter***: `en_alta`, `activo`, `suspendido` y `en_baja` **sí**; `eliminado` **no**, `409` con `bo.flag.tenant_state_invalid`. Es la **tercera** regla de esa familia y **su vocabulario de estados no coincide con las otras dos**, que es justo el motivo de que tenga clave propia: `en_alta` **sí** admite designación y **no** admite escritura de módulos (`RN-BO-71`), porque `early_adopter_since` es una columna de `tenants` que el aprovisionamiento no toca. `eliminado` se cierra por el mismo argumento de siempre: meter en la cohorte de novedades a un centro cerrado es una escritura sobre algo que ya no se opera |
| `RN-BO-109` | **La designación de *early adopter* no incrementa ningún `rules_version`, y es la única escritura del sub-paso cuya visibilidad depende del diseño de caché.** No ha cambiado ninguna regla: lo que cambia es el sujeto contra el que se evalúan las reglas de cohorte. Con una caché por sujeto hay que invalidar **la del centro afectado** —un solo prefijo, cabe en la petición, mecanismo de `ADR-045 §8.3` ya probado en `1.6c`—; con el catálogo en una sola entrada no hay nada que invalidar, porque el dato del centro no está dentro. **Es el único punto en que las salidas de `OPEN-BO-24` no son intercambiables**, y por eso la decisión tiene que nombrarlo explícitamente (`api.md §2.11.1`) |
...
| **`1.6e`** · Se consulta un *flag* cuya clave no existe en el catálogo | **Falso**, sin excepción y sin aviso (`RN-BO-44`, paso 1 de §5.11.5). Una clave mal escrita en el código **no enciende nada y no rompe la petición**: es fallo en cerrado. Quien quiera detectar la errata tiene `GET /feature-flags`, no un `500` en producción |
| **`1.6e`** · Se consulta un *flag* desde un trabajo en cola, un comando o una petición sin sesión | Si tiene reglas de rol, **falso** (`RN-BO-41`, `CA-BO-087`). Si no las tiene, se evalúa con el tenant del contexto. **Y sin tenant en el contexto, falso siempre** (`RN-BO-100`), igual que `ModuleAvailability::isEnabled()` |
| **`1.6e`** · Un *flag* de un módulo que el centro no tiene contratado está al 100 % | **Falso**, con `matched_by = 'module_disabled'` (`RN-BO-102`). Por el camino HTTP nunca se llega a preguntarlo —`EnsureModuleEnabled` responde antes con `urn:pge:error:module-disabled` (`CA-BO-092`)—, y por los demás caminos lo cierra el paso 0 del evaluador |
| **`1.6e`** · Se retira un *flag* del descriptor y se despliega | `platform:sync-registry` escribe `retired_at`, **incrementa `rules_version`** y el evaluador deja de darlo verdadero **de inmediato**, sin esperar al TTL (`RN-BO-103`). Sus reglas **no se borran**: siguen consultables como prueba de a qué centros se expuso (`datos.md §13`) |
| **`1.6e`** · Se envía un `PUT …/rules` con el mismo conjunto que ya está escrito | `200`, y es **idempotente por construcción** (`api.md §6`). Lo único que produce de más es un incremento de `rules_version` —que invalida caché de sobra— y una entrada de auditoría, que es el registro fiel de que alguien envió la operación dos veces |
| **`1.6e`** · Se apaga con `forced_off` un *flag* que tiene regla nominal de tenant **y** cohorte | **Falso para todos**, y no se mira ninguna regla (`RN-BO-37`, `CA-BO-082`). El interruptor está por encima de los cuatro ejes: es la «reversión inmediata sin nuevo despliegue» de `RNF-MANT-005`, y una reversión que admitiera excepciones no sería una reversión |
| **`1.6e`** · Se restaura la base de datos de plataforma a un punto anterior | **Puede volver a encender un *flag* que estaba en `forced_off`.** Es el único caso de todo `REQ-BO` en el que una restauración va en la dirección insegura, y por eso `operacion.md §9` obliga a revisar la lista de `forced_off` **antes** de dar el servicio por restaurado |
| **`1.6e`** · Se designa *early adopter* a un centro | Escribe `tenants.early_adopter_since` y deja entrada `tenant.early_adopter_designado`. **No escribe fila en `tenant_lifecycle_events`**: no hay transición de estado, y esa tabla es la máquina de estados y no un diario — mismo argumento con el que `datos.md §6.2` justificó `grace_period_expired_at` (`CA-BO-178`) |
| **`1.6e`** · Se clona un centro que era *early adopter* | El clon **no** hereda la designación (`RN-BO-59`, `CA-BO-123`). Heredarla metería a un centro nuevo en despliegues progresivos sin que nadie lo haya decidido |
...
**`INV-007` en la frontera de los *feature flags*: el backoffice escribe las reglas, pero no las evalúa.** La evaluación (`FeatureFlagEvaluator`) ocurre **dentro** de la petición de un centro, la consume cualquier módulo del producto y por tanto **pertenece a `REQ-CORE`**, junto a `ModuleAvailability` y **en su mismo espacio de nombres, `App\Support\FeatureFlags`** — el diseño completo, con su firma y con por qué no va en `Core\Domain`, es **§5.11.8**. El backoffice sólo escribe `feature_flags`, `feature_flag_rules` y `tenants.early_adopter_since`; nunca expone una función de evaluación al producto. Es el mismo reparto que `ADR-045 §4.8` impuso a los eventos de módulo —los emite `REQ-CORE`, no el backoffice— y por el mismo motivo: el consumidor no debe conocer al escritor.

**Con una salvedad que `1.6e` tiene que nombrar, porque es la única lectura que va en la dirección contraria**: el backoffice sí necesita **evaluar para otro centro**, sin tenant activo y con el motivo incluido, para servir `GET /tenants/{public_id}/feature-flags` y la vista previa (`api.md §2.13`). Eso lo sirve **`FeatureFlagExplainer`**, una segunda interfaz de `REQ-CORE` con sujeto explícito, **que ningún módulo del producto inyecta** y cuyo uso queda acotado a `app/Modules/Backoffice` por test de arquitectura (§5.11.8.2, `CA-BO-169`). Las dos interfaces las sirve **una sola implementación y un solo algoritmo** (`RN-BO-101`): dos habrían significado que el `matched_by` que un operador lee al depurar pudiera no ser el motivo real.

Dos consecuencias concretas de ese reparto, que hay que verificar antes de implementar y no después:

1. **El filtro por rol necesita los códigos de rol del usuario autenticado, y son dato de `REQ-CORE` — no de `REQ-PERM`** (§5.11.8.3). El evaluador vive en `REQ-CORE`, y `roles`, `role_user` y `App\Models\User::roles()` también lo son (`REQ-CORE-004`; el esquema es de `0.8` por `ADR-034 §2`). Leerlos desde `Core\Infrastructure` **no cruza ninguna frontera de módulo** y no hace falta ampliar la superficie pública de `REQ-PERM`. Lo que sí se exige es que esa lectura tenga **un método con nombre y su test** dentro de `REQ-CORE`, y no un `pluck('code')` en línea dentro del evaluador: `RN-BO-40` tiene semántica propia —se compara el código, un código inexistente no es un error, y funciona sobre roles personalizados— y esa semántica vive en un sitio (`RN-BO-107`).

   > **Corrección, hecha a la vista y no en silencio** (§5.11.8.3). Hasta la pasada de `1.6e`, este punto decía que los códigos de rol *«son dato de `REQ-PERM`»* y que, de no existir la interfaz pública correspondiente, *«añadirla es parte de `1.6e` y debe declararse como ampliación de la superficie pública de `REQ-PERM`»*. **Verificado el 2026-09-21 sobre `develop`: la premisa era inexacta.** `REQ-PERM` posee el **resolutor de permisos** (`PermissionResolver::effectivePermissionCodes()`, que devuelve códigos de **permiso** y no de rol), no la propiedad de los roles. La consecuencia práctica es que **`1.6e` no amplía la superficie pública de ningún módulo ajeno por este motivo**, y por tanto no plantea el `OPEN-BO` que `1.6b` y `1.6c` sí tuvieron que plantear (`OPEN-BO-15`, `OPEN-BO-18`). Se deja escrito lo que decía antes porque una especificación que se corrige sin decirlo deja de ser fuente de verdad — mismo criterio con el que `1.6d` trató las dos frases falsas sobre los trabajos del tenant (§5.9.3).
...
- **`CA-BO-071`** · *Dado* cualquier recurso expuesto, *cuando* aparece en una URL o en un cuerpo de respuesta, *entonces* se identifica por `public_id` ULID **o por una clave de catálogo declarada en el registro de `ADR-051 §5.1`** (`feature_flags.key` en las cuatro rutas de `api.md §2.11`, comprobando que cumple las seis condiciones de `ADR-051 §2`), y **nunca** por ninguna otra clave interna. **Reformulado el 2026-09-21** (`OPEN-BO-11`/`ADR-051`): ya no comprueba que exista «una sola excepción» —esa afirmación era falsa contra el código antes de `1.6e` (`ADR-051 §6`)— sino que **ningún recurso de este módulo, ni ningún otro cubierto por el test de arquitectura de `ADR-051 §5.2`, se direcciona por algo que no sea `public_id` o una entrada declarada en ese registro**. El *flag* conserva su `public_id` igualmente.
...
#### 13.7.1 Los que añade la pasada del sub-paso `1.6e`

> **Ninguno de los dieciocho anteriores está satisfecho hoy, y aquí no hay sorpresa que declarar como la hubo en §13.3.1: no existe absolutamente nada.** Verificado el 2026-09-21 sobre `develop` — `feature_flag`, `FeatureFlag`, `pennant`, `Unleash` y `Flagsmith` devuelven **cero resultados** en `apps/api` y en `composer.json`. Lo que sí existe es el **precedente** de cada mecanismo: `ModuleAvailability` con su interfaz en `App\Support` (§5.11.8), `ModuleDescriptor` con `depends_on`/`essential` ya declarados, `SyncModuleRegistry` con su fase de validación previa que aborta sin escribir, `runAsPlatform()` con propósito, y el `CHECK` de `admin_action_logs.action` con la migración de ampliación ya hecha dos veces.
>
> `CA-BO-080` a `CA-BO-097` describen el motor a la altura a la que se escribió el chasis. Los doce que siguen son los que hacen falta para **implementarlo**, y cubren en particular lo que la pasada de `1.6e` encontró sin cubrir: la frontera del evaluador, el vocabulario de auditoría que falta, y el coste en el camino de petición.

**El evaluador y su frontera**

- **`CA-BO-167`** · *Dado* el `enum` `PlatformCapability`, *entonces* declara **exactamente dos casos nuevos** —`flag.leer` y `flag.gestionar`— y ninguno más, y el test de catálogo de `permisos.md §9` sigue cuadrando celda a celda con `§3` y `§4`. **Verificado el 2026-09-21: hoy el `enum` tiene veinticuatro casos y ninguno es de `flag`** (`permisos.md §4.6`).
- **`CA-BO-168`** · *Dado* `FeatureFlagEvaluator::isEnabled()` invocado **sin contexto de tenant**, *entonces* devuelve **`false`**: ni lanza excepción, ni devuelve el valor de otro tenant, ni el valor por defecto de la regla global (`RN-BO-100`). Es el simétrico exacto del comportamiento que el *docblock* de `ModuleAvailability` ya declara, y se comprueba igual.
- **`CA-BO-169`** · *(test de arquitectura)* *Dado* el código completo de `app/`, *cuando* se recorren las inyecciones y los `use`, *entonces* **ningún fichero fuera de `app/Modules/Core` lee `feature_flags` ni `feature_flag_rules`** por modelo o por consulta, **ningún fichero fuera de `app/Modules/Backoffice` inyecta `FeatureFlagExplainer`**, y el backoffice **no inyecta `FeatureFlagEvaluator`** (`RN-BO-99`, §5.11.8.2, `INV-007`).
- **`CA-BO-170`** · *Dado* un conjunto de reglas y un sujeto, *cuando* se evalúa por `isEnabled()` dentro del contexto de ese tenant y por `explain()` desde el backoffice con el sujeto explícito, *entonces* **los dos caminos dan el mismo booleano**, sobre los cinco órdenes de precedencia de §5.11.5 y sobre el caso `forced_off` (`RN-BO-101`). Sin este criterio, el `matched_by` que un operador lee al depurar puede no ser el motivo por el que el centro ve la pantalla.
- **`CA-BO-171`** · *Dado* un *flag* con `module_code` de un módulo **no contratado** por el centro y una regla `global` activa, *cuando* se evalúa **desde un trabajo en cola** —fuera de `EnsureModuleEnabled`—, *entonces* es **falso** y `matched_by` es `module_disabled` (`RN-BO-102`). Es la mitad de `RN-BO-45` que `CA-BO-092` no cubre, porque aquél comprueba el camino HTTP.

**Catálogo, versión y escritura**

- **`CA-BO-172`** · *Dado* un *flag* expuesto en varios centros, *cuando* deja de declararse en el descriptor y se ejecuta `platform:sync-registry`, *entonces* queda con `retired_at`, **`rules_version` se ha incrementado**, y el evaluador devuelve falso **de inmediato y sin esperar al TTL** en todos ellos; y sus reglas **siguen consultables** por `GET /feature-flags/{key}` (`RN-BO-103`, `RN-BO-44`).
- **`CA-BO-173`** · *Dado* un *flag* con tres reglas, *cuando* se envía un `PUT …/rules` que quita una, añade otra y cambia el porcentaje de la tercera, *entonces* ocurre **en una sola transacción**, `rules_version` se incrementa **una vez y no tres**, la retirada conserva su fila con `deleted_at` y sigue siendo consultable, y los tres índices únicos parciales de `datos.md §9.3` siguen sin admitir dos reglas del mismo eje sobre el mismo objetivo (`RN-BO-104`).
- **`CA-BO-174`** · *(regresión del hallazgo de esquema de este sub-paso)* *Dado* el `CHECK` de `admin_action_logs.action` **antes** de la migración de `1.6e`, *cuando* se intenta insertar una fila con `action = 'flag.reglas_cambiadas'`, *entonces* **el motor la rechaza**; y *dado* el `CHECK` **después**, *entonces* los cuatro valores nuevos se aceptan y **ninguno de los existentes ha desaparecido ni cambiado de nombre** (`RN-BO-105`, `datos.md §4.2.2`). La primera mitad es la que demuestra que la migración hacía falta de verdad.
- **`CA-BO-175`** · *(coste en el camino de petición)* *Dado* una petición cualquiera de un usuario de un centro que **no consulta ningún *flag***, *cuando* se cuentan las consultas y las lecturas de caché, *entonces* **el motor de *flags* no ha ejecutado ninguna** (`RN-BO-106`). Es el simétrico de `CA-BO-128` para el catálogo de módulos, y la comprobación de que §12.3 —*«el único que añade código al camino de petición de todos los centros»*— describe una propiedad del diseño y no un coste pagado en fase 1.

**Escrituras, auditoría y aislamiento**

- **`CA-BO-176`** · *Dado* un usuario de un centro con un *flag* expuesto por porcentaje, otro apagado y otro filtrado por un rol que no tiene, *cuando* llama a `GET /api/v1/feature-flags`, *entonces* el conjunto de claves devuelto **coincide exactamente** con el de las que `FeatureFlagEvaluator::isEnabled()` da verdadero para él, sin una de más ni una de menos. Completa `CA-BO-097`, que comprueba qué **no** sale; éste comprueba que los dos caminos no pueden divergir.
- **`CA-BO-177`** · *(aislamiento — obligatorio para cerrar el sub-paso)* *Dado* **tres** tenants y un *flag* al 50 % con `rollout_unit: 'tenant'`, *entonces* el resultado de cada uno es estable, independiente del orden en que se consulten y **el mismo tras reiniciar el proceso**; y *cuando* se evalúa en el centro `A`, *entonces* **ninguna entrada de caché del centro `B` se lee ni se escribe** (`INV-001`, `ADR-033 §9`, y el test obligatorio de la *skill* `aislamiento-tenant`).
- **`CA-BO-178`** · *Dado* un centro al que se designa *early adopter* y después se le retira, *entonces* `tenants.early_adopter_since` se escribe y se pone a nulo, existen **dos** entradas en `admin_action_logs` —`tenant.early_adopter_designado` y `tenant.early_adopter_retirado`, ambas con `affected_tenant_id`—, y **no existe ninguna fila nueva en `tenant_lifecycle_events`**: no ha habido transición de estado (`RN-BO-46`, `datos.md §4.2.2`).
...
| `OPEN-BO-11` · Direccionar un *flag* por su `key` | **RESUELTA 2026-09-21: sí**, las cuatro rutas de `api.md §2.11` van por `key` y el *flag* conserva su `public_id`. **La reserva que dejé —que una excepción a la letra de un ADR vigente merece enmienda formal— se atendió el mismo día: `ADR-051` es esa enmienda**, y cambia la forma del registro aunque no el resultado. `ADR-051` **no lo inscribe como excepción nombrada a `REQ-BO-005`**, que es lo que yo proponía, sino como **regla general comprobable**: una clave de catálogo declarada en código es admisible en URL/API si cumple **seis condiciones a la vez** (§2 de aquel ADR), y `feature_flags.key` las cumple. El motivo por el que descarta mi propuesta es un dato y no una preferencia: **al comprobarlo contra el código, `modules.code` y `permissions.code` ya estaban expuestas igual desde antes de `1.6e`** —hallazgo de severidad Media, `ADR-051 §6`, issue [#238](https://github.com/pirexia/plataforma-educativa/issues/238)—, así que «una sola excepción» era falso el día que se escribió. **No cambia ni una línea de contenido de la especificación**: los tres documentos ya estaban escritos contra el «sí», y el ULID sigue siendo lo que guarda `admin_action_logs.subject_public_id`. **Lo que sí queda pendiente de `spec-writer` es reformular `CA-BO-071`**, que hoy comprueba «que la excepción es una y está acotada» y por tanto afirma algo falso; lo que debe comprobar es que la `key` cumple las seis condiciones y que **ningún otro recurso de este módulo** se direcciona por algo que no sea `public_id`. `ADR-051` añade además a `1.6e` el registro único de claves admitidas y su test de arquitectura (`§5`) |
...
| `OPEN-BO-24` · ¿Qué se cachea exactamente en la evaluación de un *flag*? | **Abierta · nueva** (`1.6e`). **Nace de un hueco de esta especificación**, no de una duda de diseño: la clave de caché lleva `rules_version` y **leer `rules_version` ya es una consulta**. Recomendación: **(b)**, una sola entrada con el catálogo y sus reglas, evaluada en memoria (§5.11.9.2) |
| `OPEN-BO-25` · ¿Se aprueba ampliar la superficie pública de `REQ-CORE` con las **dos interfaces** del evaluador? | **Abierta · nueva** (`1.6e`). Es el análogo de `OPEN-BO-15` y `OPEN-BO-18`, y la **única** ampliación de superficie ajena que este sub-paso necesita. Recomendación: **sí**, enumerada y sin ADR nuevo (§5.11.8) |
| `OPEN-BO-26` · El almacén de caché configurado es `database`, no Redis. ¿Se fija `CACHE_STORE` antes de construir la caché de *flags*? | **RESUELTA 2026-09-21: se fija `CACHE_STORE=redis`** para el *runtime*, no sólo para los tests. Redis ya estaba desplegado (`ADR-033 §9`); lo que faltaba era declararlo. **La caché de *flags* se construye sobre Redis**, y `datos.md §9.5` y `operacion.md §4.3` quedan vigentes **sin corrección**. Corrección de `infra/` por la sesión orquestadora, issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236). **`CLAUDE.md §1` ya no necesita corrección** — a diferencia de `OPEN-BO-23`, aquí la pieza sí estaba. Deuda residual anotada: el cliente de Redis sigue sin declararse en `composer.json` |
...
**Con `1.6e` aparecen tres más, y el reparto es distinto al de los sub-pasos anteriores: sólo una nace de una duda de diseño.** `OPEN-BO-25` es la de siempre —ampliar la superficie pública de `REQ-CORE`, igual que `OPEN-BO-15` en `1.6b` y `OPEN-BO-18` en `1.6c`— y su recomendación es la misma. Las otras dos salen de **comprobar el código**: `OPEN-BO-24` es un hueco de esta especificación que nadie había mirado —la clave de caché necesita un dato que hay que consultar para componerla— y **`OPEN-BO-26` es un hallazgo sobre el estado real del despliegue**, de la misma forma exacta que `OPEN-BO-23` con Horizon: el documento de normas da Redis por instalado y en uso, y ningún fichero del repositorio fija el almacén. **`OPEN-BO-26` y `OPEN-BO-11` eran las dos que había que resolver antes de que `implementer` tocase nada** —la primera decidía si la caché se construye, la segunda la forma de cuatro rutas— y **las dos quedaron resueltas el 2026-09-21**: `CACHE_STORE=redis` (issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236)) y rutas por `key` como excepción acotada. **Ninguna de las dos cambió una sola línea de contenido**, porque los cinco documentos ya estaban escritos contra la salida elegida; lo que cambió es que dejan de ser condicionales. `OPEN-BO-24` y `OPEN-BO-25` **siguen abiertas y no bloquean**: se responden durante la implementación sin rehacer trabajo, tal como se recomendaba.

> **`1.6e` no tiene ninguna pregunta bloqueante pendiente.** Es el estado en el que `1.6b`, `1.6c` y `1.6d` entraron en implementación, y el que §15.5 exige para pasar a `implementer`.

**Y una que esta pasada *no* tiene que plantear, y merece decirse porque se esperaba**: no hay `OPEN-BO` sobre ampliar la superficie pública de **`REQ-PERM`**. §9 la anticipaba, y al comprobarlo contra el código resultó que los códigos de rol son dato de `REQ-CORE` y no de `REQ-PERM` (§5.11.8.3). La pregunta desaparece en vez de resolverse.
...
### `OPEN-BO-11` · ¿Se puede direccionar un *feature flag* por su `key` en la URL? · **RESUELTA 2026-09-21: sí, por `ADR-051`**

> **Decisión del usuario del 2026-09-21: se aprueba.** Las **cuatro** rutas de *flag* de `api.md §2.11` se direccionan por **`key`** —`/feature-flags/comedor.reserva_v2`—, y el *flag* **conserva su `public_id` igualmente**. Las dos rutas de `/tenants/{public_id}/…` no se ven afectadas: ya usaban el ULID del centro.
>
> **La reserva que dejé aquí —que esto merecía enmienda formal de `ADR-029` y no una excepción documentada solo en este módulo— se atendió el mismo día: `ADR-051`.** No es lo que yo había propuesto (una excepción nombrada y acotada a `REQ-BO-005`): `architect` la descartó porque, comprobada contra el código, la premisa de «una sola excepción» era **falsa** el día que se escribió —`modules.code` y `permissions.code` ya estaban expuestas igual desde antes de `1.6e`, sin registro—, y un argumento que no distingue un *feature flag* de un módulo o un permiso no es una excepción, es una categoría sin nombrar. `ADR-051` registra en su lugar una **regla general y comprobable**: una clave de catálogo declarada en código es identificador válido en URL/API cuando cumple seis condiciones a la vez (`ADR-051 §2`), con registro único en código y test de arquitectura que lo hace exigible. `feature_flags.key` las cumple las seis.
>
> **Qué cambia en esta especificación**: nada de contenido en `api.md §2.11`, `api.md §2.11.1` ni `datos.md §11` — las tres ya estaban escritas contra el «sí» y siguen siendo correctas en la forma. Lo que cambia es el fundamento: ya no se sostienen en «excepción acotada a este módulo» sino en las seis condiciones de `ADR-051 §2`, que el módulo demuestra que cumple. **Qué no cambia**: `subject_public_id` de `admin_action_logs` sigue guardando el **ULID** del *flag* y no su `key` (`datos.md §4.2.2`, `ADR-051` C6) — esa columna existe para sobrevivir al borrado del sujeto, y ahí el identificador que nunca cambia es el ULID.
>
> **Trabajo que `ADR-051` deja para este sub-paso, no solo para el registro**: el registro único de claves admitidas y su test de arquitectura (`ADR-051 §5`) entran con `1.6e`, y `modules.code`/`permissions.code` pasan a declararse en ese mismo registro sin tocar ni una migración ni una ruta suya. `CA-BO-071` (§13.6) queda reformulado para comprobar las seis condiciones en vez de «una sola excepción» — ver la corrección en ese criterio.
...
**No lo decidí yo porque era la letra de un ADR vigente** (`CLAUDE.md §11`). Lo señalé en vez de resolverlo por comodidad, y con `1.6e` dejó de ser inocuo: es el sub-paso que crea esas rutas, así que la pregunta pasó de «no bloquea» a «conviene resolverla antes de escribirlas». Resuelta el 2026-09-21.
...
### `OPEN-BO-24` · ¿Qué se cachea exactamente en la evaluación de un *flag*? · **nueva en `1.6e`**

**No es una duda de diseño: es un hueco de esta especificación**, y aparece igual que apareció `OPEN-BO-14` — dos piezas ya escritas que juntas no cierran.

`datos.md §9.5` y `operacion.md §4.3` deciden que la clave de la caché de evaluación **incluye `rules_version`**, y que eso sustituye a la invalidación: incrementarla en la misma transacción que la escritura (`RN-BO-42`) deja inalcanzables de golpe todas las entradas anteriores, en todos los centros, sin tocar ninguna clave. **El mecanismo es correcto y no lo reabro.** Lo que ninguna de las dos secciones dice es de dónde sale `rules_version` **en el instante de componer la clave**: vive en `feature_flags`, luego **leerla es una consulta**. La caché no evita la consulta; evita la segunda y el cálculo.

Las tres salidas, con su coste, están en **§5.11.9.2**. En resumen:

| Salida | Qué cambia | Coste |
|---|---|---|
| **(a)** Dejarlo tal como está escrito | Nada | Una lectura de versión por *flag* y por petición; tantas entradas de caché como (*flag* × sujeto), y con `rollout_unit: 'user'` eso es una por usuario |
| **(b)** Una sola entrada con el catálogo y sus reglas, evaluada en memoria | La clave pasa a llevar una versión **global** en vez de la del *flag*. `rules_version` **se conserva** —es lo que la API devuelve y lo que `operacion.md §8` manda mirar— pero deja de estar en la clave | Una lectura por petición, resuelta una vez por proceso como el catálogo de módulos (`RN-BO-63`). **No crece con el número de usuarios** |
| **(c)** Sin caché | Se retira `BO_FLAG_CACHE_TTL` y la tercera fila de `operacion.md §4.4` | Dos consultas a dos tablas de decenas de filas. Honesta mientras no haya un consumidor real que medir |

**Recomendación: (b)**, y **(c) si `OPEN-BO-26` se resuelve dejando el almacén `database`** — sobre ese almacén, (a) es medir el ahorro en la moneda equivocada. **Lo señalo en vez de decidirlo** porque (b) se aparta de la letra de `datos.md §9.5`, aprobada el 2026-09-08, y cambiar en silencio lo que dice un documento aprobado es cómo una especificación deja de ser fuente de verdad.

**Lo que no cambia se decida lo que se decida**, y es la propiedad que hace segura a la caché y no sólo rápida: **una entrada ausente nunca significa «expuesto»** (`RN-BO-35`, `operacion.md §3`).

### `OPEN-BO-25` · ¿Se aprueba ampliar la superficie pública de `REQ-CORE` con las dos interfaces del evaluador? · **nueva en `1.6e`**

Es el análogo exacto de `OPEN-BO-15` (`1.6b`) y `OPEN-BO-18` (`1.6c`), y la **única** ampliación de superficie ajena que este sub-paso necesita. Lo que `1.6e` pide escribir fuera de `app/Modules/Backoffice`:

| Pieza | Dónde | Por qué ahí |
|---|---|---|
| `FeatureFlagEvaluator` (interfaz) | `App\Support\FeatureFlags` | Precedente literal y verificado: `App\Support\Modules\ModuleAvailability` (§5.11.8) |
| `FeatureFlagExplainer` (interfaz) | Ídem | Sujeto explícito, sólo para el backoffice, con enlace que deniega por defecto (§5.11.8.2) |
| `FeatureFlagSubject`, `FeatureFlagDecision`, `FeatureFlagRuleSet` | Ídem | Objetos de valor de las dos firmas |
| `EloquentFeatureFlagEvaluator` | `App\Modules\Core\Infrastructure` | Implementación única de las dos, con el algoritmo de §5.11.5 |
| Enlace de las dos | `CoreServiceProvider::register()` | Junto a la línea que ya enlaza `ModuleAvailability` |
| `feature_flags` en `ModuleDescriptor` y en `moduleDescriptor()` | `App\Modules\Core\Domain`, `App\Support\Modules\SyncModuleRegistry` | El catálogo se declara y se materializa donde ya se declaran y materializan `depends_on` y `essential` (`RN-BO-34`) |
| El método con nombre que devuelve los códigos de rol | `App\Modules\Core` | `roles` y `role_user` son suyos (§5.11.8.3, `RN-BO-107`) |

**Recomendación: sí, con esa lista cerrada y sin ADR nuevo.** El mecanismo ya está decidido —§9 asignó el evaluador a `REQ-CORE` en la pasada aprobada del 2026-09-08— y lo que falta es sólo el permiso para escribirlo, exactamente el mismo permiso acotado que `ADR-048 §10` pidió para `1.6b` y que `OPEN-BO-18` concedió a `1.6c`. **Si se resuelve en contra**, la única alternativa coherente es que el evaluador viva en `REQ-BO` — y eso **contradice §9** y obliga a que los cincuenta y tres módulos del producto importen del backoffice, que es lo contrario de `INV-007`. No hay una tercera salida razonable, y por eso esta pregunta no bloquea: bloquea sólo dónde se *commitea*.

### `OPEN-BO-26` · El almacén de caché configurado es `database`, no Redis · **RESUELTA 2026-09-21: se fija `CACHE_STORE=redis`**

> **Decisión del usuario del 2026-09-21: salida (a).** Se fija **`CACHE_STORE=redis`** para el *runtime* real, no sólo para los tests. **Redis ya está desplegado y en uso para otros fines** —el prefijo por tenant de `ADR-033 §9`, el límite de tasa—, así que lo que faltaba no era la pieza sino **declararla explícitamente** en la configuración de ejecución. La corrección de `compose.yaml` y de las unidades Quadlet la aplica la sesión orquestadora por separado, fuera del ámbito de esta especificación, bajo el issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236), que recoge la decisión.
>
> **Consecuencia sobre este sub-paso, y es la que importa: la caché de evaluación de *flags* se construye sobre Redis, tal como `datos.md §9.5` y `operacion.md §4.3` daban por supuesto antes de que esta pasada detectara la contradicción.** Las dos secciones quedan **vigentes sin una sola corrección**, y ninguna de las tres salidas de `OPEN-BO-24` queda descartada por este motivo: lo que esta decisión retira es el argumento *«sobre `database` la caché no ahorra nada»*, no el resto del razonamiento. `OPEN-BO-24` sigue abierta y sigue sin bloquear.
>
> **Qué cambia en los documentos**: `operacion.md §2` gana la fila de `CACHE_STORE` con su valor decidido, `operacion.md §4.3.1` deja de plantear la pregunta y registra la salida, y `operacion.md §10` cambia la fila de `CLAUDE.md §1` — **ya no hay nada que corregir allí**: la tabla de *stack* decía «Redis, instalado y en uso» y, una vez fijada la variable, **dice la verdad**. **Es la diferencia con `OPEN-BO-23`**, donde la salida fue corregir la documentación porque la pieza no estaba; aquí la pieza estaba y lo que faltaba era usarla.
>
> **Lo que queda como deuda residual y no se resuelve aquí**: **`composer.json` no declara `predis/predis` ni `ext-redis`.** El cliente existe en la imagen —los tests corren sobre Redis y pasan—, pero **no está declarado**, así que nada impide que un cambio futuro de la imagen base lo retire y el fallo aparezca en ejecución y no en el despliegue. `CLAUDE.md §1` exige declarar y envolver toda dependencia externa (`RNF-MANT-007`). **Se anota en el issue #236 y no se convierte en una pregunta abierta nueva**: no bloquea `1.6e` y es trabajo de dependencias, no de especificación de módulo.

**El hallazgo que dio lugar a la pregunta, que se conserva porque explica por qué había que preguntarlo.** Era de la misma forma exacta que `OPEN-BO-23`: un documento de normas que da por desplegada una pieza que no lo estaba del todo, y una especificación escrita encima de ese supuesto.

**Verificado el 2026-09-21 y no supuesto** (§5.11.9.1): `config/cache.php` tiene `'default' => env('CACHE_STORE', 'database')`; **ningún fichero del repositorio fija `CACHE_STORE`** para el *runtime* —`infra/quadlet/plataforma.env.example` y `infra/compose/compose.prodlike.yaml` fijan `REDIS_HOST` y nada más—; `composer.json` **no declara `predis/predis` ni `ext-redis`**; y `phpunit.xml` **sí** fuerza `CACHE_STORE=redis`, de modo que **la suite prueba un almacén distinto del que el producto usa por defecto**.

**No afirmo que el despliegue corra sobre `database`**: `CACHE_STORE` puede estar puesta en el `EnvironmentFile` real, que no está en el repositorio. Afirmo lo comprobable: el valor de respaldo es `database`, nada en el repositorio lo cambia, y `CLAUDE.md §1` describe Redis como *«instalado y en uso»*.

**Por qué le importa a `1.6e` y no le importó a los cuatro sub-pasos anteriores.** Es el primero que pone una caché en el camino de petición de un centro. Sobre el almacén `database`, **una lectura de caché es una consulta SQL a la tabla `cache`**: la caché de *flags* dejaría de ser un ahorro y pasaría a sustituir una consulta a una tabla de decenas de filas con índice por una consulta a `cache` más una escritura en cada fallo. Y **los tests no lo detectarían**, porque corren sobre Redis — es exactamente la forma de la incidencia de `1.6d`, donde la purga estaba *«decidida, documentada y no aplicada»*.

**Las tres salidas que se plantearon, con la elegida marcada.** No las decidí yo porque era una decisión de despliegue y de dependencias (`CLAUDE.md §1` exige justificar toda dependencia nueva; issue [#150](https://github.com/pirexia/plataforma-educativa/issues/150) me prohíbe arreglar lo que está fuera de mi encargo):

| Salida | Qué hay que hacer | Consecuencia sobre `1.6e` |
|---|---|---|
| **(a) · ELEGIDA, 2026-09-21** | Una línea en los ficheros de entorno de `infra/`, una dependencia declarada en `composer.json`, y comprobar que el contenedor de Redis está desplegado | La caché de §5.11.9.2 se construye tal como está diseñada. **Es la salida coherente con `ADR-033 §9` y con `CLAUDE.md §1`.** Aplicada bajo el issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236); **la declaración del cliente en `composer.json` queda como deuda residual anotada en ese mismo issue** |
| **(b)** Dejar `database` y **corregir la documentación**, como se hizo con Horizon | `CLAUDE.md §1` deja de decir «instalado y en uso», y `operacion.md §3`/`§4.1` dejan de decir Redis | `OPEN-BO-24` se resuelve como **(c)**: sin caché. Es más lento en el papel y más rápido de verdad |
| **(c)** No decidirlo ahora | — | **`1.6e` no puede cerrar su diseño de caché**, y se implementaría contra un supuesto que nadie ha comprobado. Es lo que pasó con Horizon |

**La recomendación fue: (a) si el contenedor de Redis está desplegado de verdad; (b) si no lo está; y en ningún caso (c).** Lo estaba, y se eligió **(a)**. El issue que `CLAUDE.md §6` regla 6 exige —es una contradicción entre código y documentación, **media como mínimo**— es el [#236](https://github.com/pirexia/plataforma-educativa/issues/236), abierto por la sesión orquestadora: escribir en `infra/` y en `CLAUDE.md` está fuera del ámbito de una especificación de módulo, igual que lo estuvo la corrección de `CLAUDE.md` que trajo `OPEN-BO-23`.

---

### `OPEN-BO-27` · Vista previa/impacto ciegos a `percentage` cuando `rollout_unit: 'user'` · **Abierta, no bloquea, surgida en la revisión independiente de `1.6e` (2026-09-22)**

Detalle completo, alternativas (a)/(b) y por qué no la resuelve esta sesión: §15.5.1. Resumen: `previewRules()`/`computeImpact()` calculan el impacto por centro, sin usuario concreto, así que una regla `percentage` con `rollout_unit: 'user'` nunca puede resolver a verdadero en ese cálculo — `by_rule.percentage` sale siempre 0, con independencia del porcentaje real configurado, aunque `isEnabled()` en producción sí evalúe correctamente por usuario. No es fuga de datos ni cruce de tenants: es una limitación de la herramienta de vista previa, no del motor de evaluación.
...
| # | Qué | Dónde |
|---|---|---|
| 1 | **El evaluador dejó de ser un nombre de clase.** Dos interfaces en `App\Support\FeatureFlags` con el precedente **verificado** de `ModuleAvailability`, su firma, qué hace sin contexto, y por qué no va en `Core\Domain` aunque `1.6b` y `1.6c` pusieran ahí los suyos | §5.11.8, `RN-BO-99` a `RN-BO-101`, `OPEN-BO-25` |
| 2 | **La segunda interfaz, que faltaba entera.** `api.md §2.13` promete dos lecturas —`matched_by` y la vista previa— que evalúan **para otro centro y sin tenant activo**, y que la firma del camino de petición no puede servir por construcción | §5.11.8.2, `CA-BO-169`, `CA-BO-170` |
| 3 | **§9 atribuía los códigos de rol a `REQ-PERM` y es inexacto**, verificado sobre `develop`: son de `REQ-CORE` (`REQ-CORE-004`). **La consecuencia es que una pregunta abierta que se esperaba desaparece** en vez de resolverse — no hay ampliación de superficie de `REQ-PERM` | §5.11.8.3, §9, `RN-BO-107` |
| 4 | **Hallazgo de esquema: `admin_action_logs.action` no tiene ningún valor `flag.*`**, y su vocabulario es cerrado por `CHECK`. Sin migración, `RN-BO-43` **no se puede cumplir** y el modo de fallo es que no se pueda apagar el *flag* que está rompiendo colegios | `datos.md §4.2.2`, `RN-BO-105`, `CA-BO-174` |
| 5 | **Hallazgo de despliegue: el almacén de caché configurado es `database`, no Redis**, y la suite prueba el otro. Misma forma exacta que el hallazgo de Horizon de `1.6d`, sobre la pieza de la que depende el único mecanismo que este paso pone en el camino caliente | §5.11.9.1, `OPEN-BO-26` |
| 6 | **Hueco de diseño: la clave de caché lleva `rules_version` y leerla ya es una consulta.** La caché no evita la consulta, evita la segunda | §5.11.9.2, `OPEN-BO-24` |
| 7 | **El evaluador comprueba el módulo dueño antes de mirar una regla.** `CA-BO-092` garantizaba `RN-BO-45` sólo por el camino HTTP; colas, comandos y *listeners* quedaban fuera | §5.11.8.4, `RN-BO-102`, `CA-BO-171` |
| 8 | **Dos escrituras que nadie había descrito**: retirar un *flag* también incrementa `rules_version`, y `PUT …/rules` es un reemplazo por diferencia con **un solo** incremento por petición | `RN-BO-103`, `RN-BO-104`, `CA-BO-172`, `CA-BO-173` |
| 9 | **El coste en el camino de petición, acotado con honestidad.** §12.3 dice que es «el único que añade código al camino de petición de todos los centros»: sigue siendo cierto del diseño, y en fase 1 **el único llamador es `GET /api/v1/feature-flags`** | §5.11.9, `RN-BO-106`, `CA-BO-175` |
| 10 | **El emparejamiento operación por operación**, que `1.6b`, `1.6c` y `1.6d` tienen y los *flags* no tenían | `permisos.md §4.6`, `CA-BO-167` |
| 11 | **Once reglas nuevas** (`RN-BO-99` a `RN-BO-109`) y **doce criterios nuevos** (`CA-BO-167` a `CA-BO-178`). Una clave de error nueva, `bo.flag.tenant_state_invalid`, y **ningún `type` nuevo** en el catálogo cerrado | §7.7, §13.7.1, `api.md §5` |
| 12 | **Tres migraciones, no una ni dos.** Corrección tras el cierre (revisión independiente, 2026-09-22): la cuenta original de este punto decía "dos", contando las tablas de *flags* y `tenants.early_adopter_since` como una sola migración y sumando la ampliación del `CHECK`. `datos.md §12` (autoritativa, filas 6/6b/7) siempre las separó en tres, y así se implementó — ver §15.5.1 | `datos.md §12` |

**Y cuatro preguntas que no resolví yo. Las dos bloqueantes quedaron resueltas el 2026-09-21; las dos que quedan no bloquean:**

| Pregunta | Estado | Qué queda |
|---|---|---|
| **`OPEN-BO-26`** · ¿Se fija `CACHE_STORE=redis` o se corrige la documentación? | **RESUELTA 2026-09-21: (a), se fija `CACHE_STORE=redis`** | Nada en esta especificación: `datos.md §9.5` y `operacion.md §4.3` quedan **vigentes sin corrección**. La corrección de `infra/` va por el issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236). **Deuda residual anotada**: el cliente de Redis sigue sin declararse en `composer.json` |
| **`OPEN-BO-11`** · ¿Rutas por `key` o por `public_id`? *(abierta desde 2026-09-08)* | **RESUELTA 2026-09-21: por `key`**, mediante **`ADR-051`** (enmienda de `ADR-029`: regla general de seis condiciones, no excepción nombrada) | Nada de contenido: los tres documentos ya estaban escritos contra el «sí». `CA-BO-071` se reformula para comprobar las seis condiciones de `ADR-051 §2`. `1.6e` gana el registro único de claves y su test de arquitectura (`ADR-051 §5`) |
| **`OPEN-BO-24`** · ¿Qué se cachea exactamente? | **Abierta · no bloquea** | Recomendación **(b)**. Con `OPEN-BO-26` resuelta por Redis, la salida **(c)** deja de estar sobre la mesa por el argumento del almacén. Afecta a `datos.md §9.5`, `operacion.md §4.3`/`§4.4` y `BO_FLAG_CACHE_TTL`; **`RN-BO-42` se conserva íntegra en las tres salidas**. Se resuelve durante la implementación |
| **`OPEN-BO-25`** · ¿Se amplía la superficie pública de `REQ-CORE`? | **Abierta · no bloquea** | Recomendación **sí**, lista cerrada, sin ADR nuevo. Afecta sólo a dónde se *commitea*: la alternativa —evaluador en `REQ-BO`— contradice §9 y `INV-007`. Se resuelve durante la implementación |

> **Con esto, `1.6e` no tiene ninguna pregunta bloqueante pendiente**, que es el estado en el que `1.6b`, `1.6c` y `1.6d` entraron en implementación. Lo único que falta antes de `implementer` es la aprobación de la especificación completa.

**`OPEN-BO-04` y `OPEN-BO-06` siguen abiertas y siguen sin bloquear**: la primera es un ADR que formalice el cierre de `ADR-036`, y la segunda es la retención de `admin_action_logs`, que `1.6e` no toca — las cuatro filas nuevas que este sub-paso escribe ahí se rigen por lo que se decida, no lo condicionan.

**Dependencias de otros módulos**: una, y enumerada en `OPEN-BO-25`. **No hay dependencia de ningún módulo sin implementar**: `REQ-CORE` está en producción desde `1.1`, `ModuleAvailability` y `ModuleCatalog` existen, y `tenants` tiene ya las cuatro columnas de `1.6b`. **Lo único que este sub-paso necesita y no existe es `tenants.early_adopter_since`**, que es una migración aditiva suya (`datos.md §12` fila 7).
...
### 15.5.1 Cierre de implementación (2026-09-22)

Implementado en la rama `feature/REQ-BO-005-feature-flags`. Las siete rutas de `api.md §2.11`-`§2.13` más `GET /api/v1/feature-flags` (§2.14, en `REQ-CORE`), **tres** migraciones (`feature_flags`/`feature_flag_rules`, la ampliación del `CHECK` de `admin_action_logs.action`, y `tenants.early_adopter_since` — el resumen de §15.5 punto 12 decía "dos" por error de recuento, corregido ahí mismo; `datos.md §12`, autoritativa, siempre las separó en tres), la columna `tenants.early_adopter_since`, las dos interfaces de `App\Support\FeatureFlags` (`FeatureFlagEvaluator`, `FeatureFlagExplainer`) implementadas por `EloquentFeatureFlagEvaluator` en `App\Modules\Core\Infrastructure`, y las dos capacidades nuevas (`flag.leer`, `flag.gestionar`).

**Verificación en dos fases.** El `implementer`, en su *worktree* aislado: 399/399 Pest sobre `tests/Feature/Core`, `tests/Feature/Backoffice`, `tests/Feature/Tenancy`, `tests/Feature/Support` y `tests/Unit`, Larastan (0 errores) y Pint limpios — con 74 tests de `tests/Feature/Auth` fallando en su *sandbox*, sin poder confirmar la causa (ver nota de entorno más abajo). La sesión orquestadora, contra el contenedor de referencia `plataforma-api` tras aplicar las migraciones: **740/740 Pest de la suite completa en verde** (confirma que los 74 fallos eran el problema de entorno, no una regresión), Pint limpio (842 ficheros), Larastan sin errores.

**Revisión independiente (2026-09-22, `db-reviewer`/`security-reviewer`/`doc-reviewer` en paralelo) y correcciones aplicadas en la misma sesión, antes de mezclar:**

| # | Hallazgo | Severidad | Revisor(es) | Corrección |
|---|---|---|---|---|
| 1 | `GET /api/v1/feature-flags` evaluaba sin comprobar sesión — el docblock afirmaba seguir el *guard* de `MeController` y no lo tenía; `INV-002` | **Alto** | security | Añadido `currentUser()` (401 sin sesión), mismo patrón que `MeController`. Test de regresión nuevo (`INV-002: GET /api/v1/feature-flags exige sesión...`) |
| 2 | Falta `CHECK (length(btrim(reason)) > 0)` en `feature_flag_rules.reason`, exigido por `datos.md §9.3` "en el motor, no en el FormRequest" | Media | db + doc (independientes) | Migración aditiva nueva, `2026_09_22_100300_add_reason_check_to_feature_flag_rules.php` |
| 3 | `PUT /feature-flags/{key}/rules` devolvía `affected_tenant_id` como `bigint` interno en el JSON de respuesta — `ADR-029`; `api.md §648` ya decía explícitamente que la API solo lleva `tenant_public_id` | Media | doc | `FeatureFlagsService::replaceRules()` retira el campo de la respuesta tras usarlo para `admin_action_logs` (que sí necesita el interno, `ADR-047 §4.2`) |
| 4 | Resumen de §15.5 punto 12 decía "dos migraciones" (son tres) | Media | db + doc (independientes) | Corregido en §15.5 y en el párrafo de arriba |
| 5 | `rollout_unit: 'user'` deja `previewRules()`/`computeImpact()` estructuralmente ciegos a las reglas `percentage` (siempre 0, cualquier tenant) porque el sujeto de impacto no lleva usuario | Media | security | **No corregido, documentado como limitación conocida** — `OPEN-BO-27` (ver abajo). Calcularlo con precisión exige muestrear usuarios por centro, coste no acotado por la especificación; decisión de alcance, no de esta sesión |
| 6 | Registro `ADR-051 §5.1` sin `permissions.code` — issue [#238](https://github.com/pirexia/plataforma-educativa/issues/238) solo a medias cerrado | Media | db | **No corregido — ambigüedad real de la propia especificación, señalada al usuario, no decidida por esta sesión** (ver nota abajo) |
| 7 | Comentario obsoleto en `routes.php:184` (`FeatureFlagKeyRoutes::REGISTRY`, clase inexistente; la real es `CatalogKeyRouteParameters`) | Baja | db | No corregido, issue abierto (`CLAUDE.md §5`: Baja se documenta y no se toca sin que el usuario lo pida) |

**Sobre el hallazgo 6, sin decidir**: `ADR-051 §5.2` define el test de arquitectura como un recorrido de **parámetros de ruta**; `CatalogKeyRouteParameters::REGISTRY` está diseñado y documentado exactamente para eso. `permissions.code` **nunca es parámetro de ruta** — aparece en el cuerpo de `PUT /roles/{public_id}/permissions` y en la respuesta de `GET /permissions` (`ADR-051 §4`, tabla) — así que añadirlo al registro no lo cubriría por el test pese a lo que dice `ADR-051 §5.3` ("las dos se declaran en el mismo registro y pasan a estar cubiertas por el mismo test"). Cerrarlo de verdad exige o ampliar el contrato del registro más allá de rutas (cambio de diseño) o aceptar que `permissions.code` se documenta pero no se comprueba automáticamente — ninguna de las dos la decide esta sesión de corrección de hallazgos. Issue #238 queda con este comentario, sin cerrar.

**`OPEN-BO-24` resuelta: recomendación (b), catálogo completo en una sola entrada de caché.** `FeatureFlagCatalogCache` (`App\Modules\Core\Infrastructure`) guarda el catálogo materializado con sus reglas vigentes en **una** clave fija (`feature-flags:catalog:v1`), leída directamente por el conector Redis (`Illuminate\Support\Facades\Redis`, conexión `cache`) — **deliberadamente fuera** del prefijo `t{tenant_id}:` que `TenantContext::applyCachePrefix()` aplicaría, porque el motivo de la recomendación (b) es justo evitar la copia por tenant. Invalidación explícita (no sólo TTL) en cada escritura administrativa (`EloquentFeatureFlagAdministration::setState()`/`replaceRules()`) y en la retirada de un *flag* por `platform:sync-registry` (`SyncModuleRegistry::run()`). TTL configurable por `BO_FLAG_CACHE_TTL` (`config('core.feature_flags.cache_ttl_seconds')`, por defecto 300 s) — el nombre de la variable conserva el prefijo `BO_` tal como ya lo fija `operacion.md §4.3`, aunque la clase que la lee viva en `REQ-CORE`.

**`OPEN-BO-25` resuelta: sí, aplicada sin ADR nuevo**, tal como recomendaba la especificación. `FeatureFlagEvaluator`/`FeatureFlagExplainer` amplían la superficie pública de `REQ-CORE` (`App\Support\FeatureFlags`); `App\Modules\Core\Domain\FeatureFlagAdministration` es una interfaz adicional, de consumidor único (`REQ-BO`), no descrita con firma exacta por la especificación (§5.11.8 sólo fija las dos de evaluación) — se ha diseñado siguiendo el precedente literal de `ModuleContracting`/`ModuleCatalog` (mismo reparto lectura/escritura, mismo patrón de retorno en forma de *array* que `ModuleSubscriptionsService::previewBulk()`).

**Una desviación deliberada de la letra de `datos.md §9.3`**, señalada porque cambia un nombre de columna que la especificación fijaba explícitamente: `feature_flag_rules.affected_tenant_id` (no `tenant_id`) se resuelve por su clave interna (`tenants.id`) en el modelo de dominio (`FeatureFlagRuleInput::$tenantId`), **no** por su `public_id`, para que el motor de decisión compare enteros sin necesitar una carga perezosa de `tenants` por regla nominal en el camino de evaluación — el `public_id` se resuelve aparte, sólo para las respuestas administrativas (`serializeRule()`). No cambia ninguna regla de negocio ni ningún nombre de columna en base de datos.

**Nota de entorno, mismo género que la de `1.6d` (§15.4.1), confirmada y cerrada.** El `implementer` vio fallar 74 tests de `tests/Feature/Auth` (SAML/OIDC) en su *worktree* aislado sin poder confirmar la causa desde su *sandbox*. La sesión orquestadora lo verificó de forma independiente contra el contenedor de referencia `plataforma-api`, con las tres migraciones de este sub-paso ya aplicadas: **740/740 Pest en verde, incluido `tests/Feature/Auth` completo**. Confirma que era el mismo problema de entorno que `1.6d` (desincronización de `storage/framework/testing/saml-fake-idp/` entre el *worktree* y el contenedor de referencia) y no una regresión — **cero ficheros de `app/Modules/Auth` tocados por este sub-paso**.

**`OPEN-BO-27` · nueva, no bloquea** — `previewRules()`/`computeImpact()` no pueden reflejar una regla `percentage` cuando `rollout_unit: 'user'`, porque el cálculo de impacto evalúa por centro, sin usuario concreto, y `FeatureFlagDecisionEngine::exposure()` exige un `userPublicId` no nulo para resolver esa regla. Efecto: para cualquier *flag* con reparto por usuario, la vista previa (`RN-BO-68`) y el bloque `impact` de `PUT .../rules` siempre informan `by_rule.percentage: 0`, con independencia del porcentaje configurado, aunque en producción esa regla sí exponga a usuarios concretos (`isEnabled()` sí recibe el usuario real). No es fuga de datos ni entre tenants. Alternativas para cuando se retome: (a) muestrear una cohorte de usuarios por centro en el cálculo de impacto (coste no acotado, cuántos usuarios y con qué representatividad es una decisión de producto, no de motor); (b) documentar el límite en la respuesta misma (`impact.percentage_unreliable: true` cuando `rollout_unit === 'user'`) sin resolver el cálculo. Sin decidir.
