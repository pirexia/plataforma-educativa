...
| 12 | **Dos migraciones y no una**: las dos tablas con `tenants.early_adopter_since` (ya previstas, `datos.md §12` filas 6 y 7) **más la ampliación del `CHECK`** que el punto 4 descubre | `datos.md §12` |
...
Implementado en la rama `feature/REQ-BO-005-feature-flags`. Las siete rutas de `api.md §2.11`-`§2.13` más `GET /api/v1/feature-flags` (§2.14, en `REQ-CORE`), las dos migraciones (`feature_flags`/`feature_flag_rules` y la ampliación del `CHECK` de `admin_action_logs.action`), la columna `tenants.early_adopter_since`, las dos interfaces de `App\Support\FeatureFlags` (`FeatureFlagEvaluator`, `FeatureFlagExplainer`) implementadas por `EloquentFeatureFlagEvaluator` en `App\Modules\Core\Infrastructure`, y las dos capacidades nuevas (`flag.leer`, `flag.gestionar`). Verificado, no estimado: **399/399 Pest en verde** sobre `tests/Feature/Core`, `tests/Feature/Backoffice`, `tests/Feature/Tenancy`, `tests/Feature/Support` y `tests/Unit` (el barrido completo salvo `REQ-AUTH`/`REQ-PERM`, sin relación con este sub-paso — ver nota de entorno más abajo), Larastan (0 errores) y Pint limpios.
...
**Nota de entorno, mismo género que la de `1.6d` (§15.4.1)**: al ejecutar la suite completa (`vendor/bin/pest`, sin filtrar) en el contenedor de verificación de esta sesión, 74 tests de `tests/Feature/Auth` (SAML/OIDC) fallan por una colisión de datos de fixture entre ejecuciones sucesivas del mismo *worktree* aislado — no se ha podido confirmar si es exactamente la misma causa que la de `1.6d` (desincronización de `storage/framework/testing/saml-fake-idp/` entre el *worktree* y el contenedor de referencia `plataforma-api`) porque el acceso a la ruta del contenedor de referencia para comparar quedó fuera del sandbox de esta sesión. **Cero ficheros de `app/Modules/Auth` o de sus tests fueron tocados en este sub-paso** (verificado por `git status`), y el mismo fichero de test pasa 14/14 al ejecutarse sin modificar contra el contenedor de referencia `plataforma-api`. No se ha podido cerrar esta duda con la misma certeza que el resto de la verificación — se deja explícita para que la sesión orquestadora la confirme antes de mezclar, en vez de darla por buena en silencio.
