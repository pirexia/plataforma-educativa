...
| Estado | **APROBADA** — especificación completa aprobada por el usuario el 2026-09-08, con `ADR-046` aplicado (cierra las tres decisiones que bloqueaban el arranque), `ADR-047` aplicado (resuelve `OPEN-BO-10` y con ella el último visto bueno técnico pendiente; los dos puntos de decisión propia que trajo esa pasada —`reason` fuera del `GRANT`, `affected_tenant_id` `NOT NULL` en `tenant_lifecycle_events`— ratificados por el usuario el mismo día) y `OPEN-BO-13` resuelta (lectura (a), sin lista blanca que mantener). **Sin ningún bloqueante ni pendiente para `implementer`** (§15) |
