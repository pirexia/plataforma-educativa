...
#### 5.9.6 Tres hallazgos sobre privilegios, y sólo uno lo corrige `1.6d`

Los tres salen de leer el código para escribir esto, los tres son **anteriores** a este sub-paso y ninguno es suyo. Se escriben aquí —y se abren como issue— en vez de arreglarse de paso (`CLAUDE.md §5`, issue [#150](https://github.com/pirexia/plataforma-educativa/issues/150)).
...
> **Y una consecuencia operativa del hallazgo 1 que hay que tener presente aunque se arregle**: en cuanto esa purga funcione, **la ficha de salud sólo verá los fallos de las últimas 24 horas**, y `bo:retry-provisioning` sólo podrá reparar un aprovisionamiento durante ese mismo plazo. No es un defecto de `1.6d` —es la retención que el issue #73 fijó a propósito— pero sí es algo que el operador tiene que saber, y por eso está en `operacion.md §8` y no sólo aquí.
...
| **`1.6d`** · El operador busca en la ficha un fallo de hace tres días | **No está**: `queue:prune-failed --hours=24` lo borró (cuando esa purga funcione, §5.9.6). La retención de 24 horas la fijó el issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73) a propósito, y no es de este sub-paso cambiarla |
...
| `OPEN-BO-20` · ¿Cómo se atribuye a un centro un trabajo despachado por el backoffice? | **Abierta · nueva** (`1.6d`). **Nace de una contradicción entre esta especificación y `ADR-033 §8`**, verificada sobre el código, no de una duda de diseño. No bloquea `1.6d`, pero **sí condiciona lo que `1.6d` puede prometer** |
| `OPEN-BO-21` · ¿Es sensible el reintento de un trabajo fallido? | **Abierta · nueva** (`1.6d`). Cambia una fila de la lista cerrada de `api.md §4` y una celda de `permisos.md §4.5`. Recomendación: **sí** |
| `OPEN-BO-22` · ¿Se devuelve el **mensaje** de la excepción de un trabajo fallido? | **Abierta · nueva** (`1.6d`). Es una excepción consciente a `RN-BO-33` o un requisito vaciado; no hay tercera vía. Recomendación: **sí, devolverlo** |
| `OPEN-BO-23` · ¿Se adopta Horizon, o el observatorio de colas se queda sobre las tablas del *driver* `database`? | **Abierta · nueva** (`1.6d`), no bloqueante. Nace de que `CLAUDE.md §1` nombra Horizon en el *stack* y **no está instalado**. Recomendación: **no adoptarlo ahora** |
...
### `OPEN-BO-20` · ¿Cómo se atribuye a un centro un trabajo que despachó el backoffice? · **nueva en `1.6d`**
...
### `OPEN-BO-21` · ¿Es sensible el reintento de un trabajo fallido? · **nueva en `1.6d`**
...
### `OPEN-BO-22` · ¿El mensaje de la excepción de un trabajo fallido cruza al backoffice? · **nueva en `1.6d`**
...
### `OPEN-BO-23` · ¿Se adopta Horizon? · **nueva en `1.6d`**, no bloqueante
...
### 15.4 Sub-paso `1.6d` · salud y métricas de plataforma — **especificada, pendiente de aprobación**
...
| 9 | **Quince reglas nuevas** (`RN-BO-83` a `RN-BO-97`) y **diecisiete criterios nuevos** (`CA-BO-149` a `CA-BO-165`) | §7.6, §13.9 |
| 10 | **Ninguna tabla, columna, migración, índice, trabajo en cola ni tarea programada nueva.** Es el único de los cinco sub-pasos del que las seis cosas son ciertas a la vez, y el vocabulario que necesita (`job.reintentado`) **ya está en el `CHECK` desplegado** | `datos.md §14`, `operacion.md §6.1`/`§6.2` |
...
**Preguntas abiertas que deja, ninguna bloqueante para empezar**: `OPEN-BO-20` (atribución de trabajos de plataforma — **la única que cambia lo que la ficha puede prometer**, y la que conviene responder antes de escribir el *resource*), `OPEN-BO-21` (¿sensible el reintento? — una fila de `api.md §4`), `OPEN-BO-22` (¿el mensaje de la excepción? — una aserción de `CA-BO-150`) y `OPEN-BO-23` (¿Horizon? — no toca ni una línea de `1.6d`, sólo la tabla de *stack* de `CLAUDE.md`). **`OPEN-BO-04` y `OPEN-BO-06` siguen abiertas y siguen sin bloquear**: `1.6d` no añade ni una fila a `admin_action_logs` que afecte a la retención, salvo la del reintento.

**Dependencias de otros módulos: ninguna.** A diferencia de `1.6b` y `1.6c`, este sub-paso **no amplía la superficie pública de `REQ-CORE`** ni la de ningún otro módulo: consume `ModuleCatalog`, que `1.6c` ya declaró en `Core\Domain`, y lee tablas que ya existen. No hay `OPEN-BO` equivalente a `OPEN-BO-15`/`OPEN-BO-18` que plantear.

> **¿Se aprueba esta especificación de `1.6d` antes de pasar a implementación?** Y con ella, las cuatro preguntas de §14 y **la decisión de alcance del hallazgo `Alta`**: si el arreglo de `queue:prune-failed` entra en este sub-paso o va a un `fix/` propio.
