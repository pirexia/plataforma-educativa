...
### 15.3.1 Cierre de implementación (2026-09-15)

Implementado en la rama `feature/REQ-BO-002-matriz-modulos-spec`. `ModuleCatalog`/`ModuleContracting` viven en `Core\Domain`, implementados por `DeclaredModuleCatalog` (`Core\Infrastructure`) y `ModuleContractingService` (`Core\Application`), exactamente la lista cerrada de `OPEN-BO-18`. `ModuleSubscriptionsService` (`Backoffice\Application`) orquesta capacidades, reautenticación, doble autorización e `Idempotency-Key`. Migración de privilegios en `database/migrations/2026_09_15_100100_harden_module_subscriptions_platform_grants.php` (`REVOKE`/`GRANT` de `datos.md §7`, `§7.7` con `OPEN-BO-19` aplicada). Suite completa del módulo en verde (ver mensaje del commit de cierre para el número exacto).

Dos hallazgos propios de severidad Alta, detectados al implementar y corregidos en la misma rama:

1. **`TenantContext::runAsPlatform(BackofficeEscritura)` enmascaraba la excepción de negocio del `callback()` con su propio `RuntimeException` de cierre** cuando el bloque terminaba sin escribir en `admin_action_logs` — cierto también cuando el motivo de no escribir era una excepción legítima (esencial, retirado, dependencias sin confirmar, estado del tenant), no sólo un olvido. **Corrección revisada tras `doc-reviewer` (hallazgo Alta): la primera versión dejaba de invocar `after()` en el camino de excepción, revirtiendo `ADR-046 §6.5` sin ADR nuevo.** La versión final no reabre el ADR: `after()` se sigue llamando siempre ("también si el callback lanzó", tal cual), pero su excepción nunca sustituye a la del `callback()` — se reporta con `report()` y se propaga la original. `CA-BO-028` conserva su significado original sin necesidad de renumerar nada. De paso corrige un segundo hallazgo (issue [#218](https://github.com/pirexia/plataforma-educativa/issues/218), Media, `security-reviewer`): en el camino de éxito, si `after()` lanzaba, `platformMode` quedaba sin restaurar — ahora ese tramo también está en `try`/`finally`. Issue [#215](https://github.com/pirexia/plataforma-educativa/issues/215), con test de regresión en `PlatformAccessCheckTest.php`.
2. **La instrucción de reutilizar `RequireIdempotencyKey`/`IdempotencyKey` para `POST /module-rollouts` era técnicamente inviable**: esa primitiva es de tenant (`IdempotencyKey extends TenantModel`) y el backoffice nunca tiene tenant activo. Se creó la versión de plataforma (`platform_idempotency_keys`, `PlatformIdempotencyKey`, `RequirePlatformIdempotencyKey`), documentada en el issue [#216](https://github.com/pirexia/plataforma-educativa/issues/216).

Ninguno de los dos cambia una regla de negocio de `REQ-BO-002`; ambos corrigen infraestructura compartida (`App\Support\Tenancy`) o añaden una tabla de plataforma nueva siguiendo el patrón ya establecido (`TenantMigration::platformTable()`). Quedan anotados para que `doc-reviewer`/`security-reviewer` los revisen explícitamente y para que la sesión orquestadora valore si alguno merece un ADR propio.
