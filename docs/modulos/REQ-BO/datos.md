...
| ***Feature flags*** | **Cuatro que añade `1.6e`** (§4.2.2): `flag.estado_cambiado`, `flag.reglas_cambiadas`, `tenant.early_adopter_designado`, `tenant.early_adopter_retirado` |

#### 4.2.2 Los cuatro valores que añade `1.6e`, y por qué su ausencia era un defecto y no una omisión

**Este apartado sale de una comprobación de la pasada de `1.6e` y es el hallazgo de esquema del sub-paso.** `RN-BO-43` dice que **toda** escritura de *flags* deja entrada en `admin_action_logs`, `CA-BO-095` lo verifica, y `api.md §2.11` describe cuatro escrituras. Pero `action` tiene **vocabulario cerrado por `CHECK`** (§4.2) y **ninguno de los valores desplegados sirve para ninguna de las cuatro**: `modulo.contratado` habla de otra cosa y `tenant.actualizado` taparía la designación de *early adopter* exactamente como habría tapado el cambio de `slug` (§4.2.1).

**Sin esta migración, `RN-BO-43` no se puede cumplir: el motor rechaza la fila.** Y el modo de fallo es el peor posible — la escritura del *flag* y su auditoría van en la misma transacción, así que el `CHECK` haría fallar **la operación entera**, y el síntoma sería «no puedo apagar el *flag* que está rompiendo colegios». Se añaden por migración, con el precedente literal de §4.2.1 y del issue [#173](https://github.com/pirexia/plataforma-educativa/issues/173): `DROP CONSTRAINT` + `ADD CONSTRAINT` del mismo `CHECK` con cuatro valores más, **nunca** renombrando ni retirando los existentes.

| Valor | Cuándo | Por qué no vale uno existente |
|---|---|---|
| `flag.estado_cambiado` | `PUT /feature-flags/{key}/state`, en las dos direcciones | Es el freno de emergencia de `RNF-MANT-005`, y `operacion.md §7` lo vigila como **señal de calidad**: «cada vez que se usa, algo se ha roto en producción». Esa señal se cuenta sobre este valor; mezclarlo con el de las reglas la haría incontable |
| `flag.reglas_cambiadas` | `PUT /feature-flags/{key}/rules` | Una sola entrada por escritura, porque la escritura es del **conjunto** (`api.md §2.12` punto 1). El antes/después de las reglas va en `changes`, y el recuento de centros que pasan a estar expuestos, en `context` |
| `tenant.early_adopter_designado` | `PUT /tenants/{public_id}/early-adopter`, al marcar | **Lleva `affected_tenant_id`** y es visible para el propio centro, porque `action` está entre las seis columnas concedidas a `plataforma_app` (§4.3). Un centro tiene derecho a saber que se le metió en la cohorte que recibe las novedades antes que el resto |
| `tenant.early_adopter_retirado` | Ídem, al retirar | Ídem. Se separa del anterior en vez de codificar la dirección en `changes` por el mismo motivo que `tenant.suspendido` y `tenant.reactivado` son dos valores y no uno: la pregunta «¿cuándo se le sacó de la cohorte?» tiene que ser una consulta, no un análisis de JSON |

**Los dos primeros llevan `affected_tenant_id` sólo cuando la regla nombra a un centro** (`RN-BO-43`): una regla nominal de tenant lo pobla, y una regla `global`, de cohorte o de porcentaje lo deja **nulo**, porque afecta a todos y a ninguno en particular. Con la política `tenant_visibility` de §4.3, eso significa que **una escritura de alcance global no la ve ningún centro**, que es lo correcto: el mapa de a qué ritmo estamos desplegando el producto no es dato de cliente (`datos.md §9.6`).

**Y `subject_type` gana `feature_flag` sin migración**, porque ese vocabulario es **abierto y no tiene `CHECK`** (§4.1) — exactamente como `1.6d` añadió `failed_job` (§14). `subject_public_id` guarda el `public_id` ULID del *flag*, **no su `key`**, con independencia de lo que decida `OPEN-BO-11` sobre las rutas: la columna existe para sobrevivir al borrado del sujeto y el ULID es lo que nunca cambia.
...
### 9.7 Lo que la pasada del sub-paso `1.6e` verifica, y las dos cosas que cambia

**Verificado el 2026-09-21 sobre `develop`, y no supuesto.** El estado de partida es el más limpio de los cinco sub-pasos y conviene decirlo, porque `1.6c` enseñó que lo razonable de suponer no siempre se cumple (§7.5):

| Qué | Estado real |
|---|---|
| `feature_flags`, `feature_flag_rules`, cualquier modelo, comando o configuración de *flags* | **No existe nada.** `feature_flag`, `FeatureFlag`, `pennant`, `Unleash` y `Flagsmith` dan **cero resultados** en `apps/api`, en `composer.json` y en `package.json`. La nota de §9 sigue siendo literalmente cierta tres pasos después |
| `tenants.early_adopter_since` | **No existe.** La fila 7 de §12 sigue en pie tal cual |
| Las cuatro columnas de `1.6b` en `tenants` | **Existen** (`2026_09_11_100100`): `suspension_message`, `suspended_at`, `grace_period_ends_at`, `grace_period_expired_at`. **`early_adopter_since` es la quinta y la única que falta** (§6) |
| `ModuleDescriptor` | **Existe**, en `App\Modules\Core\Domain`, con `code`, `nameKey`, `phase`, `dependsOn` y `essential`, y una factoría `fromArray()`. **Gana `featureFlags` con valor por defecto `[]`**: es aditivo puro, **no es una migración** y ningún descriptor existente hay que tocarlo |
| `SyncModuleRegistry` | **Existe**, en `App\Support\Modules`, y su forma es **exactamente** la que §9.1 necesita: una **fase de validación previa que aborta con `InvalidArgumentException` sin escribir nada**, una fase de materialización con `updateOrInsert` por `pgsql_owner`, y una fase que marca `retired_at` sobre lo no declarado **sin borrar nunca**. El catálogo de *flags* es un tercer bloque dentro de esas mismas tres fases, no un comando nuevo (`RN-BO-34`) |
| `PlatformCapability` | **Existe** con veinticuatro casos y **ninguno de `flag`** (`permisos.md §4.6`) |
| El `CHECK` de `admin_action_logs.action` | **Existe y no admite ningún valor `flag.*`.** Es el hallazgo de esquema de este sub-paso: §4.2.2 y la fila 6b de §12 |

**Lo primero que cambia: la fila 6b de §12**, ya explicada en §4.2.2. **Lo segundo es una precisión sobre cómo se escribe `rules_version`**, que no estaba dicha y que decide si el mecanismo de §9.5 funciona:

> **`rules_version` se incrementa siempre con una expresión SQL —`SET rules_version = rules_version + 1`— y nunca leyendo el valor en PHP para escribirlo después.** Tiene **dos escritores por dos conexiones distintas**: el backoffice por `plataforma_platform`, que tiene concedido el `UPDATE` de esa columna (§9.6), y `platform:sync-registry` por `plataforma_owner`, que la incrementa al marcar `retired_at` (`RN-BO-103`). Con lectura-y-escritura en PHP, dos escrituras concurrentes producen **un solo incremento**, y la consecuencia no es un número mal contado: **es que las entradas de caché de la versión anterior siguen siendo alcanzables** y el *flag* que alguien acaba de apagar sigue encendido hasta que caduque el TTL. Es el mismo género de fallo que los issues [#205](https://github.com/pirexia/plataforma-educativa/issues/205), [#207](https://github.com/pirexia/plataforma-educativa/issues/207) y [#209](https://github.com/pirexia/plataforma-educativa/issues/209) encontraron en `1.6b`, **con una solución distinta y más barata**: allí hacía falta `lockForUpdate()` porque la decisión dependía del estado leído; aquí no hay decisión que tomar, sólo un contador que subir, y el motor ya sabe hacerlo de forma atómica. **Un `lockForUpdate()` aquí sería serializar sin motivo** el camino de escritura de *flags*.

**Y una tercera comprobación que sí puso en duda §9.5 y quedó resuelta a su favor**: esta pasada encontró que **el almacén de caché configurado no era Redis** sino el de respaldo, `database` (`funcional.md §5.11.9.1`), lo que habría convertido la caché de evaluación en una consulta SQL por lectura. **El usuario lo resolvió el 2026-09-21 fijando `CACHE_STORE=redis`** para el *runtime* (`OPEN-BO-26`, issue [#236](https://github.com/pirexia/plataforma-educativa/issues/236)): **§9.5 queda vigente exactamente como estaba escrita**, y la caché versionada se construye sobre el almacén que asumía. Es la corrección de un supuesto, no de un diseño.

**Lo que no cambia, y conviene decirlo porque es la mitad del valor de esta sección**: §9.2, §9.3, §9.3.1, §9.4, §9.5 y §9.6 se mantienen **sin una sola corrección**. Las dos tablas, sus `CHECK`, sus tres índices únicos parciales, el nombre `affected_tenant_id` sin la política de `ADR-047`, la ausencia de tabla de asignaciones y el reparto de privilegios entre los tres roles siguen siendo lo que hay que implementar. La única salvedad es **`OPEN-BO-24`** (`funcional.md §14`), que no cuestiona §9.5 sino que le señala un hueco: la clave de caché lleva `rules_version`, y leer `rules_version` es ya una consulta.
...
| **6b** | **`1.6e`** | **El `CHECK` de `admin_action_logs.action` gana cuatro valores**: `flag.estado_cambiado`, `flag.reglas_cambiadas`, `tenant.early_adopter_designado`, `tenant.early_adopter_retirado` (§4.2.2) | Toma bloqueo breve de catálogo | **No estaba en la revisión original de esta tabla, y su ausencia no era inofensiva** (§4.2.2): sin ella `RN-BO-43` no se puede cumplir, porque la auditoría va en la misma transacción que la escritura del *flag* y el `CHECK` haría fallar la operación entera. `DROP CONSTRAINT` + `ADD CONSTRAINT` del mismo `CHECK`, **nunca** renombrando ni retirando valores; precedente literal en la fila 4b y en la migración del issue [#173](https://github.com/pirexia/plataforma-educativa/issues/173). **Mismo aviso que allí**: la tabla es *append-only* bajo `FORCE` sin política de escritura, así que esta migración **puede** ampliar el `CHECK` y **no podría** rellenar ninguna columna sobre las filas existentes (`ADR-047 §5.2`). Verificada por `CA-BO-174` |
