...
- **`SendPasswordResetEmail` y `SendAccountLockedEmail` implementan `ShouldBeEncrypted`** (issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73)): el token en claro que llevan en su *payload* viaja y se almacena cifrado con `APP_KEY`, tanto en la cola normal como si el trabajo agota sus 5 reintentos y cae en `failed_jobs`. `SendInvitationEmail` de 1.1 comparte el mismo patrón de "token en el *payload*" pero **no** implementa `ShouldBeEncrypted` todavía — revisar si corresponde ampliarlo ahí también. La segunda capa —no conservar el *payload* fallido más de un día, ni siquiera cifrado— **es de `REQ-BO`, no de este módulo** (`bo:purge-failed-jobs`, `docs/modulos/REQ-BO/operacion.md §6.2`): `failed_jobs` es tabla de plataforma, y el comando del framework (`queue:prune-failed`) llevaba desde `0.7` sin poder borrar nada, por apuntar a la conexión sin privilegio de `DELETE` (`REQ-BO-004`, sub-paso `1.6d`, issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73) corregido de verdad el 2026-09-21).
...
- **Los dos de correo llevan el código en el *payload* y por tanto implementan `ShouldBeEncrypted`** (issue [#73](https://github.com/pirexia/plataforma-educativa/issues/73)), igual que `SendPasswordResetEmail` y `SendAccountLockedEmail`. La segunda capa (`bo:purge-failed-jobs`, `REQ-BO`) sigue siendo la que limita la retención.
...
**El fallo de la migración 1 al revertir no es atómico, y hay que saberlo antes de intentarlo** (hallazgo de `db-reviewer`, punto de control 1, verificado ejecutándolo contra una fila SAML de prueba). `$withinTransaction = false` es correcto y necesario para el patrón `NOT VALID`/`VALIDATE CONSTRAINT` de `up()` (`datos.md §G.7.1`), pero tiene un coste en `down()`: cada sentencia se compromete por separado, así que un intento de reversión con una fila SAML presente **deja la base de datos en un estado intermedio** en el momento en que falla el primer `ALTER COLUMN … SET NOT NULL` — ya se habrán retirado los nueve `CHECK` nuevos, restaurado los cuatro antiguos y devuelto los tres `DEFAULT`, pero `protocol` y la nulabilidad de las siete columnas seguirán como estaban. **La recuperación es sencilla y no requiere intervención manual de esquema**: `up()` es idempotente (usa `hasColumn()` y `DROP CONSTRAINT IF EXISTS` en cada paso), así que volver a ejecutar `php artisan migrate` sobre esta misma migración restaura el estado correcto. No se intente “completar a mano” la reversión rellenando las columnas OIDC de la fila SAML: es exactamente el `UPDATE` de conveniencia que `datos.md §G.7.3` rechaza a propósito.
