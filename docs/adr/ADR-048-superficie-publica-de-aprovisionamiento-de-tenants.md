...
2. **Resuelto al cerrar `1.6b`**: el idioma de la invitación del primer administrador ya no es literal — `createAdministrator()` recibe `$locale` (el del centro) en `provision()` y `provisionFromTemplate()`.
3. **Resuelto al cerrar `1.6b` (2026-09-14)**: `docs/modulos/REQ-CORE/funcional.md §7` ya incluye `TenantProvisioner` como sexta interfaz pública, y `§4.7` ya documenta las cinco opciones nuevas de `tenant:provision-defaults`.
