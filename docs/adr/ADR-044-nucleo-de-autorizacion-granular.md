...
- **Un test de arquitectura (`arch()`) existe** (`apps/api/tests/Feature/Auth/PasswordResetTokenArchitectureTest.php`, Pest `pestphp/pest-plugin-arch`), corrección sobre lo escrito en la evaluación original de este ADR: no son "cero", pero sigue siendo un patrón aislado, no una práctica establecida — el issue [#163](https://github.com/pirexia/plataforma-educativa/issues/163) sigue vigente y este test es ahora su precedente de forma a copiar (hallazgo de `spec-writer`, 2026-09-04, verificado).
