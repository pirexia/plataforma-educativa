...
- **Consultar** los módulos contratados por la plataforma para su centro, **recibir aviso de las nuevas altas** y **configurar** los que estén contratados (`module_subscriptions.settings`). **No puede conmutarlos**: contratar y descontratar es potestad única del Super Administrador desde el backoffice (`ADR-045 §4.1`, `RMOD-002`). El aviso de disponibilidad al contratar es informativo y sin acción requerida: el módulo queda operativo desde ese mismo instante (`ADR-045 §12.2`).
...
- **Consumidor declarado desde `ADR-045 §4.8`**: un *listener* del evento de dominio `ModuleContracted` (emitido por `REQ-CORE` desde el paso 1.6) crea la notificación in-app de «ya tienes disponible este módulo», dirigida a los usuarios del tenant que tengan el permiso **`modulo.leer`** — resuelto **por permiso y nunca por una lista de códigos de rol escrita a mano**, para no dejar fuera a los roles personalizados que cree el centro. Hasta que este módulo exista, el aviso se deriva de `module_subscriptions.enabled_at` en el propio panel del centro, sin infraestructura de notificaciones.
...
- **Sobre qué se factura**: la unidad facturable de un módulo es `module_subscriptions.enabled` para ese tenant — la única potestad sobre el interruptor y, por tanto, el único dato con significado comercial (`ADR-045 §4.6`). No existe ninguna otra columna que represente «el centro lo tiene encendido»; lo contratado es lo facturado y lo facturado es lo utilizable.
...
> Esta pantalla es el **único** punto de escritura de `module_subscriptions.enabled` en todo el producto (`ADR-045`).

- Matriz visual **tenant × módulo**. Cada celda distingue **tres** situaciones y no más (`ADR-045 §4.7`):
  1. **No contratado** — sin fila de suscripción, o `enabled = false`. No utilizable: `RMOD-009` responde `403`.
  2. **Contratado** — utilizable por el centro desde ese mismo instante, sin estado intermedio de aceptación (`ADR-045 §12.2`).
  3. **Módulo esencial** — celda bloqueada, sin conmutador. Los módulos esenciales se **declaran en el descriptor del módulo** (`essential`), no en una lista escrita a mano dentro del código de infraestructura (`ADR-045 §4.9`).
- **Contratar y descontratar** individualmente, con motivo obligatorio, resolución automática de dependencias y aviso previo de los efectos (`RMOD-006`).
- **Contratar dispara un aviso al Administrador de Centro** de que el módulo está disponible, informativo y sin acción requerida por su parte (`ADR-045 §4.8`, `REQ-COM-003`).
- Descontratar siempre en modo *soft-disable*: los datos se preservan y volver a contratar los restituye (`RMOD-003`, `RMOD-004`).
- Vista previa del impacto antes de aplicar: usuarios afectados, pantallas que desaparecen, integraciones que dejan de funcionar y **dependencias arrastradas**.
- Activación masiva sobre varios tenants a la vez (por ejemplo, al liberar un módulo nuevo a todos los clientes de un plan), con **un aviso por cada centro afectado**, nunca uno global, y con vista previa que diga en número cuántos centros pasan a tenerlo y cuántas dependencias arrastra.
...
| `RMOD-002` | El Super Admin **contrata y descontrata** módulos por tenant desde el backoffice (`REQ-BO-002`). Es la **única** potestad sobre ese interruptor: el Administrador de Centro no conmuta módulos, ni para encender ni para apagar (`ADR-045 §4.1`). `module_subscriptions.enabled` significa «contratado por la plataforma **y por tanto** utilizable por el centro»; lo escribe únicamente la conexión de plataforma (`ADR-033 §5`), con motivo obligatorio. |
| `RMOD-003` | **Descontratar** no afecta a los datos existentes (*soft-disable*: los datos se preservan íntegros, no se borra ni se anonimiza nada). |
| `RMOD-004` | **Volver a contratar** restituye el acceso a los datos históricos tal como estaban, sin migración ni reconstrucción. |
| `RMOD-005` | Los módulos no contratados no consumen recursos de computación (sin jobs programados ni listeners activos). |
| `RMOD-006` | Dependencias entre módulos gestionadas automáticamente, **como invariante del único punto de escritura** (el backoffice), no como capa de la lectura de disponibilidad (`ADR-045 §4.5`): no se contrata un módulo sin sus dependencias —el backoffice las resuelve y las muestra en la vista previa de impacto— ni se descontrata una dependencia de un módulo contratado sin aviso explícito y confirmación. Ninguna escritura puede dejar un módulo contratado cuya dependencia no lo esté. Las dependencias se **declaran en el código** (`depends_on` en el descriptor del módulo, `ADR-034 §5`); `platform:sync-registry` valida en el despliegue que los códigos referenciados existen y que el grafo no tiene ciclos, y **aborta el despliegue** si no es así. Ante una arista nueva que deje centros incoherentes, el comando **informa y no corrige**: contratar automáticamente sería tomar una decisión comercial facturable sobre todos los centros a la vez. |
| `RMOD-007` | Precios de módulos configurables por plan de suscripción (`REQ-SAAS-001`). **Se factura sobre `module_subscriptions.enabled`**, que es la única potestad y por tanto la única columna con significado comercial: lo contratado es lo facturado y lo facturado es lo utilizable (`ADR-045 §4.6`). |
| `RMOD-008` | Módulos ocultos en la interfaz cuando no están contratados (sin enlaces muertos). |
| `RMOD-009` | Las APIs de módulos no contratados responden 403 con mensaje informativo. **Un solo código de error** (`urn:pge:error:module-disabled`) porque hay una sola causa posible, y el mensaje no revela plan, precio ni condiciones comerciales (`ADR-045 §4.10`). |
| `RMOD-010` | Los eventos de dominio emitidos por un módulo no contratado simplemente no se emiten; ningún otro módulo debe fallar por ello. **Regla derivada** (`ADR-045 §4.8`): **todo evento del ciclo de vida de un módulo pertenece a `REQ-CORE`, nunca al módulo afectado** — `ModuleContracted` y `ModuleDecontracted` los emite `REQ-CORE`, que es esencial y siempre está activo. Si pertenecieran al módulo afectado, `ModuleDecontracted` no podría emitirse nunca, porque el módulo ya estaría apagado cuando tocara emitirlo. |
...
| `ADR-045` | Contratación de módulos por tenant: **una sola potestad, la del Super Admin, con aviso al centro** (**resuelve el issue [#44](https://github.com/pirexia/plataforma-educativa/issues/44) y `OPEN-CORE-03`**; **amplía `ADR-034 §5`**, que fijó una sola columna `enabled` arrastrando en su propio texto la contradicción, y honra literalmente la cláusula de indiferencia de `ADR-044 §4.9`). `REQ-CORE-002` y `RMOD-002` dejan de contradecirse **eliminando uno de los dos actores, no repartiendo el dato**: `module_subscriptions.enabled` significa «contratado por la plataforma = utilizable», lo escribe **solo** el Super Admin por `plataforma_platform`, y el Administrador de Centro **conserva únicamente consultar (`modulo.leer`) y configurar `settings` (`modulo.actualizar`)** — sin conmutar, ni para encender ni para apagar. **Coste de esquema cero**: no se añade, renombra ni elimina ninguna columna (**se descarta el renombrado `enabled`→`contracted`** al desaparecer el motivo), luego **no hay ciclo expand/contract**. Lo único que cambia en base de datos es una **migración de privilegios** (`REVOKE UPDATE, INSERT … FROM plataforma_app` + `GRANT UPDATE (settings, …)`, precedente de `harden_failed_jobs_grants`): que un centro no pueda contratarse un módulo deja de ser un `if` en `ModulesController` y pasa a garantizarlo el motor (`INV-001`). Al contratar, **`REQ-CORE` emite `ModuleContracted`/`ModuleDecontracted`** y el panel del centro deriva el aviso de `enabled_at` **sin construir infraestructura de notificaciones** — la notificación in-app real es un listener de `REQ-COM-003` en **1.19**, dirigido por el permiso `modulo.leer` y nunca por una lista de códigos de rol; queda zanjado que **todo evento del ciclo de vida de un módulo pertenece a `REQ-CORE`, no al módulo afectado** (compatibilidad con `RMOD-010`). `RMOD-006` se aplica como **invariante en el único punto de escritura** y gana por fin sitio donde declararse (`depends_on` en `moduleDescriptor()`, con validación de códigos y ciclos que **aborta el despliegue**); `RMOD-009` no cambia (una sola causa, un solo código de error); se **factura sobre `enabled`** (`RMOD-007`/`REQ-SAAS-001`); los módulos esenciales (`core`, `auth`) se **declaran** en el descriptor en vez de la lista escrita a mano de `EloquentModuleAvailability`. **Descarta** las dos columnas con `AND` —que era la recomendación de `architect`, no seguida (`§12.1`)—, la potestad exclusiva del centro, las dos tablas, el tercer estado «pendiente de aceptación» (el módulo es **efectivo desde que se contrata**) y adelantar `REQ-COM-003` a 1.6. `§8.3` convierte en **obligatoria para 1.6** la invalidación de caché que `ADR-034 §5` exigía y nunca se implementó, con el agravante del prefijo de tenant escrito desde fuera del contexto del tenant. Entrada obligatoria de `spec-writer` para **1.6** (`REQ-BO`); `§10` lista la reescritura pendiente de diez requisitos y `§12` registra las dos decisiones del usuario, una de ellas contra la recomendación de `architect` |
| `ADR-046` | Separación del backoffice de plataforma: superficie, sesión y propósito declarado en `runAsPlatform()` (**resuelve `OPEN-BO-01`, `OPEN-BO-02` y `OPEN-BO-03`**, los tres bloqueantes de `1.6`, y **cierra los puntos 2 y 3 del issue [#6](https://github.com/pirexia/plataforma-educativa/issues/6)**; **no toca `ADR-002`**, que sigue vigente). **`OPEN-BO-01`**: se adopta la **Opción A** —mismo monolito, *guard* `platform`, grupo de rutas `/api/platform/v1` con pila propia, SPA independiente `apps/backoffice`— y **se descarta la aplicación desplegable aparte**, no por coste sino porque **haría inejecutable `RN-BO-22`/`RMOD-006`**: `ADR-045` exige una sola implementación de las dependencias de módulos en un servicio de dominio de `REQ-CORE`, y un segundo despliegue la duplica o inventa un contrato entre servicios. Cinco condiciones **vinculantes**, no recomendaciones: Traefik enruta por **`Host()`** (hoy solo por `PathPrefix`, de modo que `centroa.dominio/api/platform/…` alcanzaría la misma API — sin esto la Opción A **no cumple el requisito**), lista blanca de IP también en el *ingress* con el aviso de `forwardedHeaders.trustedIPs`, *middleware* `RequirePlatformHost` que responde `404`, **el *host* del backoffice no puede ser subdominio de `TENANCY_BASE_DOMAIN`** (`TenantHost::slugFrom()` lo resolvería como *slug*) con rechazo del *slug* colisionante al alta de tenant, y cuatro aserciones de arquitectura sobre `Route::getRoutes()`. **`OPEN-BO-02`**: se **corrige la premisa** de la especificación —`sessions` **no tiene** `tenant_id` ni RLS, está en `shared_tables.framework` y su endurecimiento es el issue [#81](https://github.com/pirexia/plataforma-educativa/issues/81)—, así que la opción de «columna anulable» no existía; la sesión de plataforma vive en **`platform_sessions`**, tabla de plataforma con **`REVOKE ALL … FROM plataforma_app`** (precedente `harden_failed_jobs_grants`), cookie y vida propias, porque en el *driver* `database` el identificador de sesión **es** la clave primaria y compartir tabla dejaría sesiones de administrador de plataforma legibles por el *runtime* de los centros. `sessions` **no se toca**. **`OPEN-BO-03`**: `runAsPlatform(PlatformAccessPurpose $purpose, Closure $callback)` con propósito **obligatorio, sin valor por defecto y literal**, tres casos (`Mantenimiento` solo bajo consola y sin auditar; `BackofficeLectura`; `BackofficeEscritura`, que **lanza al cerrar el bloque si no dejó entrada en `admin_action_logs`**), comprobación delegada en el contrato `PlatformAccessCheck` cuyo enlace por defecto **deniega los propósitos de backoffice** (`INV-002`, `INV-007`), y **prohibición de modo plataforma con tenant activo** — la combinación que `AuditRecorder` y `BelongsToTenant` documentan hoy como peligro latente y que ningún llamador actual usa. `AuditRecorder` conserva su excepción bajo `Mantenimiento` y calla bajo propósito de backoffice, cuyo rastro es `admin_action_logs`. **Descarta** auditar cada llamada y comprobar un permiso genérico dentro de la primitiva (las dos lecturas literales del issue #6, inaplicables a sus dos únicos llamadores, ambos sin sujeto), el propósito único, la bandera booleana y el propósito opcional. Entrada obligatoria de `spec-writer` para **1.6**; `§10` lista lo que hay que rehacer en `docs/modulos/REQ-BO/` y `§11` los cinco hallazgos reportados y no corregidos |
| `ADR-047` | Tablas de plataforma con visibilidad por tenant afectado (**resuelve `OPEN-BO-10`**, la puerta que la especificación de `1.6` puso antes de su primera migración; **amplía `ADR-033 §7` y `§10` sin sustituirlos**, mismo mecanismo con el que lo hizo `ADR-034`). **Estado `ACEPTADA`** (2026-09-08, aprobación explícita del usuario, incluida la decisión de `§5.1`). Añade una **quinta categoría** a la taxonomía de tablas: la fila **no pertenece** a ningún centro pero **registra un hecho que le concierne**, y `REQ-BO-007` obliga a que el centro pueda consultarlo. Cuatro reglas canónicas: la columna se llama **`affected_tenant_id`, nunca `tenant_id`** —nombre reservado a la columna de propiedad con `DEFAULT app.current_tenant_id()` y política a dos sentidos—; política **`tenant_visibility` solo `FOR SELECT`**, sin `WITH CHECK` y sin `OR … IS NULL` (fallo en cerrado en los dos sentidos: filas de alcance global y peticiones sin tenant no devuelven nada); privilegios con **`REVOKE ALL` sobre tabla *y secuencia*** más **`GRANT SELECT` de columnas enumeradas, nunca de tabla completa** —RLS filtra filas, no columnas, y un `GRANT` de tabla expondría al *runtime* de un centro el motivo interno, el nombre, la IP y el agente de usuario del personal del proveedor—; y `REVOKE UPDATE, DELETE` sobre `plataforma_platform` como capa adicional, **dejando escrito que lo que de verdad cierra la escritura es `FORCE ROW LEVEL SECURITY` sin política permisiva** (un `REVOKE` de rol lo puede revertir el propietario). La convención de nombres **no es cosmética**: los tests de esquema #8 y `SchemaInvariantsTest` definen «tabla de tenant» por el nombre literal de la columna, de modo que `tenant_lifecycle_events` y `feature_flag_rules` tal como estaban especificadas **rompían el *build***, y `tenant_lifecycle_events` además escapaba sin que se verificase su declaración en `shared_tables.platform`. **Los tests no se tocan y queda prohibido darles listas de exenciones.** Estas tablas **no usan `TenantMigration::tenantTable()`** y quedan **fuera del ciclo expand/contract para siempre** (`§5.2`): bajo `FORCE` sin política de escritura, ni el rol de migraciones puede rellenar una columna nueva — por eso los nombres se deciden antes de la primera migración y no después. `§5.1` decide, **contra lo encargado y con verificación en el código**, que `platform_admin_sessions.session_id` **no lleva clave foránea**: la fila de `platform_sessions` no existe todavía en el instante del `INSERT` (el *driver* la escribe al final de la petición), luego la FK fallaría en cada inicio de sesión; se sustituye por un barrido de huérfanas con el precedente de `CloseOrphanedUserSessions`, que además escribe el `end_reason = 'caducidad'` que hoy no tiene escritor. **Descarta** las listas de exenciones en los tests, `subject_tenant_id` como nombre, el `GRANT SELECT` de tabla completa, confiar la restricción de columnas solo al *endpoint*, dar `tenant_id` real y `tenant_isolation` a `admin_action_logs`, y adoptar la categoría para `feature_flag_rules`. Vinculante para los **53 módulos**; `§11` registra cinco hallazgos reportados y no corregidos |
| `ADR-048` | Superficie pública de aprovisionamiento de tenants: **contrato síncrono en `REQ-CORE`, no evento de `REQ-BO`** (**resuelve `OPEN-BO-15`**; concreta `INV-007` e `INV-012`; se apoya en `ADR-045 §4.8` y `ADR-046 §7.1`, **sin sustituir ningún ADR anterior**). **Estado: `ACEPTADA`**, mecanismo y permiso (decisión de `architect` sobre el mecanismo, 2026-09-11, sobre el código real y no sobre las dos opciones planteadas a ciegas; ratificación del usuario sobre el permiso de `§10` —que `1.6b` escriba código dentro de `REQ-CORE`—, 2026-09-14). `REQ-CORE` declara en su `Domain` el contrato **`TenantProvisioner`** con dos métodos —`provision()` para el alta y **`provisionFromTemplate()` para la clonación**— más dos objetos de valor (`TenantInitialSettings`, `TenantAdministrator`) y el enumerado de resultado `TenantProvisioningOutcome`, enlazado a la clase que ya existe (`ProvisionTenantDefaults`, que **no se mueve ni se renombra**, `§4.4`). **Descarta el evento de dominio** por tres motivos de fondo y no de estilo: no devuelve resultado ni propaga fallo, y `funcional.md §5.3.3`/`§5.3.5` exigen las dos cosas para escribir la transición `en_alta`→`activo` o `tenant.aprovisionamiento_fallido`; el reintento obligaría a **reemitir un hecho falso**; y pondría el vocabulario de `REQ-CORE` (idiomas, moneda, CCAA) dentro de una clase de `Backoffice`, que es `INV-007` incumplida del revés. Al leer el código **encuentra un segundo incumplimiento de `INV-007` que la especificación no había nombrado**: la clonación de `REQ-BO/funcional.md §5.6` copia `tenant_settings`, `roles` y `permission_role` —tres tablas de `REQ-CORE`— y obligaba al trabajo `CloneTenant` a leerlas y escribirlas desde `REQ-BO`; por eso el contrato tiene dos métodos y no uno. **`module_subscriptions` queda fuera del contrato** y lo sigue copiando el backoffice: meterla dentro revocaría `ADR-045 §4.1` sin ADR que lo sustituya — la frontera resultante es «`REQ-CORE` es dueño de lo que el centro **es**; `REQ-BO`, de lo que el proveedor le ha **vendido**». **Regla general derivada, vinculante para los 53 módulos** (`§4.9`): cuando un módulo necesita que otro ejecute una operación cuyo resultado o cuyo fallo le importan, se hace por **contrato síncrono en el `Domain` del módulo propietario**; el evento de dominio queda reservado a **notificar hechos consumados** y nunca lleva de vuelta un resultado — la asincronía la pone el llamador con su propio trabajo en cola, no el contrato. **Descarta** también el bus de comandos genérico, invocar el comando de consola (acoplamiento por cadenas, actor de auditoría fijado en `console`), emitir `TenantProvisioned` «para dejar el enganche puesto» (tercera fuente de verdad de un hecho que ya registran `tenant_lifecycle_events` y `admin_action_logs`), un caso `Failed` en el enumerado en vez de excepción, y renombrar la implementación a `Eloquent*`. `§5.1` corrige un defecto real verificado desde `1.1`: el primer administrador se creaba con `locale` literal `es-ES`, de modo que **la invitación de un centro alemán salía siempre en español**. `§11` registra cuatro hallazgos reportados y no corregidos, uno de ellos de severidad Media que afecta también a `1.1` (correo de invitación despachado dentro de la transacción con `after_commit => false`). Requisito previo de la implementación del sub-paso **`1.6b`**; fija el patrón para **`1.6c`** y **`1.24`** (`REQ-ONB`) |
| `ADR-049` | Segunda opinión de **Codex** sobre código ya implementado, mediante el plugin oficial `openai/codex-plugin-cc` (Apache-2.0): **solo lectura, sin autoridad de bloqueo y con prueba acotada y reversible** (**cierra la comprobación de `CLAUDE.md §1` para esa herramienta**, verificada contra la API de GitHub el 2026-09-14: 7 *releases* entre 2026-03-30 y 2026-07-08, **68 días sin *commits*** y **499 *issues* abiertas** — datos que se aceptan por el radio de daño, no porque sean buenos; se apoya en `ADR-030` y `ADR-037`, **sin sustituir ningún ADR anterior**). **No afecta a ninguna línea de código entregado**: el plugin no entra en `composer.json` ni en `package.json`, no se despliega y ningún artefacto del producto depende de él; lo que cambia es el **pipeline de revisión** y `.claude/settings.json`, que está versionado. Ratifica las tres decisiones del usuario (2026-09-14): capa gratuita de ChatGPT para una prueba acotada aceptando la cláusula de entrenamiento, **alcance de solo lectura** (`/codex:review`, `/codex:adversarial-review`, `/codex:status`, `/codex:result`, `/codex:cancel`; **prohibidos `/codex:rescue` y `/codex:transfer`** por el riesgo del issue [#150](https://github.com/pirexia/plataforma-educativa/issues/150) sin *worktree* aislado que lo contenga) y ámbito de instalación `project`. Decide lo que faltaba: **el *sandbox* de Codex restringe escritura y red, no lectura, y la lista `deny` de `.claude/settings.json` (`Read(./.env)`, `Read(./**/*.key)`…) NO se puede replicar en él** — la protección se obtiene en tres capas, y la tercera no la da la herramienta: la lista `deny` pasa a ser el **filtro de lo que sale de la máquina** (el *hook* `SessionStart` entrega a Codex la ruta del **transcript**, luego lo que sale no está acotado al *diff*), `sandbox_mode = "read-only"` + `approval_policy = "never"` cierran escritura y **puerta de escalada** (fallo en cerrado: `"untrusted"` se descarta porque delega la protección en un humano que debe decir que no cuarenta veces), y como condición previa **se auditan los tres `.env` reales del árbol** —que hoy contienen `APP_KEY`, cuatro contraseñas de PostgreSQL, `REDIS_PASSWORD`, `MAIL_PASSWORD`, `AWS_SECRET_ACCESS_KEY` y las credenciales de MinIO— para confirmar que ninguna credencial es válida fuera de la máquina de desarrollo. **La puerta de revisión (`Stop`) permanece desactivada como norma**, por consumo y porque armada supeditaría `CLAUDE.md §10` al veredicto de un tercero. **Zanja qué significa `RNF-MANT-007` para una herramienta que no es una librería**: la interfaz propia es el **protocolo de uso documentado en una *skill***, no código PHP/TS — regla general derivada para toda herramienta de desarrollo futura. **Fija el sitio en el pipeline como norma y no como sugerencia**: se invoca **después** de `implementer`, con la suite en verde y el trabajo commiteado, **antes o en paralelo** a `db-reviewer`/`security-reviewer`/`doc-reviewer` y **nunca en lugar de ninguno**; su veredicto **no tiene autoridad de bloqueo** y cada hallazgo entra por la tabla de severidad de `CLAUDE.md §5` como cualquier otro, con los descartes anotados con su motivo — y **un hallazgo que contradiga un ADR vigente no es un hallazgo, es ruido**, y se cierra citando el ADR (Codex no ha leído ninguno). **Criterio de reversión medido y no de sensación** (`§8`): calibrado sobre el *diff* ya cerrado de `1.6b` (PR [#204](https://github.com/pirexia/plataforma-educativa/pull/204)) con cuatro hallazgos reales encontrables desde el *diff* ([#200](https://github.com/pirexia/plataforma-educativa/issues/200), [#201](https://github.com/pirexia/plataforma-educativa/issues/201), [#202](https://github.com/pirexia/plataforma-educativa/issues/202), [#203](https://github.com/pirexia/plataforma-educativa/issues/203)) y tres umbrales simultáneos —cobertura ≥ 2 de 4, precisión ≥ 50 %, y **al menos un hallazgo que ninguno de los tres revisores existentes hubiera producido**—, con **reversión por defecto** si al cerrar el segundo paso de prueba no hay evaluación escrita. `§9` fija el procedimiento de reversión (seis pasos, verificado con `git diff` sobre `.claude/settings.json`) y `§5.6` una **cláusula de caducidad automática**: la prueba termina en el momento en que exista un dato personal real. `§15` registra cuatro hallazgos reportados y no corregidos, uno de ellos de proceso: **el plugin ya estaba instalado en `.claude/settings.json` cuando se escribió este ADR**, adelantándose a la aceptación de la decisión que lo autoriza y a las dos verificaciones previas de `§14.2` |
| `ADR-050` | **Cierre de la prueba de Codex**: la herramienta se mantiene como **verificación adicional permanente** y se retira el umbral de `ADR-049 §8` con su reversión por defecto (**sustituye `ADR-049 §8` completo y `§8.3` punto 1**; re-hoga `§8.3` puntos 2-4 sin cambio de contenido; **`ADR-049 §9` se conserva íntegro** y solo cambia lo que lo activa; **todo lo demás de `ADR-049` queda intacto** y `§6` lo enumera). **Estado: `ACEPTADA`** (2026-09-21, decisión expresa del usuario tras ver la tabla completa de resultados: *«No lo desinstalamos, lo utilizamos como herramienta extra de verificación»*). **Registra el resultado de la prueba sin maquillarlo**: los tres puntos de medición se ejecutaron y evaluaron, y el umbral formal **NO se cumple** — calibrado sobre PR [#204](https://github.com/pirexia/plataforma-educativa/pull/204) con **Cobertura 0 de 4** (ninguno de los hallazgos [#200](https://github.com/pirexia/plataforma-educativa/issues/200)-[#203](https://github.com/pirexia/plataforma-educativa/issues/203) reproducido), aunque en el mismo calibrado encontró **tres condiciones de carrera reales que nadie había visto** ([#205](https://github.com/pirexia/plataforma-educativa/issues/205)-[#207](https://github.com/pirexia/plataforma-educativa/issues/207), corregidas en PR [#212](https://github.com/pirexia/plataforma-educativa/pull/212)) cuya comprobación destapó además una **Crítica** ([#209](https://github.com/pirexia/plataforma-educativa/issues/209)); `1.6c` y `1.6d` con **Precisión perfecta** (3 de 3 aceptados, 0 descartados en cada uno) y **aportación diferencial real** en `1.6c` ([#224](https://github.com/pirexia/plataforma-educativa/issues/224), Alta: descontratación masiva ejecutable pese a autorización `Fallida`, que ningún revisor humano vio). Como `§8.2` exigía las tres a la vez, **el resultado formal es fracaso** y así queda escrito: este ADR **no declara superada la prueba**, sustituye hacia delante la regla que la juzgaba. **Argumenta por qué ese umbral estaba mal construido desde el principio** y no solo mal resuelto (`§3.2`): la medición irrepetible iba primero y **tenía derecho de veto sobre dos tercios de la evidencia aún sin recoger**, n = 4 sin repetición, calificaba a Codex por convención de proyecto que `§7.2` da por sentado que ignora, y premiaba el **solape** con los revisores humanos cuando el eje de aportación diferencial declara que el valor está justo en lo contrario — medía «adivinar el pasado», no «aportar valor futuro»; un quinto defecto es haber declarado *«automática y sin discusión»* una reversión que toca **configuración versionada** y que ninguna sesión puede ejecutar sin el usuario. **Decide el régimen**, que la frase del usuario no resuelve: uso **permanente y opcional** —no invocarla **no** es incumplimiento, no bloquea un cierre y **no entra en `CLAUDE.md §10`**—, **sin autoridad de bloqueo** (`ADR-049 §7.2` intacto: nueve aciertos de nueve **no otorgan autoridad**), **sin revalidación periódica** —descartada por repetir el mismo defecto y por ser un ritual manual que en solitario no se sigue, el argumento con que `ADR-049 §4` descartó su opción D—, con la medición continua reducida a lo que ya se hace: procedencia etiquetada en el issue y en `CHANGELOG.md`, y descartes anotados con su motivo. **Conserva armadas las tres condiciones de retirada inmediata** que nunca tuvieron que ver con el valor: escritura en el árbol o ejecución de `rescue`/`transfer`/puerta `Stop` **a la primera**, aparición de un **dato personal real** (`ADR-049 §5.6`, única caducidad que sobrevive) y archivo/cambio de licencia/aviso de seguridad sin corregir. Asimetría deliberada: **retirarla no exige ADR ni justificación; ampliar su alcance sí**. Deja una **regla general derivada para evaluar cualquier herramienta futura** (`§9`): ninguna condición irrepetible tiene derecho de veto, no se mide por conocimiento que el propio documento declara ausente, se mide valor marginal y no solape, y nada se declara «automático» si quien lo detecta no puede ejecutarlo. **`§11` deja un punto abierto del usuario y no de `architect`**: la política de datos de `ADR-049 §2.1` se aceptó *«para esta fase de prueba»* y la fase ha terminado — con uso indefinido procede decidir entre capa gratuita, desactivar el entrenamiento en la cuenta o capa de pago. `§10` acepta cinco riesgos residuales (desaparece la fecha que obligaba a volver a mirar, la autoridad por costumbre **empeora** con el historial de aciertos, muestra pequeña, datos de diligencia envejecidos, exposición residual ahora continua) y `§14` reporta seis hallazgos fuera de su ámbito de escritura, incluidos la *skill* `revision-con-codex` y `CLAUDE.md §2`, que siguen describiendo una prueba ya terminada |
| `ADR-051` | **Claves de catálogo declaradas en código como identificador público: enmienda de `ADR-029`** (**enmienda `ADR-029`, sección "Identificadores", tercer punto**, y la frase «única excepción argumentada» de `ADR-038 §4.2`/`§4.4` punto 3 —**sin reabrir la decisión del cursor cifrado**, que es de otra naturaleza y queda íntegra; se apoya en `ADR-034 §5` y `ADR-045 §9`). **Estado: `ACEPTADA` sin reservas** —las cuatro rutas de *feature flags* por `key` (`OPEN-BO-11`, 2026-09-21) y la generalización propuesta por `architect` (`OPEN-051-01`, ratificada por el usuario el 2026-09-21 sin cambios). Donde `ADR-029` decía que las rutas y la API usan **exclusivamente** `public_id` ULID, pasa a admitir **además** una clave de catálogo declarada en código que cumpla **seis condiciones a la vez**: **C1** la escribe el código del producto y **ningún usuario ni operador puede crearla por API**; **C2** es catálogo compartido y enumerarla dice qué tiene el producto, nunca quiénes son sus clientes; **C3** única e inmutable —cambiarla es declarar otra y marcar `retired_at`, nunca un `UPDATE`—; **C4** formato cerrado validado donde se materializa el catálogo, con la restricción de ruta correspondiente; **C5** sin dato personal ni derivable; **C6** no sustituye al ULID donde existe, que sigue siendo el identificador en auditoría, exportaciones y referencias que deben sobrevivir a la fila. **Regla general y no excepción nombrada, y el motivo es un dato y no una preferencia**: la afirmación «una sola excepción» que proponía la especificación de `1.6e` **es falsa contra el código**, y el argumento que la justificaba —única, estable, inmutable— **no discrimina**: describe la clase entera de «catálogo materializado desde el código». Las dos propiedades que de verdad protegen el motivo de `ADR-029` son C1 y C2; sin ellas la regla admitiría `tenants.slug` y un número de documento. **Habilita exactamente tres claves** (`feature_flags.key`, `modules.code`, `permissions.code`) y **nombra la que no pasa**: `roles.code` falla C1 y C2 —un centro crea roles con el código que quiera y la unicidad es por centro—, así que los roles se siguen direccionando por `public_id`. Tampoco entran un `role_code` dentro del cuerpo de una regla de *flag* (valor de comparación, no identificador) ni un segmento que selecciona un aspecto (`/tenant/settings/assets/{kind}`). **La regla es comprobable o no es una regla**: la lista de claves admitidas vive **en el código** en un único registro —no en el ADR, que es inmutable— y un **test de arquitectura** exige que todo parámetro de ruta sea `public_id`/`publicId` o una entrada declarada; registro y test entran con `1.6e`. **Reporta un hallazgo de severidad Media que la enmienda regulariza hacia delante y no absuelve**: `permissions.code` (desde `1.1`) y `modules.code` (desde `0.8` en el cuerpo, desde `1.6c` en una **ruta**) **ya estaban expuestas sin `public_id` y sin registro alguno** —ambas tablas se crean con `code` como clave primaria—; `ADR-034 §5` fijó esa forma ocho días después de `ADR-029` **sin nombrar la divergencia**, y tres cierres de fase pasaron por encima. Consecuencia sobre `1.6e`: `CA-BO-071` **tiene que reformularse**, porque hoy comprueba «que la excepción es una y está acotada» y eso es literalmente falso en el producto. Deja dos preguntas abiertas: la ratificación de la generalización y dónde vive exactamente el registro |
| `ADR-052` | **Tema por tenant, modo oscuro y tokens de diseño en la SPA** (entrada de `1.7`; no sustituye ningún ADR, `ADR-023` intacto). **Estado: `ACEPTADA`** (2026-09-22, ratificada por el usuario sin cambios sobre lo propuesto). **Un solo mecanismo de tema** para pantallas públicas y autenticadas —el tenant se resuelve por *host* también antes del *login*—, alimentado solo por `GET /tenant/branding` (`GET /tenant/settings` exige permiso de administración), en dos capas: *design system* sin noción de tenant, que escribe variables `--brand-*` por CSSOM, y contexto del centro sin noción de CSS, cargado una vez en el arranque con los dos colores en `localStorage` contra el destello. **Modo oscuro ortogonal a la marca**, tres estados (`system`/`light`/`dark`), preferencia local, sin columna nueva. Parte de un dato: ningún color alcanza 4,5:1 como texto contra los dos fondos, así que el uso del primario sobre el fondo pasa por una **variante derivada con contraste garantizado** (`--primary-on-background`, calculada en cliente, sin tocar `RN-CORE-15` ni el endpoint público), y se prohíben por test `text-/border-/ring-primary` y los colores literales. **Tokens como variables CSS en tres niveles**, sin paquete de tokens ni dependencia nueva; extracción a paquete compartido diferida a un ADR propio al abrir la SPA del backoffice. Documentación en `docs/design-system.md`, no en `docs/modulos/`. Las cuatro preguntas abiertas se resolvieron con la opción por defecto: `color_secondary` es texto sobre el primario, preferencia de modo oscuro solo local, sin control del centro sobre él, tipografía de sistema hasta `0.11c` |
| `ADR-053` | **Registro de navegación y bloques del panel de inicio en la SPA** (entrada de `1.8`; **resuelve `OPEN-CORE-16`**; no sustituye ningún ADR). **Estado: `ACEPTADA`** (2026-09-23; ratificada por el usuario sin cambios sobre lo propuesto por `architect`). Cada módulo declara su *shell* en **un único `src/modules/<modulo>/shell.ts`** (rutas, entradas de navegación, bloques del panel), con vistas y bloques cargados bajo demanda; **las rutas se mudan del *router* central al módulo**. Ensamblado por **lista explícita y ordenada** en `src/navigation/modules.ts` —patrón de `src/i18n/index.ts`, sin `import.meta.glob`—, cuyo orden es el orden del producto, y coherencia comprobada por tests del registro ensamblado. **Los permisos de una pantalla se declaran una sola vez, en `meta.permissions` de la ruta** (anyOf, lista vacía solo en autoservicio por identidad); la entrada de navegación deriva de ahí su visibilidad y no lleva permisos propios. **Las secciones del menú son un catálogo de aplicación** (`src/navigation/sections.ts`), no una por módulo; dos niveles, sin sub-entradas ni pesos numéricos. Contrato de bloque del panel: `id`, `titleKey`, `permissions` no vacía, componente asíncrono que pide sus propios datos, pinta sus estados y falla contenido en su tarjeta; orden por módulo tras los tres bloques del *shell*. **Cualquier `403` salvo `mfa-enrollment-required` recarga `GET /me`**, incluido `module-disabled` (`RMOD-008`), conservando el mensaje de `RMOD-009` en la vista actual. Obliga a ajustar `REQ-CORE/funcional.md §12.3.4`, `§12.4`, `§12.5`, `§12.6`, `§12.9` y `CA-CORE-098`/`-103`/`-152` |
...
|---------|-------|-------|-------------|
| 3.2.8 | 2026-09-23 | Product Owner | `ADR-053`: registro de navegación y bloques del panel de inicio en la SPA, añadido al índice de la sección 18, entrada de `1.8` (resuelve `OPEN-CORE-16`). **Estado `ACEPTADA`**, ratificado por el usuario sin cambios el mismo día en que `architect` lo redactó. Fija forma y ensamblado del registro de navegación (`shell.ts` por módulo, lista explícita en `src/navigation/modules.ts`, permisos declarados una sola vez en `meta.permissions` de la ruta, secciones como catálogo de aplicación) y el contrato de los bloques del panel; decide que `403 module-disabled` también recarga `GET /me`. Ajusta en el mismo commit `docs/modulos/REQ-CORE/funcional.md §12` (aprobada, 2026-09-23): resueltas también `OPEN-CORE-12` (pantallas pendientes de `REQ-CORE` diferidas a `1.9b`, nuevo paso en `PLAN-IMPLEMENTACION.md`), `OPEN-CORE-13` (motor de *widgets* configurables diferido), `OPEN-CORE-14` (objetivo táctil de 44px solo en punteros gruesos) y `OPEN-CORE-17` (solo iconos de Lucide en estados vacíos), las cuatro por decisión directa del usuario. Queda abierta y no bloqueante `OPEN-CORE-15` (issue [#258](https://github.com/pirexia/plataforma-educativa/issues/258), registro de intento de acceso a otro tenant, trabajo de servidor fuera de 1.8). Ningún requisito de las secciones 1-17 se reescribe y ninguna decisión previa de otro ADR se reabre. |
| 3.2.7 | 2026-09-21 | Product Owner | `ADR-051` (sección 18): ratificación del usuario a la generalización de `§2`-`§5` (`OPEN-051-01`), pendiente desde su alta el mismo día. Estado pasa de "parte decidida por el usuario `ACEPTADA`, generalización `PROPUESTA`" a **`ACEPTADA` sin reservas, sin cambios sobre la propuesta de `architect`**. No cambia ni una condición de las seis ni ninguna otra decisión del ADR. |
| 3.2.6 | 2026-09-21 | Product Owner | `ADR-051`: claves de catálogo declaradas en código como identificador público, enmienda de `ADR-029`, añadido al índice de la sección 18. **Estado `ACEPTADA` en su parte decidida por el usuario** —las cuatro rutas de *feature flags* de `1.6e` (`REQ-BO-005`) direccionadas por `key`, `OPEN-BO-11`, 2026-09-21— y **`PROPUESTA` en su generalización** (las seis condiciones de `§2`, el registro de `§5.1` y el test de `§5.2`), que es decisión de `architect` y excede lo aprobado por el usuario; `1.6e` no depende de esa ratificación. `spec-writer` había propuesto registrar la desviación como excepción nombrada y acotada a `REQ-BO-005`; `architect` la descarta porque, comprobada contra el código, esa afirmación era **falsa** el día que se escribió: `modules.code` y `permissions.code` ya estaban expuestas sin `public_id` desde `0.8`/`1.1` sin que nadie lo registrara. Enmienda en su lugar `ADR-029` con una regla general y comprobable: una clave de catálogo declarada en código es identificador válido en URL/API cuando cumple **seis condiciones a la vez** —la escribe el código del producto, es catálogo compartido y no de un centro, única e inmutable, formato cerrado y validado, sin dato personal, y no sustituye al ULID donde este existe—, con registro único en código y test de arquitectura que la hace exigible. **Reporta un hallazgo de severidad Media** (issue [#238](https://github.com/pirexia/plataforma-educativa/issues/238)) que la enmienda regulariza hacia delante y no absuelve retroactivamente (`ADR-034 §5` fijó la forma de tabla de referencia sin `public_id` ocho días después de `ADR-029`, sin nombrar la divergencia; tres cierres de fase y `doc-reviewer` pasaron por encima). **Ningún requisito de las secciones 1-17 se reescribe** y ninguna decisión previa de `ADR-029` ni de `ADR-038` (el precedente del cursor cifrado) se reabre. |
| 3.2.5 | 2026-09-21 | Product Owner | `ADR-050`: cierre de la prueba de Codex, añadido al índice de la sección 18. Los tres puntos de medición de `ADR-049 §8` (calibrado sobre PR #204 + `1.6c` + `1.6d`) ya están cerrados; el umbral formal **no se cumple** (Cobertura 0/4 en el calibrado, medición irrepetible), aunque Precisión y Aportación diferencial sí se cumplen con holgura (issue #224, Alta, en `1.6c`). El usuario decide expresamente mantener la herramienta, a la vista del resultado completo: *«No lo desinstalamos, lo utilizamos como herramienta extra de verificación»*. `ADR-050` **sustituye `ADR-049 §8` completo y `§8.3` punto 1**; conserva `§9` (procedimiento de reversión) íntegro como retirada **ordinaria** en vez de consecuencia de una prueba; **no instituye revalidación periódica**; y deja el uso como **permanente y opcional**, sin autoridad de bloqueo y sin entrar en la definición de terminado (`CLAUDE.md §10`). Deja un punto abierto del usuario, no resuelto por `architect`: la política de datos de la capa gratuita de ChatGPT se aceptó «para esta fase de prueba», que ha terminado. **Ningún requisito de las secciones 1-17 se reescribe** y ninguna decisión previa de `ADR-049` fuera de `§8`/`§9` cambia. |
| 3.2.4 | 2026-09-14 | Product Owner | `ADR-049`: segunda opinión de **Codex** sobre código ya implementado (plugin oficial `openai/codex-plugin-cc`, Apache-2.0), añadido al índice de la sección 18. **Estado `PROPUESTA`**: las tres decisiones del usuario de `§2` ya estaban tomadas (capa gratuita de ChatGPT para una prueba acotada, alcance de solo lectura, ámbito `project`) y el ADR las ratifica sin reabrirlas; lo que somete a aprobación son las condiciones vinculantes de `§5`-`§9`. **No reescribe ninguna decisión previa ni ningún requisito de las secciones 1-17**, y no afecta a ninguna línea de código entregado: el plugin no entra en `composer.json` ni en `package.json` y no se despliega. Lo que cambia es el **proceso de revisión** de `CLAUDE.md §4`/`§6` y `.claude/settings.json`, versionado. Cierra la comprobación de `CLAUDE.md §1` con datos verificados contra la API de GitHub (7 *releases*, 68 días sin *commits*, 499 *issues* abiertas), calibrando el criterio contra el radio de daño y dejando los datos escritos sin adornarlos. Decide las cuatro cosas que nadie más iba a decidir: que **la lista `deny` de `.claude/settings.json` no tiene equivalente en el *sandbox* de Codex** —que restringe escritura y red, no lectura— y que la protección se obtiene en tres capas, siendo la tercera una **auditoría previa de los tres `.env` del árbol** y no una opción de configuración; que la interfaz propia que exige `RNF-MANT-007` para una herramienta de desarrollo es **un protocolo documentado, no código**; que Codex se invoca después de `implementer` y **nunca en lugar de** los tres revisores obligatorios, **sin autoridad de bloqueo**, y que un hallazgo suyo contrario a un ADR vigente se cierra citando el ADR; y un **criterio de reversión medido** sobre el *diff* ya cerrado de `1.6b` (PR #204), con reversión por defecto si no hay evaluación escrita. La puerta de revisión automática (`Stop`) queda **desactivada como norma**, por consumo de cuota y por incompatibilidad con `CLAUDE.md §10`. |
| 3.2.3 | 2026-09-14 | Product Owner | `ADR-048` (sección 18): ratificación del usuario al permiso de `§10` (que `1.6b` escriba las cinco cosas enumeradas dentro de `REQ-CORE`), pendiente desde su alta el 2026-09-11. Estado pasa de "mecanismo `ACEPTADO`, permiso pendiente" a **`ACEPTADA`** sin reservas. No cambia el mecanismo ni ninguna otra decisión. |
| 3.2.2 | 2026-09-08 | Product Owner | `ADR-047`: tablas de plataforma con visibilidad por tenant afectado, añadido al índice de la sección 18, **`ACEPTADA`** (aprobación explícita del usuario, incluida la decisión de `§5.1`). Resuelve `OPEN-BO-10`, la puerta que la especificación de `1.6` puso ante su primera migración y sometió al visto bueno conjunto de `architect` y `db-reviewer`: **ambos dados, las tres piezas aprobadas con cambios y ninguna rechazada**. **Amplía `ADR-033 §7` y `§10` sin sustituirlos**, igual que hizo `ADR-034`: añade una quinta categoría de tabla —la fila no pertenece a ningún centro pero registra un hecho que le concierne, y `REQ-BO-007` obliga a que pueda consultarlo— con convención de nombres (`affected_tenant_id`, nunca `tenant_id`), política RLS solo de lectura y privilegios de columna enumerada. **Ningún requisito de las secciones 1-17 se reescribe** y ninguna decisión de `ADR-033`, `ADR-045` ni `ADR-046` cambia. Corrige antes de escribirse dos fallos de *build* ciertos en `docs/modulos/REQ-BO/datos.md` (`tenant_lifecycle_events §5.2` y `feature_flag_rules §9.3`, que nombraban `tenant_id` a una referencia) y un hueco de confidencialidad en `§4.3` (`GRANT SELECT` de tabla completa expondría al *runtime* de un centro datos personales del personal del proveedor). Vinculante para los 53 módulos. Requisito previo de la primera migración del paso `1.6` (`REQ-BO`). |
| 3.2.1 | 2026-09-08 | Product Owner | `ADR-046`: separación del backoffice de plataforma — **superficie, sesión y propósito declarado en `runAsPlatform()`**, añadido al índice de la sección 18. Resuelve las tres preguntas bloqueantes que `spec-writer` dejó abiertas en `docs/modulos/REQ-BO/funcional.md §14` (`OPEN-BO-01`, `OPEN-BO-02`, `OPEN-BO-03`) y cierra los puntos 2 y 3 del issue [#6](https://github.com/pirexia/plataforma-educativa/issues/6). **`ADR-002` no se toca**: el backoffice es un módulo más del monolito, con *guard*, grupo de rutas y SPA propios, y la aplicación desplegable aparte se descarta porque haría inejecutable la implementación única de dependencias de módulos que `ADR-045`/`RN-BO-22` exigen en `REQ-CORE`. Se corrige una premisa falsa de la especificación: **`sessions` no tiene `tenant_id`**, luego la sesión de plataforma va a tabla propia con `REVOKE ALL … FROM plataforma_app` en vez de a una columna anulable inexistente. `runAsPlatform()` cambia de firma en infraestructura compartida (`App\Support\Tenancy`), afectando a todo el producto y no solo a `REQ-BO`. Ningún requisito de las secciones 1-17 se reescribe. Entrada obligatoria del paso `1.6` (`REQ-BO`) junto con `ADR-045`. |
| 3.2.0 | 2026-09-08 | Product Owner | `ADR-045`: contratación de módulos por tenant — **una sola potestad, la del Super Admin, con aviso al centro**. Resuelve el issue [#44](https://github.com/pirexia/plataforma-educativa/issues/44) y `OPEN-CORE-03`, que arrastraban desde `1.1` la contradicción entre `REQ-CORE-002` («el Administrador de Centro activa/desactiva módulos contratados») y `RMOD-002` («el Super Admin activa/desactiva módulos por tenant») sobre un único booleano `module_subscriptions.enabled`. Se resuelve **eliminando uno de los dos actores, no repartiendo el dato**: `REQ-CORE-002` pierde la capacidad de conmutar y conserva consultar, recibir aviso de las altas y configurar `settings`. Reescritos `REQ-CORE-002`, `RMOD-002`, `RMOD-003`, `RMOD-004`, `RMOD-005`, `RMOD-006`, `RMOD-007`, `RMOD-008`, `RMOD-009`, `RMOD-010`, `REQ-SAAS-001`, `REQ-BO-002` y `REQ-COM-003`. Se factura sobre `enabled`; las dependencias de `RMOD-006` ganan sitio donde declararse (`depends_on` en el descriptor, con validación que aborta el despliegue) y se aplican como invariante de escritura, no de lectura; los módulos esenciales se declaran en vez de codificarse; y queda zanjado que todo evento del ciclo de vida de un módulo pertenece a `REQ-CORE`. **Coste de esquema cero**: ninguna columna se añade, renombra ni elimina. Decisión del usuario, 2026-09-08, **en contra de la recomendación de `architect`** (dos columnas con `AND`), con la discrepancia registrada en `ADR-045 §12.1`. `ADR-045` añadido al índice de la sección 18. Entrada obligatoria del paso `1.6` (`REQ-BO`). |
