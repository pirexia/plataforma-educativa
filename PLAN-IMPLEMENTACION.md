...
- [~] **1.3 · `REQ-AUTH-003`: MFA — TOTP, obligatoriedad por rol y restablecimiento** [SONNET]
  Especificación aprobada 2026-08-26 (`docs/modulos/REQ-AUTH/funcional.md §C`, `OPEN-AUTH-18` a `26` resueltas). `ADR-041` cerrado (dependencias `pragmarx/google2fa`+`uqr`). **En curso** desde 2026-08-26/27: modelo de datos, `MfaPolicy`, verificador TOTP, endpoints, login en dos pasos, muro, `PATCH /roles` y suite de 35 tests ya escritos (rama `feature/REQ-AUTH-003-1.3-mfa-obligatorio-por-rol`, commits hasta `ae9f0ec`), **sin verificar** (tests no ejecutados, `pint`/`phpstan` no ejecutados). Ver `memory.md` "Siguiente paso concreto" para el punto exacto de retomada.
