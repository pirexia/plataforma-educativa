...
- [ ] **1.4c · `REQ-AUTH-004` (parte 2/2): SSO institucional — SAML 2.0** [OPUS + SONNET]
  Adaptador SAML sobre el catálogo de `1.4b` ya construido: metadatos, ACS URL por tenant, gestión y rotación de certificados de firma, correlación de petición en servidor (el `state`-en-sesión de `1.4`/`1.4b` no aplica, `ADR-043 §2.1`). Dependencia externa nueva sin aprobar todavía (comparación de candidatas hecha en `ADR-043 §7.3`, decisión de biblioteca pendiente del usuario al especificar, igual que `OPEN-AUTH-35`→`ADR-042`).
