...
- [~] **1.3 · `REQ-AUTH-003`: MFA — TOTP, obligatoriedad por rol y restablecimiento** [SONNET]
  Especificación aprobada 2026-08-26 (`docs/modulos/REQ-AUTH/funcional.md §C`, `OPEN-AUTH-18` a `26` resueltas). `ADR-041` cerrado (dependencias `pragmarx/google2fa`+`uqr`). **Implementación COMPLETA y VERIFICADA 2026-08-27**: backend (10 endpoints `Auth` + `PATCH /roles` en `Core`, `MfaPolicy`, login en dos pasos, muro de sesión restringida, restablecimiento por administrador) y frontend (4 pantallas de `§C.11`, `QrCode.vue`). 320 tests Pest + 13 Vitest en verde, `pint`/`phpstan`/`eslint`/`lint:i18n`/`vue-tsc`+build limpios. En revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) antes de mezclar. Rama `feature/REQ-AUTH-003-1.3-mfa-obligatorio-por-rol`, commit `b22dd59`. Ver `memory.md` "Siguiente paso concreto".
