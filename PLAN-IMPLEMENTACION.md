...
- [x] **0.8 · Modelo de datos núcleo** [OPUS + SONNET]
  ...
  Cerrado 2026-08-18: diseño en `docs/adr/ADR-034-modelo-de-datos-nucleo.md` (Opus, subagente `architect`), implementación en 12 subpasos (Sonnet). `Person`/`User` separados (identidad vs. credencial); esquema completo de `Role`/`Permission` ahora con resolutor diferido a 1.5; `AuditLog` polimórfica append-only con `REVOKE` en el motor; `AcademicYear` con `academic_year_id` obligatorio-o-ausente, nunca nullable; `ModuleSubscription` con catálogo en código vía comando `platform:sync-registry` (0.8.11). `TenantModel` gana `SoftDeletes`+`RecordsAuthorship`. 91 tests en verde (44 nuevos), Pint y Larastan limpios. Cuatro corridas de medición de sobrecoste de RLS (0.8.12): media ~1.24%, dentro del 1-3% previsto. `db-reviewer`/`security-reviewer` aplicados por la propia sesión (sin bloqueantes tras corregir 2 hallazgos, uno Alta y uno Media, issues [#16](https://github.com/pirexia/plataforma-educativa/issues/16)/[#17](https://github.com/pirexia/plataforma-educativa/issues/17)). Detalle completo (checklist de los 12 subpasos, los 6 bugs propios encontrados, y el resultado íntegro de las revisiones) en `docs/historial/0.8-modelo-de-datos-nucleo.md`. Rama `feature/REQ-0.8-modelo-datos-nucleo`, sin mezclar a `develop` todavía.
