...
- [ ] **1.3 · `REQ-AUTH-003`: MFA — TOTP, obligatoriedad por rol y restablecimiento** [SONNET]
  Especificación aprobada 2026-08-26 (`docs/modulos/REQ-AUTH/funcional.md §C`, `OPEN-AUTH-18` a `26` resueltas). Alta/verificación TOTP, códigos de respaldo, login en dos pasos, `MfaPolicy` (obligatoriedad por rol y resolución multi-rol, `RPERM-007`), período de gracia y muro de sesión restringida, `PATCH /api/v1/roles/{public_id}` acotado a `mfa_required` (permiso nuevo `rol.actualizar` en `REQ-CORE`), vista previa de usuarios afectados y estado de cumplimiento, restablecimiento de MFA por el administrador. Método `sms` cerrado con guarda, sin proveedor (`OPEN-AUTH-18`). Dependencias nuevas por aprobar en ADR antes de implementar: `pragmarx/google2fa` (backend) y librería de QR (frontend), ambas envueltas tras interfaz propia — `OPEN-AUTH-19`/`20`.
