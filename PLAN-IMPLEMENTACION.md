...
- [x] **0.7 · Núcleo multi-tenant** [OPUS + SONNET] ⚠️ *paso crítico*
  ...
  Cerrado 2026-08-17: diseño en `docs/adr/ADR-033-implementacion-del-aislamiento-multi-tenant.md` (Opus, subagente `architect`), implementación en 11 subpasos (Sonnet). RLS como barrera primaria (`FORCE ROW LEVEL SECURITY` + política estándar en toda tabla de negocio vía el helper `App\Support\Tenancy\TenantMigration::tenantTable()`), scope de Eloquent (`TenantModel`/`BelongsToTenant`/`TenantScope`) como ergonomía secundaria. Tres roles de PostgreSQL sin `SUPERUSER` (`plataforma_owner`/`plataforma_app`/`plataforma_platform`). Middleware `ResolveTenant` por subdominio, colas conscientes de tenant (`Queue::createPayloadUsing`, listeners de `JobProcessing`/`JobProcessed`/`JobFailed`/`Looping`), almacenamiento y caché con prefijo por tenant, claves foráneas compuestas `(tenant_id, id)`. Suite de tests migrada de SQLite a PostgreSQL real (`RNF-MANT-006` exige RLS de verdad, no simulado). 47 tests en `tests/Feature/Tenancy/`, incluida la batería completa de diez tests de aislamiento de `ADR-033` §10 (el #1, caso de uso HTTP con autenticación, queda diferido a que exista el primer endpoint de negocio con `REQ-AUTH`). Detalle completo del proceso, hallazgos y bugs propios encontrados en `memory.md`.

- [x] **0.8 · Modelo de datos núcleo** [OPUS + SONNET]
  Migraciones de `AcademicYear`, `Person`, `User`, `Role`, `Permission`, `AuditLog`, `ModuleSubscription` (`Tenant` ya se hizo en 0.7.4, raíz del aislamiento). Campos de auditoría, borrado lógico, `tenant_id` y `academic_year_id`. Sección 16 del documento de requisitos.
  Cerrado 2026-08-18: diseño en `docs/adr/ADR-034-modelo-de-datos-nucleo.md` (Opus, subagente `architect`), implementación en 12 subpasos (Sonnet). `Person`/`User` separados (identidad vs. credencial); esquema completo de `Role`/`Permission` ahora con resolutor diferido a 1.5; `AuditLog` polimórfica append-only con `REVOKE` en el motor; `AcademicYear` con `academic_year_id` obligatorio-o-ausente, nunca nullable; `ModuleSubscription` con catálogo en código vía comando `platform:sync-registry` (0.8.11). `TenantModel` gana `SoftDeletes`+`RecordsAuthorship`. 94 tests en verde, Pint y Larastan limpios. Cuatro corridas de medición de sobrecoste de RLS (0.8.12): media ~1.24%, dentro del 1-3% previsto. `db-reviewer`/`security-reviewer` corrieron primero autoaplicados por la propia sesión de implementación (un *fork* sin acceso a la herramienta Agent), y **después de verdad, como subagentes independientes**, lanzados por la sesión orquestadora: confirmaron los dos hallazgos ya corregidos (Alta [#17](https://github.com/pirexia/plataforma-educativa/issues/17), Media [#16](https://github.com/pirexia/plataforma-educativa/issues/16), ambos cerrados) y encontraron dos Media más, corregidos en la misma sesión ([#19](https://github.com/pirexia/plataforma-educativa/issues/19)/[#20](https://github.com/pirexia/plataforma-educativa/issues/20), cerrados) y uno diferido a propósito a 1.2 por quedar fuera de alcance ([#18](https://github.com/pirexia/plataforma-educativa/issues/18), falta `PasswordBrokerRepository` propio con tenant en la recuperación de contraseña). Sin bloqueantes. Detalle completo (checklist de los 12 subpasos, los 6 bugs propios encontrados, y el resultado íntegro de ambas revisiones) en `docs/historial/0.8-modelo-de-datos-nucleo.md`. Mezclado a `develop` vía PR [#21](https://github.com/pirexia/plataforma-educativa/pull/21) (squash), rama `feature/REQ-0.8-modelo-datos-nucleo` borrada.

- [x] **0.9 · Auditoría e i18n transversales** [SONNET]
  ...
  Cerrado 2026-08-18: previo a empezar, se redactó `ADR-035` (pedido en sesión anterior, pendiente de escribir), que resuelve `OPEN-12` — el derecho de supresión sobre `audit_logs.changes` no se ejerce editando la fila (inmutable sin excepciones), se ejerce evitando que el valor identificativo entre desde el origen; la supresión se completa por retención (PR [#26](https://github.com/pirexia/plataforma-educativa/pull/26)). Implementación de 0.9 (subagente `fork` en *worktree* aislado): interfaz `Auditable` con tres políticas (`Full`/`Selective`/`Redacted`), `AuditChangeBuilder` con los cinco pasos de evaluación de `ADR-035 §4`, *observer* de ciclo de vida, 5 modelos de tenant declarados; sistema de i18n de 4 idiomas con `vue-i18n` (envuelto tras `useT()`, `RNF-MANT-007`) y detector de literales sin traducir (`INV-009`) enganchado a `ci-web.yml`. `security-reviewer`/`doc-reviewer` lanzados de verdad por la sesión orquestadora (la autorrevisión del *fork* no sustituye la revisión independiente, misma lección que 0.8) encontraron 1 Alta (`Tenant` fuera del alcance que `ADR-035 §8` declaraba, sin ADR sustitutivo — `CLAUDE.md §11`) y varias Media, todas corregidas en la misma sesión: `ADR-036` (sustituye esa fila de `§8`, `Tenant` se audita en `admin_action_logs` cuando exista en 1.6, issue [#27](https://github.com/pirexia/plataforma-educativa/issues/27)), guarda en `AuditRecorder`, envoltorio de `vue-i18n`, `docs/i18n.md` nuevo, `docs/modulos/REQ-CORE/datos.md` completado. 108 tests en verde, Pint/Larastan/ESLint/`vue-tsc`/`lint:i18n` limpios. Detalle completo en `docs/historial/0.9-auditoria-i18n.md`. PR de la rama `feature/REQ-0.9-auditoria-i18n` — confirmar en `memory.md` si ya se fusionó.

- [x] **0.9b · Portabilidad del despliegue: imágenes, Quadlet y secretos** [SONNET]
  ...
  Cerrado 2026-08-19: implementado por subagente `fork` en *worktree* aislado, con corte real por límite de cuota a mitad de trabajo (primera vez que se disparó el cierre automático de `CLAUDE.md §3`) y retomado sin pérdida tras el reset. Las tres pruebas obligatorias de `ARCHITECTURE.md §4.3` verificadas de verdad sobre `compose.prodlike.yaml` (arranque nativo con `systemctl --user` bloqueado por un problema de permisos preexistente del host, documentado sin forzarlo). Revisión independiente (`security-reviewer`/`doc-reviewer`) encontró 7 hallazgos Media, todos corregidos, y 4 Baja diferidos a propósito (issues [#37](https://github.com/pirexia/plataforma-educativa/issues/37)/[#38](https://github.com/pirexia/plataforma-educativa/issues/38)/[#40](https://github.com/pirexia/plataforma-educativa/issues/40)). Detalle completo en `docs/historial/0.9b-portabilidad-despliegue.md`.
...
- [ ] **0.10d · Destino de copias de seguridad** [SONNET]
  Almacenamiento de objetos en **proveedor distinto** al del host, con una copia inmutable (`REQ-BKP-001`). Debe existir antes de que entre el primer dato real. Incluye la **custodia de `APP_KEY` separada de la copia de la base de datos** (`ADR-037 §7.2`, punto 4): una copia que contenga ambas juntas anula el cifrado de los datos de categoría especial. **Desde `1.3` (2026-08-26, `OPEN-AUTH-26`, confirmado por el usuario) esto deja de ser buena práctica y pasa a ser condición de cierre**: `APP_KEY` cifra también los secretos TOTP de usuario, y perderla obliga a restablecer el MFA de todo el mundo.
...
- [x] **1.2 · `REQ-AUTH`: autenticación local y sesiones** [SONNET]
  ...
  Cerrado 2026-08-25: 10 endpoints (`apps/api/app/Modules/Auth/`), 6 pantallas públicas enrutadas, OpenAPI completo. 241 tests en verde, `pint`/`phpstan` limpios; frontend (`eslint`/`lint:i18n`/`vue-tsc`+`build`/`vitest`/Playwright e2e) en verde. Revisión independiente (`security-reviewer`/`doc-reviewer`) encontró 2 hallazgos Alta de seguridad (issues #73/#74, tokens sin cifrar en `failed_jobs` y límite de tasa ausente en `csrf-cookie`, ambos corregidos con test de regresión) y 7 Media de documentación, todos corregidos. Login por navegador real verificado de extremo a extremo tras corregir un fallo de la propia sesión (issue #71: la SPA debe servirse desde el mismo host que la API para que `document.cookie` lea la cookie CSRF). Mezclado a `develop` vía PR [#76](https://github.com/pirexia/plataforma-educativa/pull/76) (squash, commit `0d34587`), rama y *worktrees* de subagentes borrados. Detalle completo en `docs/historial/1.2-auth-local-sesiones.md`.
- [x] **1.2b · `REQ-AUTH-005`: panel de sesiones activas, cierre remoto y detección de dispositivo** [SONNET]
  Puntos 2-4 de `REQ-AUTH-005`, diferidos de `1.2` (issue #59, 2026-08-22): listado de sesiones activas por usuario, revocación individual/total, detección y alerta de login desde nuevo dispositivo. **Solo "dispositivo nuevo"** — sin geolocalización por IP (`OPEN-AUTH-13`, ninguna fuente decidida en el proyecto; el punto 4 del requisito queda cumplido a medias, `RN-AUTH-47`).
  Cerrado 2026-08-26: 2 migraciones RLS, 3 endpoints autoservicio, pantalla `/cuenta/sesiones`, 4 idiomas, OpenAPI. `ADR-040` (exclusión declarativa de auditoría). 279 tests backend + 10 frontend en verde, `pint`/`phpstan`/`eslint`/`vue-tsc`/`lint:i18n` limpios, verificado en navegador real (Playwright MCP). Pipeline de revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) con 3 hallazgos Media, todos corregidos. Mezclado a `develop` vía PR [#91](https://github.com/pirexia/plataforma-educativa/pull/91) (squash, commit `12fe917`). Detalle completo en `docs/historial/1.2b-sesiones-activas.md`.
- [x] **1.3 · `REQ-AUTH-003`: MFA — TOTP, obligatoriedad por rol y restablecimiento** [SONNET]
  TOTP con códigos de respaldo, login en dos pasos, obligatoriedad por rol (`MfaPolicy`), gracia y muro de sesión restringida, `PATCH /roles/{public_id}` acotado a `mfa_required` (permiso nuevo `rol.actualizar` en `REQ-CORE`), listado de cumplimiento, restablecimiento por administrador. `ADR-041` (dependencias `pragmarx/google2fa`+`uqr`).
  Cerrado 2026-08-26/27: 10 endpoints `Auth` + 1 `Core`, 4 pantallas (`/entrar` en dos pasos, `/cuenta/seguridad`, `/cuenta/seguridad/obligatorio`, `QrCode.vue`). 320 tests Pest + 15 Vitest en verde, `pint`/`phpstan`/`eslint`/`lint:i18n`/`vue-tsc`+build limpios, `composer audit`/`npm audit` sin vulnerabilidades. Revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`): sin hallazgos de seguridad, 8 hallazgos Media/Baja corregidos (issues #98-#105; #96 corregido aparte, antes de la revisión; #97 y #106 diferidos a propósito, ajenos a 1.3). Un subagente relanzado tras corte de cuota recortó alcance sin autorización, corregido — motivó norma nueva en `CLAUDE.md §3` (v2.2.1). Mezclado a `develop` vía PR [#107](https://github.com/pirexia/plataforma-educativa/pull/107) (squash, commit `cd13e8a`). Detalle completo en `docs/historial/1.3-mfa-obligatorio-por-rol.md`.
- [x] **1.3b · `REQ-AUTH-003`: MFA — correo como segundo factor y excepciones temporales** [SONNET]
  Correo como segundo factor, excepciones temporales nominales de la obligatoriedad, 4 tareas de mantenimiento (cierra issue #109) y pantalla de administración `/administracion/mfa`. Revisión independiente completa (`security-reviewer`/`db-reviewer`/`doc-reviewer`): sin hallazgos de seguridad, 2 Media corregidos (índices de purga, dependencias/manual sin documentar). Cerrado y mezclado 2026-08-31, PR [#123](https://github.com/pirexia/plataforma-educativa/pull/123) (squash, commit `dd68f48`). Detalle completo en `docs/historial/1.3b-mfa-correo-excepciones.md`.
- [x] **1.4 · `REQ-AUTH-002`: login con Google y fusión de cuentas** [SONNET]
  OAuth2 con PKCE, fusión automática de cuenta solo con email verificado (sin alta automática de usuarios, decisión restrictiva del 2026-08-31), vinculación/desvinculación desde perfil, resolución de tenant por `redirect_uri` propia de cada centro. `ADR-042` (`laravel/socialite`, envuelto tras interfaz propia).
  Cerrado 2026-09-01: 6 endpoints `Auth`, 3 pantallas (botón en `/entrar`, `/entrar/google`, bloque en `/cuenta/seguridad`). 406 tests Pest + 39 Vitest en verde, `pint`/`phpstan`/`eslint`/`lint:i18n`/`vue-tsc`+build limpios, verificado en navegador real (Playwright MCP) con proveedor simulado. Hallazgo crítico en plena implementación corregido antes de tocar código (issue #140, valor por defecto inseguro). Revisión independiente en dos pasadas de `doc-reviewer` (2 Alta bloqueantes en la primera, corregidas y reverificadas de forma independiente en la segunda); `security-reviewer`/`db-reviewer` sin bloqueantes. Mezclado a `develop` vía PR [#143](https://github.com/pirexia/plataforma-educativa/pull/143). Detalle completo en `docs/historial/1.4-google-login-fusion-cuentas.md`.
- [x] **1.4b · `REQ-AUTH-004` (parte 1/2): SSO institucional — OIDC y aprovisionamiento** [OPUS + SONNET]
  Añadido al plan (issue [#58](https://github.com/pirexia/plataforma-educativa/issues/58), 2026-08-22): requisito MUST de fase 1 sin paso asignado hasta ahora. Alcance dividido en dos pasos por `ADR-043` (evaluación previa de `architect`, 2026-09-01): SAML rompe cuatro cosas a la vez que OIDC no (mecanismo de sesión del *callback*, envoltorio de la dependencia, perfil de riesgo, ciclo de vida del certificado) — ver `1.4c`. Este paso: catálogo `identity_providers` por tenant (autoservicio del propio centro, `ADR-043 §8.3`), OIDC genérico por tenant (Azure AD/Entra ID, Google Workspace con `hd` — cierra `OPEN-AUTH-33`), mapeo de atributos por lista blanca cerrada, re-tecleado de `user_identities` por proveedor concreto (`ADR-043 §3.6`). Aprovisionamiento **solo emparejamiento** con `Person` ya existente en el censo, nunca creación automática (decisión del usuario, 2026-09-01, `ADR-043 §8.1`). `client_secret` por tenant cifrado en tabla propia (`ADR-043 §8.2`).
  Cerrado 2026-09-02: 9 endpoints nuevos + 2 modificados, catálogo de proveedores con pantallas de autoservicio, emisor OIDC simulado para desarrollo y tests. 451 tests Pest + 106 Vitest en verde, `pint`/Larastan/`eslint`/`lint:i18n`/`vue-tsc` limpios, 14/14 *checks* de CI en verde. Revisión independiente: `security-reviewer` sin hallazgos; `db-reviewer` y `doc-reviewer` en dos pasadas cada uno (Alta bloqueantes en la primera — DDL bloqueante sobre tabla viva, documentos raíz desincronizados — resueltas y reverificadas contra el fichero real en la segunda, que a su vez encontró hallazgos Media nuevos introducidos por la propia corrección, también resueltos). Mezclado a `develop` vía PR [#149](https://github.com/pirexia/plataforma-educativa/pull/149). Detalle completo en `docs/historial/1.4b-sso-institucional-oidc.md`.
- [x] **1.4c · `REQ-AUTH-004` (parte 2/2): SSO institucional — SAML 2.0** [OPUS + SONNET]
  Adaptador SAML sobre el catálogo de `1.4b` (discriminador `protocol` + tabla hija `saml_identity_provider_settings`), gestión y rotación de certificados de firma, correlación de petición en servidor con consumo atómico (`saml_auth_requests`/`saml_consumed_assertions`). `ADR-043 §10` (reevaluación de `architect`): dependencia `SAML-Toolkits/php-saml` 4.x aprobada tras corregir dos observaciones erróneas de la comparación inicial de `§7.3`. Ocho decisiones del usuario (2026-09-02, todas la recomendada): sin SSO iniciado por el IdP, excepción de CSRF acotada solo al ACS con correlación en servidor, VO propio, clave de firma del SP por fichero (nunca en base de datos), MFA nunca exento.
  Cerrado 2026-09-04: 506 tests Pest + 118 Vitest en verde, `pint`/Larastan/`eslint`/`lint:i18n`/`vue-tsc` limpios. Revisión independiente en dos pasadas de las tres disciplinas: primera pasada con 1 Alta bloqueante coincidente (`security-reviewer`+`doc-reviewer`: cobertura de test del ACS ausente, issue #152) más otros 5 Alta/Media de `db-reviewer`/`doc-reviewer`; al cerrar #152, `test-writer` encontró tres bugs reales más (#155, #156, #157), corregidos por la sesión orquestadora junto con un cuarto propio (`DatabaseSessionHandler::$exists` reutilizado entre sesiones de test). Segunda pasada: `db-reviewer` sin hallazgos nuevos, `doc-reviewer` 2 Media corregidos, `security-reviewer` sin hallazgos Crítico/Alta. Dos incidentes de proceso durante la implementación (subagentes con operaciones de `git` destructivas/no autorizadas sobre trabajo ajeno, issue #150), corregidos en el momento. Mezclado a `develop` vía PR [#160](https://github.com/pirexia/plataforma-educativa/pull/160). Detalle completo en `docs/historial/1.4c-sso-institucional-saml.md`.
- [x] **1.5 · `REQ-PERM`: núcleo de autorización granular** [OPUS + SONNET] ⚠️ *paso crítico*
  Vocabulario cerrado de ámbitos con resolutores por módulo, resolución multi-rol (deny ciego al ámbito), categoría especial en conjunción sobre el rol que concede, CRUD y clonación de roles por API, auditoría de roles/concesiones/asignaciones (issue [#165](https://github.com/pirexia/plataforma-educativa/issues/165)), endpoint de permisos efectivos con procedencia. Sección 11. Diseño en `ADR-044`. **No incluye** interfaz gráfica (ver `1.5b`) ni `RPERM-008` (permisos condicionales, diferido a `1.16`/`REQ-CALIF`).
  Cerrado 2026-09-07: 544 tests Pest en verde, Larastan/Pint limpios. Revisión independiente de las tres disciplinas: `db-reviewer` 1 Alta bloqueante (issue #166, migración sin `$withinTransaction = false`, mismo patrón ya visto en 1.4b/1.4c), corregida; `security-reviewer` sin hallazgos Crítico/Alta, 3 Media de cobertura de test (issues #167-#169, bypass del test de arquitectura de `runAsPlatform()` por nullsafe/llamada dinámica, y dos criterios de aceptación sin test propio), corregidas; `doc-reviewer` 3 Media de cabeceras/manual desincronizados, corregidas. Issue #165 (auditoría, Alta) cerrado. Detalle completo en `docs/historial/1.5-nucleo-autorizacion-granular.md`.
...
- [x] **1.6c · `REQ-BO`: matriz de módulos** [SONNET]
  ...
  Cerrado 2026-09-16: implementado, revisión independiente en dos pasadas (`db-reviewer`/`security-reviewer`/`doc-reviewer`) más dos pasos de prueba de `/codex:review` (`ADR-049 §8`), doce issues encontrados y cerrados (#215/#216/#218-#227). 676/676 Pest en verde. Detalle completo en `CHANGELOG.md` y `docs/modulos/REQ-BO/funcional.md §15.3`. Siguiente sub-paso: `1.6d` (salud y métricas de plataforma).
- [x] **1.6d · `REQ-BO`: salud y métricas de plataforma** [SONNET]
  ...
  Cerrado 2026-09-21: implementado sin ninguna migración, revisión independiente (`db-reviewer`/`security-reviewer`/`doc-reviewer`) más un tercer paso de prueba de `/codex:review` (`ADR-049 §8`) — 3 hallazgos P2 de validación aceptados y corregidos, sin Crítico/Alto en código, 2 Media de deuda de índice declaradas (issues #230/#231) y 2 Baja documentadas (#232/#233). 708/708 Pest en verde. Detalle completo en `CHANGELOG.md` y `docs/modulos/REQ-BO/funcional.md §15.4`/`§15.4.1`. **Evaluación final de la prueba de Codex cerrada en este paso** (`ADR-049 §8.2`/`§8.3`): el umbral de cobertura no se cumple (falló en el calibrado sobre PR #204, 0/4) — por la letra del ADR, corresponde revertir. Pendiente de decisión del usuario (detalle en `memory.md`). Siguiente sub-paso: `1.6e` (motor de *feature flags*).
- [x] **1.6e · `REQ-BO`: motor de *feature flags*** [SONNET]
  `REQ-BO-005` puntos 1-2 (adelanta `REQ-OPS-002`, decisión del usuario 2026-09-08 — es el único punto del requisito que no depende de ningún módulo de fase 2 que falte): activación por tenant/rol/porcentaje, *early adopters*, evaluador en `REQ-CORE`. Último de los cinco, por ser el único que añade código al camino de petición de todos los centros.
  Cerrado 2026-09-22: implementado, revisión independiente (`db-reviewer`/`security-reviewer`/`doc-reviewer` en paralelo) con 1 hallazgo Alto (`GET /api/v1/feature-flags` sin autenticación, `INV-002`) y 5 Media, todos corregidos salvo dos dejados como decisión abierta (`OPEN-BO-27`, límite de la vista previa; issue #238, ambigüedad de `ADR-051` con `permissions.code`) y una Baja documentada (issue #239). 741/741 Pest en verde. Detalle completo en `CHANGELOG.md` y `docs/modulos/REQ-BO/funcional.md §15.5`/`§15.5.1`. **Los cinco sub-pasos de `REQ-BO` (1.6-1.6e) quedan cerrados.** Siguiente paso del plan: Bloque B, `1.7` (Design system).

  **No incluye ninguno de los cinco**: `REQ-BO-003` (planes/facturación, depende de `REQ-SAAS`), impersonación (`REQ-SUP-003`, fase 2, merece paso y ADR propios), ventanas de mantenimiento/avisos (`REQ-OPS-001`+`REQ-COM`, 1.19), exportación durante la baja (`REQ-OPS-004`) ni purga física (`REQ-PRIV-006`) — riesgo aceptado explícitamente por el usuario 2026-09-08, ni interfaz gráfica (posterior a `1.7`/`1.9`, mismo precedente que `1.5b`; los cinco sub-pasos se operan solo por API). Tampoco la cuota de tenant por espacio/miembros activos (issue [#79](https://github.com/pirexia/plataforma-educativa/issues/79)) — decisión del usuario 2026-08-25: sin modelo comercial de planes/tiers todavía, se aborda en un paso propio posterior una vez exista esa decisión (candidato **`1.6z`**, para no colisionar con la numeración de los cinco sub-pasos de arriba).

  División en cinco confirmada por el usuario 2026-09-08 (recomendación de `spec-writer`, orden forzado técnicamente: nada puede escribir sin que exista primero un Super Administrador autenticado). Detalle completo, preguntas resueltas y pendientes (`OPEN-BO-04`, `-06`, `-10`, `-12`, `-13`, ninguna bloqueante) en `docs/modulos/REQ-BO/funcional.md §12`/`§14`.
...
- [ ] **1.7b · Estandarización de módulos: tests de arquitectura y generador** [OPUS + SONNET] · *candidato, no comprometido*
  Añadido como candidato (issue [#163](https://github.com/pirexia/plataforma-educativa/issues/163), 2026-09-04) tras evaluar y descartar la idea de una plantilla de módulo con toda la complejidad. Tres piezas, por orden de valor: (1) tests de arquitectura con Pest `arch()` — hoy hay cero, y con 53 módulos la deriva solo la frena algo que falle en CI (`INV-007`, `ServiceProvider` por convención, prohibición de `varchar`/`timestamp` sin zona de `ADR-029`, `public_id` en todo modelo expuesto), más los cuatro candidatos concretos que añade `ADR-044 §8` (control de acceso por código de rol en vez de permiso; emisión de evento `read` en todo permiso `is_special_category`; consulta de recurso con ámbito restringido solo por la API sancionada; `runAsPlatform()` fuera de su lista de excepciones); (2) generador `php artisan make:module` con el esqueleto **mínimo** más lo que de verdad se repite (`ServiceProvider`, `routes.php`, migración base con `tenant_id` + auditoría + RLS, factory, test de aislamiento entre tenants); (3) módulo de referencia designado en `ARCHITECTURE.md` en vez de plantilla muerta. **Deliberadamente posterior a `1.5`, `1.5b` y `1.7`**: `1.5`/`1.5b` definen la matriz de permisos que usarán los 53 módulos y `1.7` la forma del módulo de frontend; hacerlo antes garantiza rehacerlo. Requiere ADR propio (`CLAUDE.md` §6.3).
