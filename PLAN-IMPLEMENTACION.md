...
- [ ] **1.2b · `REQ-AUTH-005`: panel de sesiones activas, cierre remoto y detección de dispositivo** [SONNET] — **IMPLEMENTADO, PENDIENTE DE REVISIÓN**
  ...
  Especificación aprobada 2026-08-25 (`docs/modulos/REQ-AUTH/*.md`, Parte B) y `ADR-040` (exclusión declarativa de auditoría). Implementado 2026-08-25/26 en `feature/REQ-AUTH-005-1.2b-sesiones-activas` (commit `d8bc5f2`): 2 migraciones RLS, 3 endpoints autoservicio, pantalla `/cuenta/sesiones`, 4 idiomas, OpenAPI. 275 tests backend + 10 frontend en verde, `pint`/`phpstan`/`eslint`/`vue-tsc`/`lint:i18n` limpios, verificado en navegador real (Playwright MCP). Pendiente del pipeline de revisión independiente (`db-reviewer`/`security-reviewer`/`doc-reviewer`) antes de mezclar — no se marca `[x]` hasta entonces. Detalle completo en `memory.md`.
