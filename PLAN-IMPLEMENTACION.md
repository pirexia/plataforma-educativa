...
- [ ] **1.3b · `REQ-AUTH-003`: MFA — correo como segundo factor y excepciones temporales** [SONNET]
  Partido de `1.3` por tamaño (`OPEN-AUTH-24`, 2026-08-26; mismo precedente que `1.2`/`1.2b`, issue #59). Método de correo como segundo factor (desactivado por defecto, `totp` no desactivable — `OPEN-AUTH-25`), excepciones temporales nominales de la obligatoriedad, y pantalla de administración de MFA/roles (`OPEN-AUTH-28`, entra sin condición). Especificación aprobada 2026-08-27 (`docs/modulos/REQ-AUTH/*.md` §D), rama `feature/REQ-AUTH-003-1.3b-mfa-correo-excepciones`.
  **Código completo 2026-08-28** — 4 piezas de 4 (backend+frontend), verificadas por herramienta (`pint`/`phpstan`/Pest 169 tests/`eslint`/`vue-tsc`+build/Vitest/`lint:i18n`, todo limpio) y en navegador real (Playwright). Pendiente solo la revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) y el merge. Ver detalle en `memory.md`.
