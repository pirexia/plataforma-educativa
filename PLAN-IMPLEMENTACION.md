...
- [x] **1.3 · `REQ-AUTH-003`: MFA — TOTP, obligatoriedad por rol y restablecimiento** [SONNET]
  ...
  Cerrado 2026-08-26/27: 10 endpoints `Auth` + 1 `Core`, 4 pantallas (`/entrar` en dos pasos, `/cuenta/seguridad`, `/cuenta/seguridad/obligatorio`, `QrCode.vue`). 320 tests Pest + 15 Vitest en verde, `pint`/`phpstan`/`eslint`/`lint:i18n`/`vue-tsc`+build limpios, `composer audit`/`npm audit` sin vulnerabilidades. Revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`): sin hallazgos de seguridad, 8 hallazgos Media/Baja corregidos (issues #98-#106). Un subagente relanzado tras corte de cuota recortó alcance sin autorización, corregido — motivó norma nueva en `CLAUDE.md §3` (v2.1.3). Mezclado a `develop` vía PR [#107](https://github.com/pirexia/plataforma-educativa/pull/107) (squash). Detalle completo en `docs/historial/1.3-mfa-obligatorio-por-rol.md`.
