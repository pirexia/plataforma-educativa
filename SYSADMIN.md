...
| `AUTH_MFA_MAX_DELIVERIES` | `3` | `apps/api/config/auth-local.php` | Reenvíos admitidos por desafío de segundo factor (`docs/modulos/REQ-AUTH/funcional.md §C.4.4.1`, 1.3). |
...
| `AUTH_MFA_GRACE_DEFAULT_DAYS` | `7` | `apps/api/config/auth-local.php` | Valor por defecto de `tenant_settings.mfa_grace_period_days` en el aprovisionamiento de un tenant nuevo (1.3). |
| `AUTH_MFA_MAX_EXEMPTION_DAYS` | `90` | `apps/api/config/auth-local.php` | Tope de caducidad de una excepción temporal nominal de MFA (`RN-AUTH-68`). **Diferido a `1.3b`**: en `1.3` la variable existe y la lee el esquema, pero ningún endpoint la usa todavía porque `POST /mfa-exemptions` no existe (`docs/modulos/REQ-AUTH/funcional.md §C.4.11`). |
| `AUTH_MFA_FACTOR_PURGE_DAYS` | `30` | `apps/api/config/auth-local.php` | Retención de factores TOTP borrados lógicamente antes de purgarlos físicamente (`docs/modulos/REQ-AUTH/datos.md §C.11`, 1.3). |
| `AUTH_MFA_CHALLENGE_RETENTION_HOURS` | `24` | `apps/api/config/auth-local.php` | Retención de desafíos de segundo factor ya consumidos o caducados (1.3). |
