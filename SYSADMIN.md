...
| `AUTH_MFA_CHALLENGE_TTL_MINUTES` | `5` | `apps/api/config/auth-local.php` | Vida del desafío de segundo factor del login en dos pasos (`RN-AUTH-54`, 1.3). |
| `AUTH_MFA_MAX_ATTEMPTS` | `5` | `apps/api/config/auth-local.php` | Intentos admitidos por desafío y por alta de factor antes de matarlo (`RN-AUTH-54`, `RN-AUTH-59`, 1.3). |
| `AUTH_MFA_ENROLLMENT_TTL_MINUTES` | `10` | `apps/api/config/auth-local.php` | Vida de un alta de factor TOTP sin confirmar (`RN-AUTH-59`, 1.3). |
| `AUTH_MFA_CODE_TTL_MINUTES` | `10` | `apps/api/config/auth-local.php` | Vida del código de segundo factor entregado por correo (1.3). |
| `AUTH_MFA_MAX_DELIVERIES` | `3` | `apps/api/config/auth-local.php` | Tope de entregas (correo) por desafío de segundo factor — al superarlo, `429` sin generar código y sin matar el desafío (`RN-AUTH-79`, `docs/modulos/REQ-AUTH/funcional.md §D.4.3`). Aplicado de verdad desde 1.3b: en 1.3 la variable existía y no la leía nadie (no había método de entrega implementado). |
| `AUTH_MFA_RECOVERY_CODE_COUNT` | `10` | `apps/api/config/auth-local.php` | Códigos de respaldo generados por juego (`docs/modulos/REQ-AUTH/funcional.md §C.4.3`, 1.3). |
| `AUTH_MFA_TOTP_WINDOW` | `1` | `apps/api/config/auth-local.php` | Pasos de tolerancia a cada lado en la verificación TOTP (`RN-AUTH-58`, 1.3). **Con guarda de arranque**: un valor por encima de `2` amplía la ventana de validez de un código a más de dos minutos y medio (un código capturado sirve más tiempo) y `AuthServiceProvider` aborta el arranque de la aplicación. |
| `AUTH_MFA_GRACE_DEFAULT_DAYS` | `7` | `apps/api/config/auth-local.php` | Valor por defecto de `tenant_settings.mfa_grace_period_days` en el aprovisionamiento de un tenant nuevo (1.3); plazo completo también al reabrir una obligación tras una excepción (`RN-AUTH-82`, 1.3b). |
| `AUTH_MFA_MAX_EXEMPTION_DAYS` | `90` | `apps/api/config/auth-local.php` | Tope de caducidad de una excepción temporal nominal de MFA, validado en `POST /mfa-exemptions` (`RN-AUTH-68`/`RN-AUTH-81`, 1.3b, `docs/modulos/REQ-AUTH/funcional.md §D.4.6`). |
| `AUTH_MFA_FACTOR_PURGE_DAYS` | `30` | `apps/api/config/auth-local.php` | Retención de factores (TOTP o correo) borrados lógicamente antes de purgarlos físicamente. Aplicado de verdad desde 1.3b por `PurgeMfaFactors` (`RN-AUTH-85`); en 1.3 la variable existía sin ninguna tarea que la usara (issue [#109](https://github.com/pirexia/plataforma-educativa/issues/109), cerrado). |
| `AUTH_MFA_CHALLENGE_RETENTION_HOURS` | `24` | `apps/api/config/auth-local.php` | Retención de desafíos de segundo factor ya consumidos. Aplicado de verdad desde 1.3b por `PurgeMfaChallenges` (`RN-AUTH-85`, issue [#109](https://github.com/pirexia/plataforma-educativa/issues/109), cerrado). |
| `AUTH_MFA_EXEMPTION_REOPEN_WINDOW_HOURS` | `48` | `apps/api/config/auth-local.php` | Ventana hacia atrás que `ReopenExpiredMfaExemptions` recorre buscando excepciones recién caducadas, sin revocar (1.3b, `docs/modulos/REQ-AUTH/funcional.md §D.4.9`). No es la única garantía: `MfaPolicy::resolve()` reabre la obligación en la siguiente petición del titular aunque el *scheduler* lleve más de 48 horas caído. |
...
| `VITE_API_URL` | `http://localhost:8000/api/v1` (`apps/web/.env`), sobrescrito por `compose.yaml` a `http://demo.plataforma.test:8000/api/v1` | `apps/web/.env` / `compose.yaml` (servicio `web`) | **Requisito de despliegue de `REQ-AUTH` (1.2, issue [#71](https://github.com/pirexia/plataforma-educativa/issues/71)).** La resolución de tenant por subdominio (`ADR-014`) exige que el host de cada petición a la API termine en `TENANCY_BASE_DOMAIN`; un valor de host plano (`localhost`) hace que `TenantHost::slugFrom()` no resuelva ningún tenant y el login por navegador dé `404`. Pero apuntar solo la API al host de tenant **sin mover también la SPA** no basta: `document.cookie` de una página en `localhost` no puede leer una cookie fijada por `demo.plataforma.test` (el puerto no importa, el host sí) — el síntoma pasa de `404` a `419` (CSRF). La SPA debe navegarse por `http://demo.plataforma.test:5173/`, no por `http://localhost:5173/`, y por eso `CORS_ALLOWED_ORIGINS` (fila de arriba) también cambia. Requiere además `apps/web/vite.config.ts` con `server.allowedHosts` (Vite ≥ 6 bloquea con `403` cualquier `Host` no reconocido) y una entrada en el `hosts` del sistema que ejecuta el navegador — en WSL2, el de **Windows**, no el de la distribución: `127.0.0.1 demo.plataforma.test`. Parche temporal fijado a un solo tenant de desarrollo (`demo`); ver el issue para la decisión definitiva pendiente. Además, el `302` del *callback* de Google (`REQ-AUTH-002`, 1.4) es relativo y se resuelve contra el origen de la **API** (`:8000`), no el de la SPA — en este entorno de orígenes separados aterriza en `404` (issue [#141](https://github.com/pirexia/plataforma-educativa/issues/141), sin afectar a producción/*staging*, que comparten origen vía Traefik). |
| `AUTH_OAUTH_DRIVER` | **`none`** | `apps/api/config/auth-local.php` | **`REQ-AUTH-002` (1.4).** Tres valores: `none` (sin proveedor externo, estado seguro de cualquier entorno, no dispara ninguna guarda de arranque), `google` (real) y `fake` (proveedor simulado, `operacion.md §E.10`). **Guarda de arranque**: `fake` fuera de `local`/`testing` aborta la aplicación, en todos los entornos — es la protección contra que un proveedor de identidad simulado llegue a producción (issue [#140](https://github.com/pirexia/plataforma-educativa/issues/140), cerrado con `none` como valor por defecto real). El entorno de desarrollo lo fija a `fake` **explícitamente** en `compose.yaml` (ya no se hereda de un valor por defecto implícito). |
| `AUTH_GOOGLE_CLIENT_ID` | *(vacío)* | `apps/api/config/auth-local.php` | Identificador del cliente OAuth de Google. No es secreto, pero identifica el despliegue. Solo se lee con `AUTH_OAUTH_DRIVER=google`. |
| `AUTH_GOOGLE_CLIENT_SECRET` | *(vacío)* | `apps/api/config/auth-local.php` | **Secreto.** Va en el gestor de secretos de `ADR-037 §7`, nunca en `.env` versionado ni en la unidad Quadlet en claro. Solo se lee con `AUTH_OAUTH_DRIVER=google`. **Guarda de arranque**: `google` sin este secreto aborta el arranque, en todos los entornos — evita que el sistema levante con el botón de Google pintado y todo el mundo termine en `error_proveedor` sin que nadie sepa por qué. Su compromiso no da acceso a ninguna cuenta por sí solo (hace falta además el `code` de un usuario), pero permite suplantar a la aplicación frente a Google. |
| `APP_URL` | — | `apps/api/.env` | **Tercera guarda de arranque de `OAuthEnvironmentGuard`** (`assertGoogleDriverRequiresHttps`), sin variable propia: con `AUTH_OAUTH_DRIVER=google` fuera de `local`, si el esquema de `APP_URL` no es `https`, la aplicación **no arranca**. Google no admite URIs de redirección sin TLS, así que un despliegue así solo podía fallar de todos modos — se prefiere que falle el reinicio a que el centro entero descubra el problema pulsando el botón. |
| `AUTH_OAUTH_STATE_TTL_MINUTES` | `10` | `apps/api/config/auth-local.php` | Vida del `state` y del verificador PKCE en el *payload* de la sesión del servidor (`RN-AUTH-91`, 1.4). |
| `AUTH_RATE_LIMIT_OAUTH_START_PER_IP` | `10` | `apps/api/config/auth-local.php` | Arranques del flujo de Google por IP y minuto (*bucket* `oauth_start_ip`, 1.4). |
| `AUTH_RATE_LIMIT_OAUTH_CALLBACK_PER_IP` | `20` | `apps/api/config/auth-local.php` | *Callbacks* de Google por IP y minuto (*bucket* `oauth_callback_ip`, 1.4). |
| `AUTH_RATE_LIMIT_IDENTITY_PROVIDERS_PER_IP` | `60` | `apps/api/config/auth-local.php` | Consultas de `GET /auth/identity-providers` por IP y minuto (*bucket* `identity_providers_ip`, 1.4) — mismo valor que `GET /auth/csrf-cookie`, porque la pantalla de login lo pide en cada carga. |
| `AUTH_RATE_LIMIT_MFA_CHALLENGE_READ_PER_SESSION` | `30` | `apps/api/config/auth-local.php` | Lecturas de `GET /auth/mfa-challenges` por sesión y minuto (*bucket* `mfa_challenge_read_session`, 1.4) — el desafío que `/entrar/google` recupera tras el *callback* federado (`api.md §E.5b`). |
| `AUTH_SSO_ALLOW_INSECURE_DISCOVERY` | **`false`** | `apps/api/config/auth-local.php` | **`REQ-AUTH-004` (1.4b).** Afloja las guardas 1 (esquema) y 2 (dirección pública) de la validación del documento de descubrimiento. **Guarda de arranque**: `true` fuera de `local`/`testing` aborta la aplicación, en todos los entornos. `compose.yaml` de desarrollo lo fija a `true` **explícitamente** (necesario para el emisor OIDC simulado, `operacion.md §F.10`). |
| `AUTH_SSO_DISCOVERY_TIMEOUT_SECONDS` / `AUTH_SSO_DISCOVERY_MAX_BYTES` / `AUTH_SSO_DISCOVERY_MAX_REDIRECTS` | `5` / `262144` / `3` | `apps/api/config/auth-local.php` | `REQ-AUTH-004` (1.4b). Las guardas 3 y 4 contra SSRF de la validación del documento de descubrimiento (`funcional.md §F.4.2`). |
| `AUTH_SSO_DISCOVERY_REFRESH_DAYS` / `AUTH_SSO_SECRET_EXPIRY_WARNING_DAYS` | `7` / `30` | `apps/api/config/auth-local.php` | `REQ-AUTH-004` (1.4b). Cadencia de `auth:refresh-oidc-discovery` y antelación de `auth:warn-expiring-client-secrets` (`operacion.md §F.4`). |
| `AUTH_SSO_CLOCK_SKEW_SECONDS` / `AUTH_SSO_TOKEN_TIMEOUT_SECONDS` | `120` / `5` | `apps/api/config/auth-local.php` | `REQ-AUTH-004` (1.4b). Tolerancia de reloj al validar `exp`/`iat` del `id_token` (`RN-AUTH-104`) y tiempo de espera del canje de código/`userinfo`. |

**`REQ-AUTH-002` (1.4) añade un paso manual en el alta de cada tenant**: hay que registrar `https://{slug}.{base}/api/v1/auth/oauth/google/callback` en la consola de cliente OAuth de Google **antes** de que ese centro pueda usar el botón "Continuar con Google" — si se olvida, el centro tiene el botón pintado y todos sus usuarios reciben `redirect_uri_mismatch`, sin que el alta de tenant lo detecte (`docs/modulos/REQ-AUTH/operacion.md §E.12.2`). Hay además un tope de URIs registradas por cliente OAuth que hay que verificar en la consola de Google y que actúa como **límite duro del número de centros** con este diseño (decisión `OPEN-AUTH-30`, `funcional.md §E.3`); cuando se acerque, toca revisar la migración a un *callback* central (`funcional.md §E.3.3`). La reversión de una incidencia del proveedor es siempre la misma: `AUTH_OAUTH_DRIVER=none` y reinicio — sin tocar base de datos, sin desplegar, sin retirar credenciales; ningún usuario depende de Google para entrar (`RN-AUTH-96`).

**`APP_KEY` cifra ahora secretos TOTP de usuario (desde `REQ-AUTH-003`, 1.3).** Hasta 1.2b cifraba solo cosas regenerables (*payload* de sesión, cursores de paginación); desde 1.3 cifra también `user_mfa_factors.secret_encrypted` (`docs/modulos/REQ-AUTH/datos.md §C.2`). Perder `APP_KEY`, o restaurarla junto a una copia de base de datos con una clave distinta, deja **todos** los factores TOTP del sistema sin poder verificarse, sin salida por la aplicación — ver la consecuencia completa y el procedimiento de recuperación en `RUNBOOK.md` y en `docs/modulos/REQ-AUTH/operacion.md §C.2.2`. `ADR-037 §7.2` punto 4 ya exigía custodiar `APP_KEY` separada de la copia de la base de datos; desde 1.3 esa obligación tiene consecuencia catastrófica si se incumple, no es ya solo buena práctica.

**Nota de despliegue de 1.3** (`docs/modulos/REQ-AUTH/operacion.md §C.11.1`): el día en que `REQ-AUTH-003` entra en producción, `administrador_centro` y `soporte_plataforma` de **todos** los tenants pasan a estar obligados a tener MFA, con siete días de gracia contados desde ese momento — el atributo `mfa_required = true` lo sembró `ProvisionTenantDefaults` desde 1.1, pero hasta 1.3 nada lo comprobaba. Nadie lo ha pedido: es un efecto del despliegue, no una decisión de ningún centro, y hay que avisar antes y comprobar que cada tenant tiene más de un administrador de centro (uno solo que pierda el dispositivo y los códigos no tiene salida por la aplicación, `RN-AUTH-67`).

**`REQ-AUTH-004` (1.4b) — SSO institucional, `AUTH_SSO_*`** (`docs/modulos/REQ-AUTH/operacion.md §F.2`): a diferencia de Google (1.4), **no hay ningún paso manual de operador al dar de alta un tenant** — cada centro registra su propia `redirect_uri` (`https://{slug}.{base}/api/v1/auth/oauth/oidc/callback`, la misma para cualquier proveedor que catalogue) en **su propio** IdP, desde `/administracion/sso` con sesión de `administrador_centro`, sin tocar consola ni variable de entorno alguna. Variables propias, ninguna secreta (todas con valor seguro por defecto, `apps/api/config/auth-local.php`): `AUTH_SSO_DISCOVERY_TIMEOUT_SECONDS` (`5`), `AUTH_SSO_DISCOVERY_MAX_BYTES` (`262144`), `AUTH_SSO_DISCOVERY_MAX_REDIRECTS` (`3`), `AUTH_SSO_DISCOVERY_REFRESH_DAYS` (`7`), `AUTH_SSO_SECRET_EXPIRY_WARNING_DAYS` (`30`), `AUTH_SSO_CLOCK_SKEW_SECONDS` (`120`), `AUTH_SSO_TOKEN_TIMEOUT_SECONDS` (`5`). **`AUTH_SSO_ALLOW_INSECURE_DISCOVERY`** (por defecto `false`, `true` en `compose.yaml` de desarrollo) es la variable con guarda de arranque: `true` fuera de `local`/`testing` **aborta la aplicación**, en todos los entornos — mismo patrón que `AUTH_OAUTH_DRIVER=fake` (issue [#140](https://github.com/pirexia/plataforma-educativa/issues/140)), porque afloja dos de las cinco guardas contra SSRF de la validación del documento de descubrimiento (`docs/modulos/REQ-AUTH/funcional.md §F.4.2`).

**`APP_KEY` gana responsabilidad por segunda vez** (`docs/modulos/REQ-AUTH/operacion.md §F.2.2`, `§F.11`): desde 1.4b cifra también `identity_provider_secrets.client_secret`, la credencial de cliente de cada proveedor de cada tenant frente a su IdP. Perder `APP_KEY`, o restaurar una copia de base de datos con una clave distinta, deja **el SSO institucional de todos los centros** sin funcionar hasta que cada uno vuelva a cargar su credencial — recuperable, pero es una llamada a todos los centros el día que ocurra. El procedimiento de rotación de `APP_KEY` de `RUNBOOK.md`/`§C.11.1` **tiene que incluir ahora también `identity_provider_secrets`**, no solo `user_mfa_factors`.

**`REQ-AUTH-004` (1.4c) — SSO institucional SAML 2.0, `AUTH_SAML_*`** (`docs/modulos/REQ-AUTH/operacion.md §G.2`): igual que en OIDC, **ningún paso manual de operador al dar de alta un tenant** — cada centro cataloga su propio IdP SAML desde `/administracion/sso`, pegando la URL de sus metadatos o el XML. Variables propias, ninguna secreta salvo la clave de firma (`apps/api/config/auth-local.php`): `AUTH_SAML_METADATA_TIMEOUT_SECONDS` (`5`), `AUTH_SAML_METADATA_MAX_BYTES` (`524288`), `AUTH_SAML_METADATA_MAX_REDIRECTS` (`3`), `AUTH_SAML_METADATA_MAX_DEPTH` (`20`), `AUTH_SAML_METADATA_REFRESH_DAYS` (`7`), `AUTH_SAML_AUTH_REQUEST_RETENTION_HOURS` (`24`), `AUTH_SAML_MIN_CERTIFICATE_KEY_BITS` (`2048`). Reutiliza sin duplicar `AUTH_SSO_CLOCK_SKEW_SECONDS` y `AUTH_SSO_SECRET_EXPIRY_WARNING_DAYS` de 1.4b. **`AUTH_SAML_ALLOW_INSECURE_METADATA`** (por defecto `false`, `true` en `compose.yaml` de desarrollo) es la variable con guarda de arranque: `true` fuera de `local`/`testing` **aborta la aplicación** — mismo patrón que `AUTH_SSO_ALLOW_INSECURE_DISCOVERY`, y aquí es más grave: con `http` admitido, el documento que declara con qué certificado se verifica quién entra en un centro viaja en claro y lo puede reescribir cualquiera en el camino.

**Custodia de la clave privada de firma del SP** (`docs/modulos/REQ-AUTH/operacion.md §G.2.3`, `OPEN-AUTH-44` resuelta salida A): a diferencia del `client_secret` de cada proveedor OIDC (que va cifrado en base de datos, ver más abajo), la clave de firma del SP es **de plataforma, una sola, no por tenant**, y por eso vive fuera de base de datos, en el mecanismo de secretos de `ADR-037 §7`:

- **`AUTH_SAML_SP_SIGNING_KEY_PATH`** y **`AUTH_SAML_SP_SIGNING_CERT_PATH`**: rutas a un fichero de clave privada y su certificado público, montados (nunca en el repositorio, `CLAUDE.md §4`), con la etiqueta `:Z` (SELinux `enforcing`), permisos `0400` y propiedad del usuario del servicio. **Vacías por defecto** — sin ellas, ningún proveedor puede activar `sign_authn_requests` (`409`, `RN-AUTH-128`), pero todo lo demás funciona: firmar el `AuthnRequest` es opcional.
- **Segunda guarda de arranque**: `AUTH_SAML_SP_SIGNING_KEY_PATH` informada pero el fichero no legible ⇒ la aplicación no arranca — una clave configurada y ausente es peor que ninguna clave, porque fallaría en el camino del acceso, uno a uno, sin que nada lo agregue.
- **Sin rotación automática.** Rotarla es reemplazar el fichero y pedir a cada centro con `sign_authn_requests` activo que vuelva a cargar nuestros metadatos de SP (`GET /identity-providers/{publicId}/metadata`) en su IdP — procedimiento manual, en `RUNBOOK.md`.
- **No es la misma responsabilidad que `APP_KEY`**, y es deliberado: `ADR-043 §10.9` decidió esta clave fuera de base de datos precisamente para que `APP_KEY` no ganara una tercera responsabilidad. Un certificado del IdP, en cambio, **es material público** y no se cifra en reposo (`RN-AUTH-127`).
