...
| `CORS_ALLOWED_ORIGINS` | `http://localhost:5173` | `apps/api/config/cors.php` | Orígenes desde los que el navegador puede hacer peticiones con `credentials: 'include'` (issue [#68](https://github.com/pirexia/plataforma-educativa/issues/68)). **Solo importa en desarrollo**: producción/*staging* comparten origen SPA+API vía Traefik (`ADR-028`), sin petición cross-origin de por medio. Lista separada por comas si se añade un puerto de depuración adicional. |
