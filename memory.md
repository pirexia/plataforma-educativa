...
| [#83](https://github.com/pirexia/plataforma-educativa/issues/83) | El patrón de test `withCookie()` + `postJson/getJson/deleteJson` (usado en `SessionEndpointTest.php` al menos, CA-AUTH-004/005/016) no transmite la cookie de sesión — "pasa" por un artefacto de caché de `Store`/`Guard` en memoria dentro de un mismo test, no por round-trip real. Patrón correcto ya en `tests/Pest.php` (`resetSessionState()`/`withSessionCookie()`), usado por los tests de `1.2b`. No se ha auditado/corregido la suite preexistente. | Media |
...
**Rama abierta: `feature/REQ-AUTH-005-1.2b-sesiones-activas`, colgada de `develop`, empujada a `origin` (commit `d8bc5f2`).** Working tree limpio, todo commiteado. `1.2b` implementado y en verde (backend 275 tests, frontend 10 + `eslint`/`vue-tsc`/`lint:i18n`, `pint`/`phpstan` limpios) — ver el resumen completo en "Estado actual".

1. **Lanzar el pipeline de revisión independiente antes de mezclar** (`CLAUDE.md §5.4`, mismo patrón que 1.2): `db-reviewer` (migraciones de `user_known_devices`/`user_sessions`, RLS), `security-reviewer` (autoservicio por identidad en los tres endpoints nuevos, redacción de `session_id`, cookie `pge_device`), `doc-reviewer` (coherencia de `docs/modulos/REQ-AUTH/*.md` Parte B con lo realmente implementado). **No hacer merge ni PR sin que el usuario lo pida.**
2. **Issue [#83](https://github.com/pirexia/plataforma-educativa/issues/83), sin empezar**: auditar y corregir los tests preexistentes (`SessionEndpointTest.php` al menos, CA-AUTH-004/005/016) que usan el patrón roto `withCookie()` + `postJson/getJson/deleteJson` — no transmite la cookie de sesión, "pasan" solo por un artefacto de caché de `Store` en memoria dentro de un mismo test. El patrón correcto (`withCredentials()`+`withUnencryptedCookie()`+`resetSessionState()`) ya está en `tests/Pest.php`.
3. **Issue [#78](https://github.com/pirexia/plataforma-educativa/issues/78), sin empezar, no bloquea nada**: el usuario pidió buscar en internet software existente de gestión de AMPA/asociaciones de centros educativos antes de especificar la generalización de `REQ-GOB-004`.
4. **Requisito de entorno para probar en navegador real** (issue #71): `apps/web/vite.config.ts` y `compose.yaml` sirven la SPA desde `demo.plataforma.test:5173`; necesita `127.0.0.1 demo.plataforma.test` en el `hosts` de **Windows** (no el de WSL2) — ya confirmado presente en esta máquina de desarrollo (usado para verificar `1.2b` con Playwright MCP).
5. **Nota de entorno, sigue aplicando**: `.env` no editable desde esta sesión (bloqueo de permisos). Variables inline en cada `Bash`, `podman exec <servicio> printenv`. `SESSION_LIFETIME=480`, `CORS_ALLOWED_ORIGINS` y `VITE_API_URL` siguen parcheados en `compose.yaml` en vez de en sus `.env` respectivos — trasladarlos si algún día `.env` es editable desde la sesión.
