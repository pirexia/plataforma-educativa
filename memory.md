...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver abajo). **Fase 1, bloque A: 1.1 cerrado. 1.2 en curso: espec aprobada, implementación sin empezar.**
**Paso activo**: **1.2 · `REQ-AUTH`: autenticación local y sesiones `[SONNET]` — ESPECIFICACIÓN APROBADA (2026-08-22), CÓDIGO SIN EMPEZAR.** Cerrado por límite de cuota justo antes de crear la rama de implementación. Ver "Siguiente paso concreto" para retomar sin perder contexto.
**1.1 · `REQ-CORE`: tenants y usuarios — CERRADO Y MEZCLADO** (2026-08-21/22). PR [#56](https://github.com/pirexia/plataforma-educativa/pull/56), *squash* a `develop` (commit `d32e4e9`). 76 `CA-CORE-*` con test, 183/183 en verde. **Nota honesta pendiente**: no se pudo verificar el resultado de `ci-api.yml`/`ci-web.yml` sobre el PR antes de mezclar (token `gh` sin permiso `checks`/`actions:read`, 403). Revisar en la interfaz de GitHub cuando haya ocasión.
**Rama**: `feature/REQ-1.2-auth-local-sesiones`, creada esta sesión, con un commit (espec + `ADR-039`), empujada a `origin`. `develop` sigue limpia y sincronizada. Sin *worktrees* abiertos (las tres `worktree-agent-*` huérfanas de sesiones anteriores, sin trabajo único, se borraron esta sesión).
...
**Retomar 1.2 (`REQ-AUTH`) directamente en implementación. La especificación está aprobada y cerrada, no hay que releerla entera — solo lo que haga falta al tocar cada pieza.**

1. **Crear la rama `feature/REQ-1.2-auth-local-sesiones` desde `develop`** (limpia, sin nada pendiente de commitear salvo lo que esta sesión de cierre ya deja listo — ver punto 2). Es lo único que faltaba antes de escribir código cuando se cortó la cuota.
2. **Espec cerrada en `docs/modulos/REQ-AUTH/{funcional,datos,api,permisos,operacion}.md`** (aprobada 2026-08-22, sección 14 de `funcional.md`). Resumen para no releerla entera:
   - 6 pantallas: login, activación, solicitud de recuperación, restablecimiento, cuenta bloqueada, cambio de contraseña (`/cuenta/contrasena`). Austeras, sobre Tailwind actual, sin esperar a 1.7.
   - 9+1 endpoints: `GET /auth/csrf-cookie`, `POST`/`DELETE /auth/session` (login/logout), `POST /auth/invitation-redemptions`, `POST /auth/password-reset-requests`, `POST /auth/password-resets`, `POST /auth/account-unlocks`, `GET`/`DELETE /account-lockouts[/{public_id}]`, `POST /auth/password-changes`.
   - Tablas nuevas: `login_attempts` (append-only, 90 días), `account_lockouts` (con `expires_at` para la caducidad automática de `AUTH_LOCKOUT_MINUTES`, 15 por defecto). `password_reset_tokens`: expand con `token_hash` SHA-256. `tenant_settings.session_timeout_minutes` (NOT NULL DEFAULT 30, CHECK 5-480).
   - Issue #18: repositorio propio de recuperación, **nada de `PasswordBroker`/`Facades\Password`/`DatabaseTokenRepository`** de Laravel — hay que escribir el test de arquitectura que lo prohíbe.
   - Issue #8: `SESSION_DOMAIN` debe quedar vacío (host-only) en todos los entornos — guarda de arranque que aborte si alguien lo rellena.
   - `ADR-039` (nuevo, `docs/adr/ADR-039-vocabulario-auth-en-audit-logs.md`, ACEPTADA): migración con dos `ALTER` aditivos sobre `audit_logs` (`event` +3 valores, `actor_type` +1, `'anonymous'`) — **va en `apps/api/app/Modules/Auth/Database/migrations/`, es lo primero a implementar porque el resto de la auditoría de login/logout depende de ella**. También exige tocar `AuditActor::resolveType()` (nueva rama `'anonymous'` antes de caer en `'system'`).
   - `REQ-CORE/api.md §4` ya corregido (esta sesión) para no contradecir la forma real del canje (`POST /auth/invitation-redemptions`, token en el cuerpo).
3. **Empezar por la migración de `ADR-039`** (bloquea la auditoría de `login`/`logout`/`password_reset_requested`, nada más depende de ella salvo esa parte), luego el resto en el orden que tenga sentido técnico (repositorio de contraseña → login/logout → bloqueo → recuperación → cambio auto-servicio → pantallas).
4. Issues de planificación abiertos esta sesión, no bloquean 1.2: [#58](https://github.com/pirexia/plataforma-educativa/issues/58) (`REQ-AUTH-004` SSO sin paso, ahora `1.4b` en el plan) y [#59](https://github.com/pirexia/plataforma-educativa/issues/59) (resto de `REQ-AUTH-005` diferido a `1.2b`).
5. Preguntas abiertas de la espec que **no** bloquean y llevan recomendación ya escrita (no reabrir): `OPEN-AUTH-04` (sin endpoint de comprobación previa de token), `-07` (`0.10c` bloquea *operar* el piloto, no implementar/probar), `-09` (sin "recordarme"), `-10` (`sessions` sin `tenant_id`/RLS hasta 1.2b, issue Media a abrir en 1.2b), `-11` (aislamiento de cookie con TLS real no verificable hasta `0.10b`/`0.10e`, anotar en `SYSADMIN.md §6`).
6. **Nota de entorno de sesiones anteriores, sigue aplicando**: en el *worktree* de esta rama no se puede crear `.env` (bloqueo de permisos más amplio de lo declarado). Pasar las variables de conexión inline en cada `Bash` (`DB_HOST=127.0.0.1`, `DB_PORT=5432`, `DB_DATABASE=plataforma_test`, `DB_USERNAME=plataforma_app`, `REDIS_HOST=127.0.0.1`, `REDIS_PORT=6379`, más `APP_KEY`/contraseñas — `podman exec plataforma-api printenv` para obtenerlas). Crear `storage/framework/{views,cache/data,sessions,testing}`, `storage/logs`, `bootstrap/cache` con `mkdir -p` si faltan.
7. Los issues previos de `Problemas abiertos` no bloquean nada del trabajo actual; están correctamente diferidos a los pasos donde existe el código que los necesita (1.3/1.5/1.6/`REQ-BO-001`/`0.10d`).
