...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2 y 1.2b cerrados y mezclados. `1.3` (MFA — TOTP, obligatoriedad por rol, restablecimiento): especificación APROBADA 2026-08-26, partida en `1.3`/`1.3b`. Pendiente: ADR de las dos dependencias nuevas, luego implementación (ver "Siguiente paso concreto").**
...
- `OPEN-AUTH-19`/`20` → aprobadas `pragmarx/google2fa` (backend) y librería QR (frontend), envueltas tras interfaz propia. **Falta la comprobación formal de `CLAUDE.md §1` (mantenimiento/licencia/*releases*) en un ADR antes de implementar** — es el siguiente paso de esta sesión.
...
1. **Commitear** la aprobación de la especificación y la partición del plan antes de seguir (commit de documentación, referencia `REQ-AUTH-003`).
2. **ADR antes que `implementer`**: lanzar `architect` (Opus) para comprobar `CLAUDE.md §1` (mantenimiento activo, licencia, frecuencia de *releases*) de `pragmarx/google2fa` (backend) y de la librería de QR elegida para la SPA (frontend), y escribir el ADR correspondiente (`028` en adelante, fichero propio en `/docs/adr/`). Mismo patrón que `ADR-040` en `1.2b`. **No empezar la implementación sin este ADR.**
3. **Implementación de `1.3`** (subagente `implementer`, Sonnet) con el alcance fijado en `docs/modulos/REQ-AUTH/funcional.md §C.16`: TOTP, códigos de respaldo, login en dos pasos, `MfaPolicy`, gracia y muro, `PATCH /roles/{public_id}` acotado, vista previa/cumplimiento, restablecimiento por admin. Revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) antes de mezclar, como en `1.2`/`1.2b`.
4. **`1.3b`** (correo como 2FA, excepciones temporales, pantalla de administración si `1.5` se retrasa) es un paso posterior con **rama propia nueva** — no confundir con esta rama.
5. **Requisito de entorno para probar en navegador real** (issue #71): `apps/web/vite.config.ts` y `compose.yaml` sirven la SPA desde `demo.plataforma.test:5173`; necesita `127.0.0.1 demo.plataforma.test` en el `hosts` de **Windows** (no el de WSL2) — ya confirmado presente en esta máquina de desarrollo.
6. **Nota de entorno, sigue aplicando**: `.env` no editable desde esta sesión (bloqueo de permisos). Variables inline en cada `Bash`, `podman exec <servicio> printenv`. `SESSION_LIFETIME=480`, `CORS_ALLOWED_ORIGINS` y `VITE_API_URL` siguen parcheados en `compose.yaml` en vez de en sus `.env` respectivos — trasladarlos si algún día `.env` es editable desde la sesión.
