...
**Paso activo**: **1.1 · `REQ-CORE`: tenants y usuarios** `[OPUS + SONNET]`. Especificación aprobada. **Implementación funcionalmente COMPLETA** (los 76 `CA-CORE-*` tienen test que los referencia; 181/181 tests de `apps/api` en verde, frontend con `vue-tsc`/`eslint`/`lint:i18n`/`vitest`/`build` en verde). **Queda pendiente cerrar el paso**: revisión independiente `security-reviewer`/`doc-reviewer` lanzada en esta sesión, resultado sin procesar todavía (ver "Siguiente paso concreto" — es literalmente lo primero que hay que mirar al retomar). Detalle de qué se hizo en "Trabajo en curso".
**Rama**: `feature/REQ-1.1-core-tenants-usuarios-cont`, en el *worktree* `.claude/worktrees/agent-a8c0eca2242941d64`, arrancada desde el mismo commit base (`eb8e10a`) que `feature/REQ-1.1-core-tenants-usuarios` (esa rama sigue en uso en el checkout principal; git no permite la misma rama en dos *worktrees*, de ahí el nombre `-cont`, sucesor del `-wt` de la sesión anterior). **La orquestadora ya avisó que gestiona ella la fusión/consolidación de ramas — no tocarlo, centrarse en el código.** `develop` intacta.
...
- **1.1: cinco commits de esta sesión** (rama `feature/REQ-1.1-core-tenants-usuarios-cont`, del más antiguo al más reciente):
  ...
  - **181/181 tests de `apps/api` en verde** (176 backend + los nuevos), frontend con `vue-tsc -b`, `eslint`, `npm run lint:i18n`, `vitest run` y `npm run build` en verde.
  - **`CA-CORE-*`**: los 76 tienen test que los referencia. Ninguno pendiente.
  - **Revisión independiente**: `security-reviewer` y `doc-reviewer` lanzados como subagentes en paralelo al cierre de esta sesión, sobre el rango `eb8e10a..HEAD` (los 5 commits de esta sesión). **Resultado sin procesar todavía** — si retomas y ves esto sin actualizar, comprueba primero si las notificaciones de esos dos agentes llegaron; si no, vuelve a lanzarlos con el mismo rango antes de dar el paso por cerrado.
...
| [#51](https://github.com/pirexia/plataforma-educativa/issues/51) | Larastan no reconoce las columnas de `TenantMigration::tenantTable()`: 228 errores `property.notFound` en todo el proyecto, preexistentes desde 0.7/0.8. Ningún commit anterior pasó análisis estático limpio de verdad. Requiere sesión propia (extensión de Larastan, `@property` explícitos, o `ide-helper`). | Media |
| [#52](https://github.com/pirexia/plataforma-educativa/issues/52) | 4 ficheros preexistentes de `REQ-CORE` no pasan `pint --test` (cosmético, `pint` sin `--test` los arregla solo). | Baja |

---

## Siguiente paso concreto

**1.1 está funcionalmente completo. Lo único que falta para cerrar el paso de verdad:**

1. **Recoger el resultado de `security-reviewer`/`doc-reviewer`**, lanzados en paralelo al final de esta sesión sobre `eb8e10a..HEAD` (los 5 commits nuevos: activos de marca, importación+idempotencia+purgas+interfaces+eventos, tests transversales, OpenAPI, cliente TS). Si la notificación ya llegó, procesar sus hallazgos (corregir Media/Alta/Crítica en la misma sesión; anotar Baja como issue sin resolver, `CLAUDE.md §5`). Si no lanzaste tú esos agentes y no hay notificación pendiente, vuelve a lanzarlos con el mismo rango antes de dar el paso por cerrado — **no autorrevisar como sustituto**, es la lección repetida de 0.8/0.9/0.9b.
2. Tras aplicar (o descartar razonadamente) los hallazgos de la revisión: **fusionar/renombrar la rama** `feature/REQ-1.1-core-tenants-usuarios-cont` — la orquestadora dijo que lo gestiona ella, pero confirmar antes de dar 1.1 por cerrado que ha ocurrido (no debe quedar una rama `-cont` colgando igual que no debía quedar la `-wt`).
...
4. Los issues [#51](https://github.com/pirexia/plataforma-educativa/issues/51) (Media, Larastan/`TenantMigration`) y [#52](https://github.com/pirexia/plataforma-educativa/issues/52) (Baja, `pint`) quedan abiertos a propósito — no bloquean 1.1, pero sí conviene decidir cuándo se atacan (candidato: antes de que el número de módulos haga el problema de `#51` más caro de arreglar).
