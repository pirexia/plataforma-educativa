...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver abajo). **Fase 1, bloque A: 1.1 cerrado. Siguiente: 1.2.**
**Paso activo**: **1.1 · `REQ-CORE`: tenants y usuarios `[OPUS + SONNET]` — CERRADO Y MEZCLADO** (2026-08-21/22). PR [#56](https://github.com/pirexia/plataforma-educativa/pull/56), *squash* a `develop` (commit `d32e4e9`), rama `feature/REQ-1.1-core-tenants-usuarios` borrada local y remota. Implementación completa (76 `CA-CORE-*` con test, 183/183 en verde, PHPStan 0 errores tras arreglar un hueco de herramienta preexistente desde 0.7 — issue #51 —, revisión independiente `security-reviewer`/`doc-reviewer` sin hallazgos Alto/Crítico). **Nota honesta**: no se pudo verificar el resultado de `ci-api.yml`/`ci-web.yml` sobre el propio PR antes de mezclar — el token de `gh` no tiene permiso para leer *check runs* (`gh pr checks`/`gh api .../check-runs` dan 403 "Resource not accessible"). Se mezcló confiando en la verificación local exhaustiva (misma suite, mismos comandos), no en el semáforo verde de GitHub, que no se pudo ver. `branch protection` con *required status checks* sigue sin activar desde 0.6, así que esto tampoco bloqueó el merge por sí solo. **Sesión siguiente: si hay ocasión, revisar en la interfaz de GitHub que los workflows de `#56` corrieron en verde de verdad, y si el token de `gh` puede ampliarse con permiso de `checks`/`actions:read`.**
**Rama**: `develop`, limpia y sincronizada. Sin ramas de trabajo ni *worktrees* abiertos.
