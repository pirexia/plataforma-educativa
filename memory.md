...
- **`1.4c` · `REQ-AUTH-004` (parte 2/2, SAML 2.0) — EN CURSO, verificado hasta `630b895` (2026-09-04).** Backend y frontend completos, con los 56 `CA-AUTH-311`-`366` a test real (`INV-015`). Los tres hallazgos que dejó la tanda anterior (#155, #156, #157) más uno nuevo destapado al resolverlos, los cuatro corregidos y verificados yo mismo, no de palabra:
  - **#155** (Alta): `FakeSamlIdentityProviderController` no firmaba el `<samlp:Response>`, solo la `Assertion` — corregido firmando los dos nodos. Causa raíz añadida al investigar: `FakeSamlKeyMaterial` cacheaba con `Cache::rememberForever()` bajo `CACHE_STORE=redis` forzado solo en `phpunit.xml`, pero el flujo cruza el proceso de Pest y el `artisan serve` persistente del contenedor — cada uno generaba su propia clave. Pasado a fichero compartido en `storage/framework/testing/` con `flock()`.
  - **#156** (Alta): `UpdateIdentityProviderRequest::targetProtocol()` comparaba `is_string()` contra un valor que Eloquent ya devolvía casteado a enum — siempre falso, `PATCH` nunca aplicaba campos SAML. Corregido.
  - **#157** (Media): `datos.md §G.5` decía que la huella del certificado sí se auditaba, citando en la misma frase dos secciones de `ADR-043` que dicen justo lo contrario — `datos.md` era el equivocado, no `funcional.md`. Corregidos el modelo (`fingerprint_sha256` a `$auditSecretAttributes`) y `datos.md`.
  - **Nuevo, encontrado al destaparse los anteriores**: `CA-AUTH-354` (vínculo MFA pendiente) fallaba por `DatabaseSessionHandler::$exists` —bandera de instancia, no por sesión— reutilizada entre peticiones de sesiones distintas dentro del mismo test: el `INSERT` de la sesión del ACS se convertía en silencio en un `UPDATE` de cero filas. Arreglo de test únicamente (`resetSessionState()` en `completeSamlFlow()`), sin tocar producción.

  **Suite completa verificada por ejecución real tras las cuatro correcciones: 506/506 Pest en verde** (`php -d memory_limit=512M ./vendor/bin/pest`, issue #106), **118/118 Vitest sin cambios**. Commits de arreglo: `df6c931`, `b6249bb`, `b56ef1f`, `630b895`. Issues #152/#155/#156/#157 **ya cerrados en GitHub** (2026-09-03/04). Segunda pasada de revisión independiente sobre el código corregido: `db-reviewer` sin hallazgos nuevos; `doc-reviewer` 2 Media (cabeceras "pendiente de aprobación" desincronizadas en 4 de 5 documentos de la Parte G, y esta misma nota afirmando issues sin cerrar cuando ya lo estaban) — ambos corregidos. **Qué falta con certeza**: `security-reviewer` (segunda pasada) todavía en curso; completar `CHANGELOG.md` con las cifras finales al cerrar el paso; mezclar.
