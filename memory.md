...
**Fase**: 0 — Cimientos
**Paso activo**: 0.1-0.6 cerrados. **0.7 (núcleo multi-tenant, paso crítico) EN IMPLEMENTACIÓN.** Diseño aprobado (`ADR-033`). De los 11 subpasos del plan del ADR, **0.7.1 terminado y commiteado**; 0.7.2 (`TenantContext`) empezado pero sin código todavía (solo exploración de `app/Support/`); 0.7.3-0.7.11 sin empezar. Ver `Trabajo en curso` para el detalle exacto de dónde retomar.
**Rama**: `feature/paso-0.7-nucleo-multitenant`, creada a partir de la punta de `chore/checkpoint-sesion-0.7` (que a su vez cuelga de `develop`, PR #5 todavía abierto y sin mezclar). `develop` sigue en `8b4c790`, sin cambios. Cuando se mezcle, primero `chore/checkpoint-sesion-0.7` → `develop`, después `feature/paso-0.7-nucleo-multitenant` → `develop` (mismo orden en que se crearon, para que la historia quede lineal).
**Última sesión (2026-08-17)**: aprobado `ADR-033` por el usuario ("ejecútalo"). Creada rama de implementación y arrancados los 11 subpasos del ADR con seguimiento por tareas (herramienta `Task*`, que se desconectó a mitad de sesión — de aquí en adelante el seguimiento de progreso de 0.7 vive **solo en este fichero**, sección `Trabajo en curso`). **0.7.1 cerrado** (commit `eb1baa3`): esquema `app`, función `app.current_tenant_id()`, tres roles PostgreSQL (`plataforma_owner`/`plataforma_app`/`plataforma_platform`, ninguno `SUPERUSER`) vía `infra/containers/postgres/init/01-tenancy.{sh,sql}`, aplicado a mano sobre el volumen de desarrollo existente y verificado (`\du` muestra los atributos correctos, la API ya conecta como `plataforma_app`). Documentado en `SYSADMIN.md` §2b. Excepción añadida a `.gitignore` (`*.sql` es global para volcados de datos, no para scripts de esquema versionados). Sesión previa (antes de esto) había cerrado el diseño (`ADR-033`) y corregido una discrepancia documental `ADR-024`/`ADR-027`/`ADR-030` no relacionada con 0.7 (detalle en `CHANGELOG.md` 2026-08-17), más el fix de filtros `paths:` en CI (PR #5, todavía por revisar/mezclar) y quedan pendientes los pasos manuales de GitHub (branch protection, Dependabot alerts, Renovate App — falló por error transitorio, permiso "Checks" del PAT).
...
- **0.7 en implementación** (rama `feature/paso-0.7-nucleo-multitenant`). Diseño en `docs/adr/ADR-033-implementacion-del-aislamiento-multi-tenant.md`, sección "Plan de implementación" (11 subpasos, commits independientes). Checklist real de esta rama:
  ...
  - [ ] **0.7.2** `TenantContext` (`app/Support/Tenancy/TenantContext.php`, namespace mirroring `App\Support\Modules\...`): `enter/leave/runFor/tenantId()`, `tenantId()` lanza `TenantContextMissing` si no hay tenant. **Sin código escrito todavía** — solo se había mirado `app/Support/` (contiene únicamente `Modules/ModuleServiceProviderDiscovery.php`) y `app/Providers/AppServiceProvider.php` (vacío, plantilla por defecto) para decidir dónde engancharlo. Empezar por aquí.
  - [ ] **0.7.3** Conexiones `pgsql` (ya reapunta a `plataforma_app` desde 0.7.1) + nuevas `pgsql_owner` (`plataforma_owner`, para migraciones vía `--database=pgsql_owner`) y `pgsql_platform` (`plataforma_platform`, BYPASSRLS) en `config/database.php`, usando las variables `DB_OWNER_*`/`DB_PLATFORM_*` ya presentes en `apps/api/.env`/`.env.example` desde 0.7.1. Listener de `ConnectionEstablished` que reaplica `set_config('app.tenant_id', ?, false)` con parámetro ligado.
  - [ ] **0.7.4** Tabla `tenants` + modelo. Política RLS propia.
  - [ ] **0.7.5** Middleware `ResolveTenant`.
  - [ ] **0.7.6** `TenantModel` + `BelongsToTenant` + `TenantScope`.
  - [ ] **0.7.7** Colas conscientes de tenant.
  - [ ] **0.7.8** Redis y almacenamiento con prefijo de tenant.
  - [ ] **0.7.9** `config/tenancy.php` (tablas compartidas) + helper `tenantTable()` de migración.
  - [ ] **0.7.10** Suite de tests de `apps/api/phpunit.xml` de SQLite a PostgreSQL real; servicio `postgres:17` en `ci-api.yml`.
  - [ ] **0.7.11** Los diez tests de `tests/Feature/Tenancy/` (tabla completa en el ADR, sección 10).
