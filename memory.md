...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2, 1.2b, 1.3, 1.3b y 1.4 cerrados y mezclados.** Bloque A completo. Siguiente paso: **`1.4b`**.
...
**Rama activa**: `develop`, limpia y sincronizada con `origin` tras el merge de `1.4`. Sin rama de trabajo abierta — toca crear una nueva para `1.4b`.
...
**`1.4` cerrado y mezclado el 2026-09-01** (PR [#143](https://github.com/pirexia/plataforma-educativa/pull/143)). Detalle completo en `docs/historial/1.4-google-login-fusion-cuentas.md`. Rama local y remota borradas. **Bloque A de la fase 1 (`1.1`-`1.4`) completo.**

**Siguiente paso del plan**: **`1.4b · REQ-AUTH-004: SSO institucional (SAML 2.0 / OIDC)`** [OPUS + SONNET] — issue [#58](https://github.com/pirexia/plataforma-educativa/issues/58). Etiquetado OPUS+SONNET a propósito: valorar con `architect` el impacto en el modelo de identidad cerrado en 1.1/1.4 antes de especificar (SAML 2.0 + OIDC para Azure AD/Entra ID y Google Workspace, mapeo de atributos, *just-in-time provisioning* — esto último es justo lo que 1.4 dejó fuera a propósito en `OPEN-AUTH-31`, y donde reaparecen "apellidos"/"foto" que 1.4 no tuvo que resolver).

**Nota para el arranque de `1.4b`**: el token del MCP de GitHub usado en sesiones anteriores parecía no tener permiso para leer *checks*/Actions (`403` con `gh` CLI); resultó ser que las llamadas del MCP (`mcp__github__pull_request_read` método `get_check_runs`/`get_status`) sí funcionan y usan credenciales distintas a las del `gh` CLI del shell — usar esas, no `gh api .../check-runs` ni `gh run list`, que siguen dando 403.
