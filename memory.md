...
| [#109](https://github.com/pirexia/plataforma-educativa/issues/109) | 4 tareas de mantenimiento de MFA (`PurgeMfaChallenges`/`PurgeMfaEnrollments`/`PurgeMfaFactors`/`MaterializeMfaObligations`) declaradas en `operacion.md §C.4` desde 1.3 pero ausentes del código. **Incorporado al alcance de `1.3b`** (decisión del usuario 2026-08-27, `OPEN-AUTH-29`), no es trabajo pendiente aparte. | Media |
...
**`1.3b` en curso** (`REQ-AUTH-003`, correo como 2FA + excepciones temporales + pantalla mínima de administración + las 4 tareas de mantenimiento de #109), rama `feature/REQ-AUTH-003-1.3b-mfa-correo-excepciones`. Especificación aprobada y commiteada (`8b4e6df`, Parte D de los 5 ficheros de `docs/modulos/REQ-AUTH/`), con las tres preguntas abiertas resueltas (`OPEN-AUTH-27` solo a nivel de tenant, `OPEN-AUTH-28` sí incluir pantalla, `OPEN-AUTH-29` las 4 tareas dentro de esta rama). `architect` valorado y descartado a propósito para este paso: no cumple ninguno de los tres disparadores de `CLAUDE.md §2` (sin dependencia nueva, sin cambio al modelo núcleo, sin alterar un ADR existente).

1. **Implementación en curso, con un corte por límite de cuota ya ocurrido** (primer `implementer`, cortado a mitad de la pieza 1 — correo como segundo factor). Relanzado un segundo `implementer` (2026-08-27) con instrucciones explícitas de no recortar alcance y de revisar el diff sin commitear que dejó el primero antes de darlo por bueno (`CLAUDE.md §3`). Migración ya commiteada (`9b60629`). **Contrastar lo entregado contra la especificación de la Parte D antes de aprobar**, no dar por bueno el resumen del subagente sin más.
2. **Tras cerrar `1.3b`** (mezclado a `develop`, cierre de sesión completo por `CLAUDE.md §3`): siguiente paso es un `chore/` dedicado para corregir los 4 issues de vigencia de documentación abiertos en la auditoría de 2026-08-27 (`#111` `SECURITY.md`, `#112` `README.md`, `#113` `docs/REQUISITOS-...md`, `#114` `PRIVACY.md`). Decisión explícita del usuario de no interrumpir `1.3b` para esto.
3. **Requisito de entorno para probar en navegador real** (issue #71): la SPA se sirve desde `demo.plataforma.test:5173`; necesita `127.0.0.1 demo.plataforma.test` en el `hosts` de **Windows** — confirmado presente.
4. **Nota de entorno**: `compose.yaml` ya fija `target: dev` en `api`/`web` (issue #96, cerrado). Contenedor `api`: `podman exec -w /var/www/html plataforma-api <cmd>`. Contenedor `web`: **`WORKDIR` es `/app`, no `/var/www/html`** — `podman exec -w /app plataforma-web <cmd>`. Suite Pest completa necesita `php -d memory_limit=512M` (128M por defecto no basta, issue #106, ajeno a `Auth`).
