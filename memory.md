...
**Siguiente paso concreto de 1.2b**: `architect` redactando `ADR-040` (exclusión del observer de auditoría para `UserSession`/login). Al terminar, lanzar `implementer` para construir 1.2b en `feature/REQ-AUTH-005-1.2b-sesiones-activas` siguiendo la Parte B de la especificación + las 5 decisiones. Después, pipeline habitual antes de mezclar: `db-reviewer` (hay migraciones nuevas), `security-reviewer`, `doc-reviewer`.
...
| [#78](https://github.com/pirexia/plataforma-educativa/issues/78) | Generalizar `REQ-GOB-004` (AMPA) a módulo de varias asociaciones internas por tenant, con roles propios y admin delegado (centro crea, no administra el interior). Requiere investigación de mercado + `architect`. Idea del usuario 2026-08-25. | Planificación |
