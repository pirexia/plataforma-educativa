...
1. **Confirmar que la rama se fusionó/consolidó**: `feature/REQ-1.1-core-tenants-usuarios-cont` — la orquestadora dijo que lo gestiona ella, pero comprobar antes de dar 1.1 por cerrado que ha ocurrido (no debe quedar una rama `-cont` colgando igual que no debía quedar la `-wt` de la sesión anterior).
...
4. Los issues [#51](https://github.com/pirexia/plataforma-educativa/issues/51) (Media, Larastan/`TenantMigration`), [#52](https://github.com/pirexia/plataforma-educativa/issues/52) (Baja, `pint`) y [#54](https://github.com/pirexia/plataforma-educativa/issues/54) (Baja, `SvgSanitizer`) quedan abiertos a propósito — no bloquean nada, pero conviene decidir cuándo se atacan (`#51` candidato a antes de que el número de módulos lo haga más caro de arreglar).
