...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2, 1.2b, 1.3 y 1.3b cerrados y mezclados.** Bloque A completo. Siguiente paso: `chore/` de vigencia de documentación (ver "Siguiente paso concreto"), y después `1.4`.
...
**Rama activa**: `develop`, limpia y sincronizada con `origin`. Sin rama de trabajo abierta — toca crear una nueva para el `chore/` de documentación.
...
| [#115](https://github.com/pirexia/plataforma-educativa/issues/115) | `MfaResetService::reset()` lanzaba el 403 de autorrestablecimiento sin `detailKey`. **Resuelto en el commit `c7f276a`** (1.3b, pieza 2); issue cerrado en GitHub el 2026-08-31 (había quedado abierto por descuido). | Resuelta y cerrada |
...
**`1.3b` cerrado y mezclado el 2026-08-31** (PR [#123](https://github.com/pirexia/plataforma-educativa/pull/123), commit `dd68f48`). Detalle completo de las 4 piezas, la cronología y la revisión independiente en `docs/historial/1.3b-mfa-correo-excepciones.md`. Rama local y remota borradas. **Bloque A de la fase 1 (`1.1`-`1.3b`) completo.**

**Siguiente paso, según decisión ya registrada del usuario (2026-08-27)**: un `chore/` dedicado para los 4 issues de vigencia de documentación raíz abiertos en la auditoría de esa fecha — [#111](https://github.com/pirexia/plataforma-educativa/issues/111) (`SECURITY.md` describe un producto sin auth/MFA/permisos que ya existen desde 1.1-1.3), [#112](https://github.com/pirexia/plataforma-educativa/issues/112) (`README.md` desactualizado), [#113](https://github.com/pirexia/plataforma-educativa/issues/113) (`docs/REQUISITOS-...md` sin la entrada 3.1.1 en su propio historial), [#114](https://github.com/pirexia/plataforma-educativa/issues/114) (`PRIVACY.md §2.1` sin la cookie de sesión/`XSRF-TOKEN` de 1.2 en el RAT). Después de ese `chore/`, siguiente paso del plan es **`1.4 · REQ-AUTH: login con Google y fusión de cuentas`** [SONNET].
