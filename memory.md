...
7. **Nada de OpenAPI, `pint`, `phpstan` pendiente para lo ya hecho**: las tres piezas commiteadas pasan `pint --test`/`phpstan` limpios y la suite completa de `Auth` (169 tests) en verde. **Sí queda pendiente, para el cierre final del paso completo**: `security-reviewer`/`db-reviewer`/`doc-reviewer` (no se han ejecutado todavía — se reservan para cuando la pieza 3 esté escrita, no antes), y el frontend (`eslint`/`vue-tsc`+build/Vitest/Playwright) no se ha tocado en absoluto porque la pieza 3 es la única con código de frontend.
