...
12. **Siguiente paso inmediato (2026-08-28)**: código completo y verificado (herramienta + navegador). Falta lanzar la revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) sobre las 4 piezas, corregir lo que salga, y mezclar a `develop` (PR, *squash*, borrado de la rama). Solo entonces `1.3b` se marca `[x]` en `PLAN-IMPLEMENTACION.md` y se aplica el cierre de sesión completo de `CLAUDE.md §3`.
