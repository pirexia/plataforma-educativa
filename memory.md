...
**Paso activo**: **1.1 · `REQ-CORE`: tenants y usuarios** `[OPUS + SONNET]`. Especificación aprobada. **Implementación COMPLETA, incluida la revisión independiente** (los 76 `CA-CORE-*` tienen test que los referencia; 183/183 tests de `apps/api` en verde, frontend con `vue-tsc`/`eslint`/`lint:i18n`/`vitest`/`build` en verde; `security-reviewer` y `doc-reviewer` ejecutados como subagentes independientes, sus hallazgos Media corregidos en la misma sesión — ver "Trabajo en curso"). **Solo queda pendiente la fusión/consolidación de rama**, que la orquestadora dijo que gestiona ella — confirmarlo antes de marcar el paso como cerrado del todo, y decidir el motor de PDF (o diferirlo explícitamente a 1.17).
...
**Rama adicional pendiente de consolidar**: `fix/REQ-CORE-larastan-schema-scan` (mismo patrón que la `-cont`: `feature/REQ-1.1-core-tenants-usuarios` seguía abierta en el checkout principal, así que el arreglo del issue #51 se hizo en rama propia desde el mismo commit base, `9574c5a`, en el *worktree* `agent-aae6327bb0dcd32ae`). Dos commits (`33d98ea`, `604a494`), ver "Trabajo en curso". La orquestadora consolida esta también junto con la `-cont`.
...
| [#55](https://github.com/pirexia/plataforma-educativa/issues/55) | `TenantMigrationTest::"sin FK antes de 0.8.4"` falla en una BD recién provisionada (asume que `users.tenant_id` no existe todavía; solo pasaba por una tabla de prueba superviviente de sesiones muy anteriores en la BD de desarrollo compartida). Rompería `ci-api.yml` en el primer run real. Necesita reescritura del test, no mecánico. | Media |

---

## Siguiente paso concreto

**1.1 está completo, incluida la revisión independiente. Lo único que falta:**

1. **Confirmar que las ramas se fusionaron/consolidaron**: `feature/REQ-1.1-core-tenants-usuarios-cont` **y** `fix/REQ-CORE-larastan-schema-scan` — la orquestadora dijo que lo gestiona ella, pero comprobar antes de dar 1.1 por cerrado que ha ocurrido (no deben quedar ramas colgando).
2. **Arrancar el siguiente paso del plan** (revisar `PLAN-IMPLEMENTACION.md` para el nombre exacto — probablemente 1.2, `REQ-AUTH`: login, canje de invitación, recuperación de contraseña, MFA — es lo primero que 1.1 dejó explícitamente bloqueado, `OPEN-CORE-01`).
...
4. **Antes de abrir el PR de 1.1 a `develop`**: el issue [#55](https://github.com/pirexia/plataforma-educativa/issues/55) (Media) hará fallar `ci-api.yml` en el primer run real (BD efímera de CI, no la de desarrollo compartida) — decidir quién reescribe `TenantMigrationTest.php` antes o justo después de abrir el PR, para no descubrirlo ya en rojo. Los issues [#52](https://github.com/pirexia/plataforma-educativa/issues/52) (Baja, `pint`, 9 ficheros) y [#54](https://github.com/pirexia/plataforma-educativa/issues/54) (Baja, `SvgSanitizer`) siguen abiertos a propósito, no bloquean nada.
