...
**Fase**: 0 — Cimientos
**Paso activo**: 0.1-0.6 cerrados. **0.7 (núcleo multi-tenant, paso crítico) EN IMPLEMENTACIÓN.** Diseño aprobado (`ADR-033`). De los 11 subpasos: **0.7.1, 0.7.2 y 0.7.10 terminados**; 0.7.3 con lo imprescindible ya hecho (conexiones `pgsql_owner`/`pgsql_platform`); 0.7.4-0.7.9 y el resto de 0.7.11 sin empezar. Siguiente: **0.7.4 (tabla `tenants` + modelo)**. Ver `Trabajo en curso` para el checklist exacto.
...
- **0.7 en implementación** (rama `feature/paso-0.7-nucleo-multitenant`). Diseño en `docs/adr/ADR-033-implementacion-del-aislamiento-multi-tenant.md`, sección "Plan de implementación" (11 subpasos, commits independientes). Checklist real de esta rama:
  ...
  - [ ] **0.7.8** Redis y almacenamiento con prefijo de tenant (el prefijo de caché básico ya funciona desde 0.7.2 y tiene test; quedan rate limit, canales de difusión y S3 por prefijo).
  ...
  - [ ] **0.7.11** Los diez tests de `tests/Feature/Tenancy/` (tabla completa en el ADR, sección 10). El de `TenantContext` (6 tests) ya cubre parte del terreno de los tests 2 (RLS vía GUC), 4 (fallo en cerrado) y 6 (aislamiento de caché) a nivel unitario; los diez de la tabla del ADR son a nivel de integración (HTTP/Eloquent) y siguen pendientes.
