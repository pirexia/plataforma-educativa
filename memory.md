...
- **`1.4c` · `REQ-AUTH-004` (parte 2/2, SAML 2.0) — EN CURSO, verificado hasta `8f1745b` (2026-09-03).** Backend y frontend completos; `security-reviewer`/`doc-reviewer` (issue #152) encontraron que la infraestructura de test del ACS estaba construida pero sin usar — cerrado con seis ficheros de test nuevos que llevan los 56 `CA-AUTH-311`-`366` a test real (`INV-015`): `SamlCatalogTest.php` (+`CA-AUTH-325`; `CA-AUTH-320` documentado como no testeable honestamente, mismo hueco que `CA-AUTH-263` de 1.4b), `SamlCertificatesTest.php`, `SamlAssertionValidationTest.php`, `SamlAcsTest.php`, `SamlLoginTest.php`, `SamlTransversalTest.php`. **Suite completa verificada por ejecución real: 506 tests Pest, 489 en verde, 17 en rojo** (`php -d memory_limit=512M ./vendor/bin/pest`, issue #106) — los 17 no son huecos: son tests correctos que revelan que `FakeSamlIdentityProviderController` nunca firma el `<samlp:Response>` (solo la `Assertion`), así que ningún login SAML simulado completa con éxito (issue #155, bloqueante, con el arreglo de una línea ya identificado). Dos hallazgos más, issues propios: #156 (`PATCH /identity-providers/{id}` nunca aplica campos exclusivos de SAML — `targetProtocol()` compara `is_string()` contra un enum que Eloquent ya devuelve casteado) y #157 (`funcional.md`/`RN-AUTH-127` y `datos.md §G.5` se contradicen sobre si la huella del certificado del IdP entra en `audit_logs`; decisión editorial, no técnica). Ninguno de los tres corregido a propósito — no era el alcance del encargo. Frontend sin cambios desde `a401898`: 118/118 Vitest en verde (no reverificado en esta vuelta). Commits del lote de tests: `da3d57c`, `4ebd612`, `b865973`, `b936a61`, `efeb390`, `8f1745b`. **Qué falta con certeza**: resolver #155 (desbloquea 17 tests sin tocarlos), decidir #156/#157, y solo entonces relanzar la revisión independiente completa (`security-reviewer`/`db-reviewer`/`doc-reviewer`) antes de cerrar y mezclar — issue #152 sigue abierto a propósito, no se cierra sin la verificación real que pedía.
