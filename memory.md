...
**Fase**: 0 — Cimientos
**Paso activo**: 0.1-0.7 cerrados y mezclados a `develop`. Todos los cabos sueltos de infraestructura de 0.7 resueltos (ver punto 2 de `Siguiente paso concreto`). **En curso: 0.8 · Modelo de datos núcleo** `[OPUS]`.
**Rama**: `develop` es ahora la **rama por defecto del repositorio** (cambiada desde `main` el 2026-08-18, ver `Siguiente paso concreto`). Local limpia, sincronizada con `origin/develop`. Sin ramas de trabajo abiertas.
**Última sesión (2026-08-17/18)**: cerrado el paso 0.7 (núcleo multi-tenant) completo — diseño, 11 subpasos de implementación, revisión de `db-reviewer`/`security-reviewer`, corrección de hallazgos, y mezcla a `develop`. Detalle completo archivado en `docs/historial/0.7-nucleo-multitenant.md`. Sesión siguiente (2026-08-18) corrigió esta misma entrada (se había quedado desactualizada por un corte de cuota) y cerró los pasos manuales de GitHub pendientes: branch protection, Dependabot alerts, Renovate (con la sorpresa del `renovate.json` sobrescrito en `main`, resuelta cambiando la rama por defecto a `develop`).
