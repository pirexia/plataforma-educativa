...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2, 1.2b, 1.3 y 1.3b cerrados y mezclados.** Bloque A completo. `chore/` de vigencia de documentación **cerrado 2026-08-31** (ver más abajo). Siguiente paso: **`1.4`**.
...
**`chore/` de vigencia de documentación + CI — CERRADO Y MEZCLADO** (2026-08-31). Tres PR: [#130](https://github.com/pirexia/plataforma-educativa/pull/130) (docs, issues #111-#114/#125-#129), [#133](https://github.com/pirexia/plataforma-educativa/pull/133) (CI rota preexistente, issues #131/#132/#135/#136), [#138](https://github.com/pirexia/plataforma-educativa/pull/138) (fix `available_methods` MFA, issue #137). Hallazgo mayor: **la CI llevaba rota desde hacía fases sin que nadie lo hubiera podido comprobar** (permiso de checks del token, resuelto en esta sesión) — 4 capas de fallos en cascada, incluida la construcción de la imagen de producción de la API rota desde 1.2. Los tres PR terminan con los 14 *checks* en verde de verdad (suite completa de 351 tests). 13 issues más cerrados por hallarse ya resueltos en código y dejados abiertos por descuido, tras auditar los 49 issues abiertos del repositorio. Detalle completo en `docs/historial/chore-vigencia-documentacion-y-ci.md`.
...
**Rama activa**: `develop`, limpia y sincronizada con `origin`. Sin rama de trabajo abierta — toca crear una nueva para `1.4`.
...
**`chore/` de vigencia de documentación + CI cerrado y mezclado el 2026-08-31** (PR [#130](https://github.com/pirexia/plataforma-educativa/pull/130)/[#133](https://github.com/pirexia/plataforma-educativa/pull/133)/[#138](https://github.com/pirexia/plataforma-educativa/pull/138)). Detalle completo en `docs/historial/chore-vigencia-documentacion-y-ci.md`. Ramas locales y remotas borradas. **Bloque A de la fase 1 (`1.1`-`1.3b`) completo, y la CI del repositorio verificada en verde de verdad por primera vez.**

**Siguiente paso del plan**: **`1.4 · REQ-AUTH: login con Google y fusión de cuentas`** [SONNET].

**Nota para el arranque de `1.4`**: el token del MCP de GitHub usado en sesiones anteriores parecía no tener permiso para leer *checks*/Actions (`403` con `gh` CLI); resultó ser que las llamadas del MCP (`mcp__github__pull_request_read` método `get_check_runs`/`get_status`) sí funcionan y usan credenciales distintas a las del `gh` CLI del shell — usar esas, no `gh api .../check-runs` ni `gh run list`, que siguen dando 403.
