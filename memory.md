...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2 y 1.2b cerrados y mezclados. `1.3` (MFA — TOTP, obligatoriedad por rol, restablecimiento): implementación COMPLETA (backend + frontend) y VERIFICADA de verdad, en revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer` lanzados 2026-08-27, resultado pendiente). Falta corregir sus hallazgos y mezclar (ver "Siguiente paso concreto").**

**Cierre por cuota, segunda vez consecutiva** (2026-08-26→27, madrugada), resuelto: dos subagentes `implementer` seguidos murieron por "session limit" de la cuenta (no por fallo de su trabajo), cada vez protegido con commit `wip` + push. **2026-08-27, retomado**: causa raíz del bloqueo de verificación encontrada y corregida — `compose.yaml` no fijaba `target: dev` en el build multi-etapa de `api`/`web` desde `0.9b` (issue [#96](https://github.com/pirexia/plataforma-educativa/issues/96), corregido, imágenes reconstruidas). Con eso, **backend de `1.3` verificado de verdad**: `pint`/`phpstan` limpios, **317 tests en verde** (39 de MFA), sin regresiones.

**Hallazgo importante, 2026-08-27**: el segundo `implementer` había recortado `GET /mfa-compliance/users` (listado individualizado de cumplimiento) a `1.3b` **sin autorización** — la especificación aprobada (commit `ce399bc`) sí lo incluía en `1.3`, distinto del agregado. El usuario, al revisarlo, decidió **restaurarlo en `1.3`**. Dos `implementer` lanzados en paralelo el 2026-08-27, ambos terminados con éxito:
- **Restauración + reconciliación de documentación** (commits `d3c137d`/`b22dd59`): `MfaComplianceDirectory::listUsers()`, filtro `state` con `obligated` como alias servidor de `pending`+`past_deadline` (el dominio no tiene ese tercer estado — decisión documentada, no inventada), paginación `ADR-038 §4.3`. Conteos de endpoints/permisos reconciliados en `api.md`/`permisos.md`/`funcional.md`. Issue [#97](https://github.com/pirexia/plataforma-educativa/issues/97) abierto por un fallo de validación YAML preexistente en `components.yaml` (ajeno a 1.3, no corregido, fuera de alcance).
- **Frontend completo** (commit `51682e5`): 4 pantallas/componentes de `§C.11` (`/entrar` en dos pasos, `/cuenta/seguridad`, `/cuenta/seguridad/obligatorio`, `QrCode.vue` con `uqr`), i18n 4 idiomas, WCAG, verificado en navegador real con Playwright MCP (tenant de demo ficticio, `ADR-030`/`REQ-SEED-005`).

**Verificación final combinada, 2026-08-27** (sesión orquestadora, tras converger ambas ramas de trabajo en la misma rama de git): `pint`/`phpstan` limpios, **320 tests Pest en verde** (`memory_limit=512M` — el límite de 128M del PHP CLI no basta para la suite completa por un test ajeno de subida de ficheros, `UploadBrandingAsset.php`, no relacionado con 1.3). Frontend: `eslint`, `lint:i18n`, `vue-tsc`+`build`, Vitest (13/13) todos en verde. **`1.3` funcionalmente completo y verificado.** En revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) antes de mezclar — es la única pieza que falta.
...
**Commit más reciente**: `b22dd59` (rama `feature/REQ-AUTH-003-1.3-mfa-obligatorio-por-rol`, empujada a `origin`, sincronizada). Implementación completa y verificada — ver "Estado actual".

1. **Recoger el resultado de la revisión independiente** (`security-reviewer`/`db-reviewer`/`doc-reviewer`, lanzados 2026-08-27 en paralelo sobre el diff completo `develop...HEAD`) y corregir lo que encuentren, con el mismo criterio que `1.2`/`1.2b`: Alta/Crítica se corrige antes de mezclar sin excepción, Media se corrige si no descarrila, Baja se documenta y se pregunta.
2. **Tras corregir hallazgos**: re-verificar (`pint`/`phpstan`/Pest backend, `eslint`/`lint:i18n`/`vue-tsc`+build/Vitest frontend), commit, push.
3. **Cerrar el paso**: marcar `1.3` como `[x]` en `PLAN-IMPLEMENTACION.md` con el resumen de cierre (patrón de `1.2b`), actualizar `memory.md`, mezclar a `develop` (squash, como los pasos anteriores), borrar la rama local y remota.
4. **`1.3b`** (correo como 2FA, excepciones temporales nominales — su modelo de datos, `user_mfa_exemptions`, ya existe desde `1.3`, solo faltan los 3 endpoints y la lógica —, pantalla de administración si `1.5` se retrasa) es un paso posterior con **rama propia nueva**.
5. **Requisito de entorno para probar en navegador real** (issue #71): la SPA se sirve desde `demo.plataforma.test:5173`; necesita `127.0.0.1 demo.plataforma.test` en el `hosts` de **Windows** — confirmado presente.
6. **Nota de entorno, corregida 2026-08-27**: `compose.yaml` ya fija `target: dev` en `api`/`web` (issue #96, cerrado) — cualquier `podman compose build` futuro reconstruye correctamente con Composer/herramientas de desarrollo. Contenedor `api`: `podman exec -w /var/www/html plataforma-api <cmd>`. Contenedor `web`: **`WORKDIR` es `/app`, no `/var/www/html`** — `podman exec -w /app plataforma-web <cmd>`. Suite Pest completa necesita `php -d memory_limit=512M` (128M por defecto no basta, test ajeno de subida de ficheros en `Core`, no relacionado con `Auth`).
7. **`.env` sigue sin ser editable desde esta sesión** (bloqueo de permisos). `SESSION_LIFETIME=480`, `CORS_ALLOWED_ORIGINS` y `VITE_API_URL` siguen parcheados en `compose.yaml` en vez de en sus `.env` respectivos.
