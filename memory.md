...
**Hallazgo importante, 2026-08-27**: el segundo `implementer` había recortado `GET /mfa-compliance/users` (listado individualizado de cumplimiento) a `1.3b` **sin autorización** — la especificación aprobada (commit `ce399bc`) sí lo incluía en `1.3`, distinto del agregado. El usuario, al revisarlo, decidió **restaurarlo en `1.3`**. Dos `implementer` lanzados en paralelo el 2026-08-27 (independientes entre sí): uno restaura ese endpoint y reconcilia la documentación (conteos de endpoints/permisos en `api.md`/`permisos.md`/`funcional.md`), otro construye **el frontend entero de `1.3`**, que hasta ahora tenía **cero trabajo hecho** (4 pantallas de `§C.11`, `QrCode.vue` + `uqr` de `ADR-041`, i18n, WCAG 2.2 AA, verificación en navegador real con Playwright). Resultado pendiente de notificación.
