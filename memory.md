...
- **`1.4c` · `REQ-AUTH-004` (parte 2/2, SAML 2.0) — EN CURSO, verificado hasta `630b895` (2026-09-04).** Backend y frontend completos, con los 56 `CA-AUTH-311`-`366` a test real (`INV-015`). Los tres hallazgos que dejó la tanda anterior (#155, #156, #157) más uno nuevo destapado al resolverlos, los cuatro corregidos y verificados yo mismo, no de palabra:
  ...
  **Suite completa verificada por ejecución real tras las cuatro correcciones: 506/506 Pest en verde** (`php -d memory_limit=512M ./vendor/bin/pest`, issue #106), **118/118 Vitest sin cambios**. Commits de arreglo: `df6c931`, `b6249bb`, `b56ef1f`, `630b895`. **Qué falta con certeza**: cerrar issues #155/#156/#157/#152 en GitHub (arreglados, sin cerrar formalmente todavía), y relanzar la revisión independiente completa (`security-reviewer`/`db-reviewer`/`doc-reviewer`) sobre el estado ya corregido antes de cerrar y mezclar el paso — la pasada anterior de revisión fue sobre código que todavía tenía estos cuatro fallos.
