...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2 y 1.2b cerrados y mezclados. `1.3` (MFA — TOTP, obligatoriedad por rol, restablecimiento): especificación APROBADA 2026-08-26, partida en `1.3`/`1.3b`, `ADR-041` de dependencias cerrado. Nada pendiente antes de implementar (ver "Siguiente paso concreto").**
...
1. **Implementación de `1.3`** (subagente `implementer`, Sonnet), único paso pendiente antes de revisión, con el alcance fijado en `docs/modulos/REQ-AUTH/funcional.md §C.16` y los envoltorios exactos fijados por `ADR-041`: `app/Modules/Auth/Domain/MfaVerifier.php` (firma de `§C.9.2`), `app/Modules/Auth/Domain/TotpProvisioner.php`, `app/Modules/Auth/Infrastructure/Google2FaTotpVerifier.php` (único fichero que importa `PragmaRX\Google2FA`, *binding* en `AuthServiceProvider`, patrón `IpGeolocator`/`NullIpGeolocator`), `apps/web/src/components/QrCode.vue` (props `{ value, label, moduleSize? }`, fuera de `components/ui/` y de `modules/auth/`). **Tres trampas ya verificadas a evitar** (detalle completo arriba en "Estado actual"): `generateSecretKey()` mide caracteres base32 no bytes (pasar 32, no 20), `verifyKeyNewer()` devuelve `int|false` (comparar con `false === `, guardar el entero en `last_used_step`), y usar `uqr.encode()` + plantilla Vue propia — **nunca** `uqr.renderSVG()`.
2. **Tras implementar**: revisión independiente (`security-reviewer`/`db-reviewer`/`doc-reviewer`) antes de mezclar, como en `1.2`/`1.2b`.
3. **`1.3b`** (correo como 2FA, excepciones temporales, pantalla de administración si `1.5` se retrasa) es un paso posterior con **rama propia nueva** — no confundir con esta rama.
4. **Requisito de entorno para probar en navegador real** (issue #71): `apps/web/vite.config.ts` y `compose.yaml` sirven la SPA desde `demo.plataforma.test:5173`; necesita `127.0.0.1 demo.plataforma.test` en el `hosts` de **Windows** (no el de WSL2) — ya confirmado presente en esta máquina de desarrollo.
