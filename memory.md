...
| [#78](https://github.com/pirexia/plataforma-educativa/issues/78) | Generalizar `REQ-GOB-004` (AMPA) a módulo de varias asociaciones internas por tenant, con roles propios y admin delegado. **Alcance cerrado 2026-08-25**: incluye cuotas de socio con control de impago y firma electrónica, reutilizando la infraestructura de `REQ-FIN`/`REQ-DOC` (independencia solo legal/contable, no técnica). Investigación de mercado hecha (sin precedente para el admin delegado). Pendiente de `spec-writer`/`architect`. | Planificación |
