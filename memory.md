...
**Fase**: 0 cerrada en la práctica (lo pendiente de `0.10`-`0.12` es negocio, no código — ver "Bloqueantes"). **Fase 1, bloque A: 1.1, 1.2, 1.2b y 1.3 cerrados y mezclados. `1.3b` en curso** (piezas 1/2/4 de 4 completas, pieza 3 —pantalla de administración— pendiente; detalle en "Siguiente paso concreto").
...
4. **Pieza 3 (pantalla `/administracion/mfa`) — NO EMPEZADA.** Es el siguiente paso, y el punto correcto para retomar: no aporta ni un endpoint ni un permiso nuevo (consume los siete que ya existen tras la pieza 2), autónoma sin `AppLayout`/design system, cuatro áreas (cumplimiento, conmutador `mfa_required` con vista previa, restablecimiento, excepciones), `CA-AUTH-176`. Detalle completo en `funcional.md §D.1.3`/`§D.9.1`.
5. **Documentación actualizada ya en esta rama** (piezas 1-2-4): `SYSADMIN.md` (variables de entorno, incluida `AUTH_MFA_EXEMPTION_REOPEN_WINDOW_HOURS` nueva), `RUNBOOK.md` (matiz de recuperación de `APP_KEY` con factor de correo, consecuencia de revertir con factores de correo confirmados), `docs/manual-usuario/admin.md` (correo como 2FA, excepciones temporales — **sin** describir la pantalla de administración, que todavía no existe). **Pendiente tras la pieza 3**: `SYSADMIN.md`/`RUNBOOK.md` no necesitan más cambios previsibles; el manual de administración si necesitará la referencia a la ruta `/administracion/mfa` en cuanto exista.
...
7. **Nada de OpenAPI, `pint`, `phpstan` pendiente para lo ya hecho**: las tres piezas commiteadas pasan `pint --test`/`phpstan` limpios y la suite completa de `Auth` (169 tests) en verde. **Sí queda pendiente, para el cierre final del paso completo**: `security-reviewer`/`db-reviewer`/`doc-reviewer`.
7b. **Corrección 2026-08-27, 22h**: la afirmación anterior de este punto ("el frontend no se ha tocado porque la pieza 3 es la única con código de frontend") era **falsa** — error de seguimiento de la sesión orquestadora, no de un subagente. `funcional.md §D.1.4`/§D.9 ya contaban, dentro del tamaño aprobado del paso, **3 pantallas de autoservicio ampliadas + 1 componente nuevo (`MfaEmailEnrollment.vue`)** como parte de la pieza 1 (correo como segundo factor): selector de método con destino enmascarado y cuenta atrás en `LoginView.vue`, alta por correo en `AccountSecurityView.vue` y en el muro de obligatoriedad. La pieza 1 solo tenía el backend completo — **sin ninguna forma de que un usuario real lo usara**. Detectado por el `implementer` de la pieza 3 al leer la especificación completa antes de tocar código, en vez de asumir que el encargo recibido era correcto (`CLAUDE.md §0`). En curso de implementación junto con la pieza 3.
